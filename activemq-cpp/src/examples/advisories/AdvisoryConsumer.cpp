...
void AdvisoryConsumer::onMessage(const cms::Message* message)
{
    if (message->getCMSType() == "Advisory")
    {
        ...
        if (amqMessage != NULL && amqMessage->getDataStructure() != NULL)
        {
            ...
            std::cout << "Got ProducerInfo for producer: "
                      << info->getProducerId()->toString() << std::endl;
        }

        if (message->propertyExists("producerCount"))
        {
            std::string producerCount =
                message->getStringProperty("producerCount");
    ...
    else
    {
        ...
        if (txtMessage != NULL)
        {
            std::cout << "Producer Reports Status as: " << txtMessage->getText()
                      << std::endl;
        }
    }
}
