...
void DisplayResults()
{
    ...
    if (TestResults.sent.get() > 0)
    {
        printf("Reliability = %0.3f%%\n",
               (double)(((double)(TestResults.received.get()) * 100) /
                        (double)TestResults.sent.get()));
        printf("Sequenced = %0.3f%%\n",
               (double)(((double)(TestResults.received.get() -
                                  TestResults.badSequenceMessages.get()) *
                         100) /
                        (double)TestResults.sent.get()));
        printf("Output performance = %0.3f ms/msg\n",
               (double)((double)mills / (double)TestResults.sent.get()));
        if ((TestResults.received.get() + TestResults.invalidMessages.get()) >
            0)
        {
            printf("Input performance = %0.3f ms/msg\n",
                   (double)((double)mills /
                            (double)(TestResults.received.get() +
                                     TestResults.invalidMessages.get())));
        }
    }
...
int main(int argc, char** argv)
{
    ...
    if (!bOK)
    {
        printf("Usage: %s [-b #] [-i #] [-m] [-n name] [-p] [-r] [-s #] [-t #] "
               "[-u header] [-v] [-x # ]\n",
               argv[0]);
        printf(" -a   : Do not use thread pools when dispatching incoming "
               "messages (default is to use pools)\n");
        printf(" -b # : Number of seconds between heartbeat messages (0 "
               "disables) (default is 5)\n");
        printf(" -f # : Minimum number of threads to have available when use "
               "thread pools (default is 3)\n");
        printf(" -g # : Maximum number of threads that can be created in each "
               "thread pool (default is 100)\n");
        printf(" -i # : Number of milliseconds to wait between message sends "
               "(-1 is random delay between 0 and 1000ms) (default is -1)\n");
        printf(" -n s : Specify the name of the topic to use (default is "
               "'topic')\n");
        printf(" -p   : Pause for keyboard input (default is false)\n");
        printf(" -r   : Display test summary (default is false)\n");
        printf(" -s # : Number of seconds to run test for (0 is until CTRL+C "
               "is entered) (default is 3600 - 1hr)\n");
        printf(" -t # : Number of threads to use (default is 25)\n");
        printf(" -u s : Use the specified header name for ID and utilize "
               "selectors\n");
        printf(" -v   : Display verbose progress where the following symbols "
        ...
        printf(
            "        (%c) = Received message sequence is off by 10 or more\n",
            SYM_BIG_DIFF);
        printf("        (#) = Number indicating how far off received sequence "
               "number is\n");
        printf("        (%c) = Good heartbeat sent and received\n",
               SYM_MON_GOOD);
        printf("        (%c) = Failed to send and receive heartbeat\n",
               SYM_MON_BAD);
        printf("        (%c) = CMS exception received while send/receiving "
               "heartbeat\n",
               SYM_MON_CMS);
        printf("        (%c) = Exception received while send/receiving "
               "heartbeat\n",
               SYM_MON_EXC);
        printf(" -x # : Seed the random number generator with the specified "
               "seed (0 uses time) (default is 0)\n");
        printf("\n");
        return -1;
    }
    ...
    for (int i = 0; i < cnt; i++)
    {
        ...
        sar[i] = new TestSenderAndReceiver(url,
                                           topic.c_str(),
                                           nhdr.c_str(),
                                           true,
                                           false,
