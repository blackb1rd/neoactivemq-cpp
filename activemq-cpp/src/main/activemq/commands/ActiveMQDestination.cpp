...
const std::string ActiveMQDestination::TEMP_QUEUE_QUALIFED_PREFIX =
    "temp-queue://";
const std::string ActiveMQDestination::TEMP_TOPIC_QUALIFED_PREFIX =
    "temp-topic://";
...
ActiveMQDestination::ActiveMQDestination()
    : BaseDataStructure(),
      ...
      orderedTarget(DEFAULT_ORDERED_TARGET),
      physicalName(),
      options(),
      hashCode()
{
...
bool ActiveMQDestination::equals(const DataStructure* value) const
{
    ...
    const ActiveMQDestination* valuePtr =
    ...
    const std::string& name)
{
    ...
    {
        result.reset(
            new ActiveMQQueue(name.substr(QUEUE_QUALIFIED_PREFIX.length())));
        return result;
    }
    else if (name.find(TOPIC_QUALIFIED_PREFIX) == 0)
    {
        result.reset(
            new ActiveMQTopic(name.substr(TOPIC_QUALIFIED_PREFIX.length())));
        return result;
    }
    else if (name.find(TEMP_QUEUE_QUALIFED_PREFIX) == 0)
    {
        result.reset(new ActiveMQTempQueue(
            name.substr(TEMP_QUEUE_QUALIFED_PREFIX.length())));
        return result;
    }
    else if (name.find(TEMP_TOPIC_QUALIFED_PREFIX) == 0)
    {
        result.reset(new ActiveMQTempTopic(
            name.substr(TEMP_TOPIC_QUALIFED_PREFIX.length())));
        return result;
    }
...
ActiveMQDestination::getCompositeDestinations() const
{
    ...
    if (this->isComposite())
    {
        ...
        StringTokenizer     iter(this->physicalName,
                             ActiveMQDestination::COMPOSITE_SEPARATOR);
...
int ActiveMQDestination::compareTo(const ActiveMQDestination& value) const
{
    ...
    int valueComp = StringUtils::compare(this->getPhysicalName().c_str(),
                                         value.getPhysicalName().c_str());
    if (valueComp != 0)
    {
        return valueComp;
    }
