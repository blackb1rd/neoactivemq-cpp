...
namespace commands
{
    ...
    {
    private:
        std::unique_ptr<wireformat::openwire::utils::MessagePropertyInterceptor>
    ...
    public:
        ...
        virtual cms::Message::ValueType getPropertyValueType(
            const std::string& name) const
        {
            try
            {
                // Ensure properties are unmarshaled before accessing
                this->ensurePropertiesUnmarshaled();
                util::PrimitiveValueNode::PrimitiveType type =
                    this->getMessageProperties().getValueType(name);

                // Just map the values that are actually allowed in Message
                // Properties, the others all qualify as unknown.
                switch (type)
                {
        ...
        virtual bool getBooleanProperty(const std::string& name) const
        {
            try
            {
                // Ensure properties are unmarshaled before accessing (lazy
                // unmarshal for corrupted message detection)
                this->ensurePropertiesUnmarshaled();
        ...
        virtual void setBooleanProperty(const std::string& name, bool value)
        {
            if (name == "")
            {
                throw cms::CMSException(
                    "Message Property names must not be empty",
                    NULL);
            }
        ...
        virtual std::string getCMSMessageID() const
        {
            try
            {
                return wireformat::openwire::marshal::BaseDataStreamMarshaller::
                    toString(this->getMessageId().get());
            }
            AMQ_CATCH_ALL_THROW_CMSEXCEPTION()
        }
