...
void ActiveMQStreamMessage::copyDataStructure(const DataStructure* src)
{
    ...
    const ActiveMQStreamMessage* srcPtr =
    ...
    ActiveMQStreamMessage* nonConstSrc =
        const_cast<ActiveMQStreamMessage*>(srcPtr);
...
char ActiveMQStreamMessage::readChar() const
{
    ...
    try
    {
        ...
        {
            // Java Char is two bytes (UTF-16); discard high byte, return low byte.
...
void ActiveMQStreamMessage::writeChar(char value)
{
    ...
    try
    {
        ...
        // Java Char is two bytes (UTF-16); write high byte then low byte.
...
unsigned short ActiveMQStreamMessage::readUnsignedShort() const
{
    ...
    try
    {
        ...
        if (type == PrimitiveValueNode::STRING_TYPE)
        {
            return (unsigned short)Short::valueOf(this->dataIn->readUTF())
                .shortValue();
        }
...
std::string ActiveMQStreamMessage::readString() const
{
    ...
    try
    {
        ...
        if (type == PrimitiveValueNode::BOOLEAN_TYPE)
        {
            return (this->dataIn->readBoolean() ? Boolean::_TRUE
                                                : Boolean::_FALSE)
                .toString();
        }
        ...
        if (type == PrimitiveValueNode::CHAR_TYPE)
        {
            // Java Char is two bytes (UTF-16); discard high byte, use low byte.
            this->dataIn->readByte();
    ...
    try
    {
        if (this->impl->remainingBytes != -1)
        {
            throw cms::IllegalStateException(
                "Cannot read the next type during an byte array read "
                "operation, complete the read first.");
        }
        ...
        switch (type)
        {
            ...
            default:
                throw MessageFormatException("Unknown type found in stream",
                                             NULL);
        }
    ...
    if (this->dataOut.get() != NULL)
    {
        ...
        if (this->impl->bytesOut->size() > 0)
        {
            std::pair<unsigned char*, int> array =
                this->impl->bytesOut->toByteArray();
            this->setContent(
