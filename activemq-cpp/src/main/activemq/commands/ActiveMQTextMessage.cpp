...
{
    std::unique_ptr<ActiveMQTextMessage> message(new ActiveMQTextMessage());
...
void ActiveMQTextMessage::copyDataStructure(const DataStructure* src)
{
    ...
    const ActiveMQTextMessage* srcPtr =
        dynamic_cast<const ActiveMQTextMessage*>(src);
    ...
    try
    {
        ...
        else
        {
            ...
            try
            {
                ...
                this->text.reset(
                    new std::string(MarshallingSupport::readString32(dataIn)));
