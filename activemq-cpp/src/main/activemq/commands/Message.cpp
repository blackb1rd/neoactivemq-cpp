...
void Message::copyDataStructure(const DataStructure* src)
{
    ...
    // 2. beforeMarshal() clears marshalledProperties and re-marshals from
    // properties map
    // 3. If properties map is empty, marshalledProperties stays empty
    // 4. Later unmarshal will fail to restore properties -> "Key does not exist
    // in map"
...
std::string Message::toString() const
{
    ...
    stream << ", ";
    stream << "JMSXGroupFirstForConsumer = "
           << this->isJMSXGroupFirstForConsumer();
    stream << " }";
...
bool Message::equals(const DataStructure* value) const
{
    ...
    for (size_t imarshalledProperties = 0;
         imarshalledProperties < this->getMarshalledProperties().size();
         ++imarshalledProperties)
    {
        if (this->getMarshalledProperties()[imarshalledProperties] !=
            valuePtr->getMarshalledProperties()[imarshalledProperties])
        {
            return false;
        }
    }
    ...
    if (this->isJMSXGroupFirstForConsumer() !=
        valuePtr->isJMSXGroupFirstForConsumer())
    {
        return false;
    }
...
////////////////////////////////////////////////////////////////////////////////
void Message::setMarshalledProperties(
...
void Message::afterUnmarshal(wireformat::WireFormat* wireFormat AMQCPP_UNUSED)
{
    ...
    // This allows corrupted properties to be detected when consumer accesses
    // them, enabling proper exception handling and redelivery mechanisms
    propertiesUnmarshaled = false;
}
...
void Message::ensurePropertiesUnmarshaled() const
{
    ...
    synchronized(&propertiesUnmarshalMutex)
    {
        ...
        try
        {
            ...
            // If this fails (corrupted properties), exception propagates to
            // consumer code Consumer will catch it, rollback transaction, and
            // trigger redelivery
