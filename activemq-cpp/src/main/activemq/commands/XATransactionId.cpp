      ...
      branchQualifier()
{
    ...
    if (result == -1)
    {
        throw cms::XAException(
            "Invalid Xid Branch Qualifier is larger than MAXBQUALSIZE");
    }
    else
    {
    ...
    {
        throw cms::XAException(
            "Invalid Xid Global Transaction Id is larger than MAXGTRIDSIZE");
    }
    else
    {
...
std::string XATransactionId::toString() const
{
    ...
    {
        stream << "[";
        for (size_t iglobalTransactionId = 0;
             iglobalTransactionId < this->getGlobalTransactionId().size();
    ...
    {
        stream << "[";
        for (size_t ibranchQualifier = 0;
             ibranchQualifier < this->getBranchQualifier().size();
             ++ibranchQualifier)
        {
...
bool XATransactionId::equals(const DataStructure* value) const
{
    ...
    const XATransactionId* valuePtr =
        dynamic_cast<const XATransactionId*>(value);
    ...
    for (size_t iglobalTransactionId = 0;
         ...
         ++iglobalTransactionId)
    {
        if (this->getGlobalTransactionId()[iglobalTransactionId] !=
            valuePtr->getGlobalTransactionId()[iglobalTransactionId])
        {
            return false;
        }
    }
    for (size_t ibranchQualifier = 0;
         ...
         ++ibranchQualifier)
    {
        if (this->getBranchQualifier()[ibranchQualifier] !=
            valuePtr->getBranchQualifier()[ibranchQualifier])
        {
            return false;
        }
    }
...
////////////////////////////////////////////////////////////////////////////////
void XATransactionId::setGlobalTransactionId(
...
////////////////////////////////////////////////////////////////////////////////
void XATransactionId::setBranchQualifier(
...
int XATransactionId::getBranchQualifier(unsigned char* buffer, int size) const
{
    ...
    {
        throw cms::XAException(
            "Error: Negative size value passed to getBranchQualifier()");
    }

    if (buffer == NULL)
    {
        throw cms::XAException(
            "Error: NULL buffer pointer passed to getBranchQualifier()");
    }
...
////////////////////////////////////////////////////////////////////////////////
int XATransactionId::getGlobalTransactionId(unsigned char* buffer,
                                            int            size) const
{
    if (size < 0)
    {
        throw cms::XAException(
            "Error: Negative size value passed to getGlobalTransactionId()");
    }

    if (buffer == NULL)
    {
        throw cms::XAException(
            "Error: NULL buffer pointer passed to getGlobalTransactionId()");
    }
