...
namespace core
{
    namespace kernels
    {
        ...
        /**
         * Class used to clear a Consumer's dispatch queue asynchronously from
         * the connection class's Scheduler instance.
         */
        ...
        /**
         * Class used to Hook a session that has been closed into the
         * Transaction it is currently a part of.  Once the Transaction has been
         * Committed or Rolled back this Synchronization can finish the Close of
         * the session.
         */
        ...
        public:
            CloseSynhcronization(ActiveMQSessionKernel* session,
                                 SessionConfig*         config)
                : Synchronization(),
                  session(session),
                  config(config)
      ...
      lastDeliveredSequenceId(-2)
{
    if (id == NULL || connection == NULL)
    {
        throw ActiveMQException(__FILE__,
                                __LINE__,
                                "ActiveMQSessionKernel::ActiveMQSessionKernel "
                                "- Constructor called with NULL data");
    }
    ...
    AMQ_LOG_INFO("SessionKernel",
                 "Session created, sessionId=" << id->toString()
                                               << " ackMode=" << (int)ackMode);
    ...
    try
    {
        AMQ_LOG_INFO("SessionKernel",
                     "Closing session, sessionId="
                         << this->sessionInfo->getSessionId()->toString());
        dispose();
...
void ActiveMQSessionKernel::dispose()
{
    ...
    public:
        Finalizer(ActiveMQSessionKernel* session,
                  ActiveMQConnection*    connection)
            : session(session),
              connection(connection)
        {
    ...
    try
    {
        ...
        // Roll Back the transaction since we were closed without an explicit
        // call to commit it.
...
void ActiveMQSessionKernel::commit()
{
    try
    {
        ...
        if (!this->isTransacted())
        {
            throw ActiveMQException(__FILE__,
                                    __LINE__,
                                    "ActiveMQSessionKernel::commit - This "
                                    "Session is not Transacted");
        }

        AMQ_LOG_DEBUG("SessionKernel",
                      "Committing transaction, sessionId="
                          << this->sessionInfo->getSessionId()->toString());
        ...
        AMQ_LOG_INFO("SessionKernel",
                     "Transaction committed, sessionId="
                         << this->sessionInfo->getSessionId()->toString());
    }
    AMQ_CATCH_ALL_THROW_CMSEXCEPTION()
}
...
void ActiveMQSessionKernel::rollback()
{
    try
    {
        ...
        if (!this->isTransacted())
        {
            throw ActiveMQException(__FILE__,
                                    __LINE__,
                                    "ActiveMQSessionKernel::rollback - This "
                                    "Session is not Transacted");
        }

        AMQ_LOG_DEBUG("SessionKernel",
                      "Rolling back transaction, sessionId="
                          << this->sessionInfo->getSessionId()->toString());
        ...
        AMQ_LOG_INFO("SessionKernel",
                     "Transaction rolled back, sessionId="
                         << this->sessionInfo->getSessionId()->toString());
    }
    AMQ_CATCH_ALL_THROW_CMSEXCEPTION()
}
...
////////////////////////////////////////////////////////////////////////////////
cms::MessageConsumer* ActiveMQSessionKernel::createConsumer(
    ...
    bool                    noLocal)
{
    try
    {
        ...
        if (amqDestination == NULL)
        {
            throw ActiveMQException(__FILE__,
                                    __LINE__,
                                    "Destination was either NULL or not "
        ...
        {
            prefetch =
                this->connection->getPrefetchPolicy()->getTopicPrefetch();
        }
        else
        {
            prefetch =
                this->connection->getPrefetchPolicy()->getQueuePrefetch();
        }
        ...
        try
        {
            ...
            AMQ_LOG_DEBUG("SessionKernel",
                          "createConsumer() sending ConsumerInfo cmdId="
...
////////////////////////////////////////////////////////////////////////////////
cms::MessageConsumer* ActiveMQSessionKernel::createDurableConsumer(
    ...
    bool               noLocal)
{
    try
    {
        ...
        try
        {
            this->addConsumer(consumer);
            AMQ_LOG_DEBUG("SessionKernel",
                          "createDurableConsumer() sending ConsumerInfo cmdId="
                              << consumer->getConsumerInfo()->getCommandId()
                              << " dest=" << dest->getPhysicalName()
                              << " sub=" << name << " selector=" << selector);
            this->connection->syncRequest(consumer->getConsumerInfo());
            AMQ_LOG_DEBUG(
                "SessionKernel",
...
////////////////////////////////////////////////////////////////////////////////
cms::MessageProducer* ActiveMQSessionKernel::createProducer(
    const cms::Destination* destination)
{
    try
    {
        ...
        Pointer<ActiveMQProducerKernel> producer(
            new ActiveMQProducerKernel(this,
                                       ...
                                       dest,
                                       this->connection->getSendTimeout()));
...
////////////////////////////////////////////////////////////////////////////////
cms::QueueBrowser* ActiveMQSessionKernel::createBrowser(
    const cms::Queue*  queue,
    const std::string& selector)
{
    try
    {
...
cms::Queue* ActiveMQSessionKernel::createQueue(const std::string& queueName)
{
    try
    {
        ...
        if (queueName.find(
                commands::ActiveMQDestination::TEMP_DESTINATION_NAME_PREFIX) ==
            0)
        {
...
cms::TemporaryQueue* ActiveMQSessionKernel::createTemporaryQueue()
{
    try
    {
        ...
        std::unique_ptr<commands::ActiveMQTempQueue> queue(
            new commands::ActiveMQTempQueue(
                this->createTemporaryDestinationName()));
...
cms::TemporaryTopic* ActiveMQSessionKernel::createTemporaryTopic()
{
    try
    {
        ...
        std::unique_ptr<commands::ActiveMQTempTopic> topic(
            new commands::ActiveMQTempTopic(createTemporaryDestinationName()));
...
cms::BytesMessage* ActiveMQSessionKernel::createBytesMessage()
{
    ...
    {
        this->checkClosed();
        commands::ActiveMQBytesMessage* message =
            new commands::ActiveMQBytesMessage();
        message->setConnection(this->connection);
        return message;
    }
    AMQ_CATCH_ALL_THROW_CMSEXCEPTION()
...
cms::StreamMessage* ActiveMQSessionKernel::createStreamMessage()
{
    ...
    {
        this->checkClosed();
        commands::ActiveMQStreamMessage* message =
            new commands::ActiveMQStreamMessage();
        message->setConnection(this->connection);
        return message;
    }
    AMQ_CATCH_ALL_THROW_CMSEXCEPTION()
...
cms::TextMessage* ActiveMQSessionKernel::createTextMessage()
{
    ...
    {
        this->checkClosed();
        commands::ActiveMQTextMessage* message =
            new commands::ActiveMQTextMessage();
        message->setConnection(this->connection);
        return message;
    }
    AMQ_CATCH_ALL_THROW_CMSEXCEPTION()
...
cms::MapMessage* ActiveMQSessionKernel::createMapMessage()
{
    ...
    {
        this->checkClosed();
        commands::ActiveMQMapMessage* message =
            new commands::ActiveMQMapMessage();
        message->setConnection(this->connection);
        return message;
    }
    AMQ_CATCH_ALL_THROW_CMSEXCEPTION()
...
bool ActiveMQSessionKernel::isTransacted() const
{
    return (this->ackMode == Session::SESSION_TRANSACTED) ||
           this->transaction->isInXATransaction();
}
    ...
    cms::AsyncCallback*                    onComplete)
{
    try
    {
        ...
        if (destination->isTemporary())
        {
            ...
            if (this->connection->isDeleted(tempDest))
            {
                throw cms::InvalidDestinationException(
                    std::string("Cannot publish to a deleted Destination: ") +
        ...
        {
            // Ensure that a new transaction is started if this is the first
            // message sent since the last commit, Broker is notified of a new
            // TX.
            doStartTransaction();
            ...
            // NOTE:
            // Now we copy the message before sending, this allows the user to
            // reuse the message object without interfering with the copy that's
            // being sent.  We could make this step optional to increase
            // performance but for now we won't. To not do this implies that the
            // user must never reuse the message object, or know that the
            // configuration of Transports doesn't involve the message hanging
            // around beyond the point that send returns.  When the transform
            // step results in a new Message object being created we can just
            // use that new instance, but when the original cms::Message pointer
            // was already a commands::Message then we need to clone it.
            if (ActiveMQMessageTransformation::transformMessage(message,
                                                                connection,
                                                                &transformed))
            {
...
////////////////////////////////////////////////////////////////////////////////
void ActiveMQSessionKernel::setMessageTransformer(
    cms::MessageTransformer* transformer)
{
...
void ActiveMQSessionKernel::unsubscribe(const std::string& name)
{
    try
    {
        ...
        AMQ_LOG_INFO("SessionKernel",
                     "Durable subscription unsubscribed, name=" << name);
    }
    AMQ_CATCH_ALL_THROW_CMSEXCEPTION()
}
...
////////////////////////////////////////////////////////////////////////////////
void ActiveMQSessionKernel::createTemporaryDestination(
    ...
    Pointer<ActiveMQConsumerKernel> consumer)
{
    try
    {
        bool connectionClosed = this->connection->isClosed();

        // Only remove dispatcher if connection is still open
        if (!connectionClosed)
        {
        ...
        try
        {
            ...
            // Only remove audited dispatcher if connection is still open
            // This prevents accessing destroyed ConnectionAudit.
            if (!connectionClosed)
            {
