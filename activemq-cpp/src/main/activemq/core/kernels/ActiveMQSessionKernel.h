...
namespace core
{
    ...
    namespace kernels
    {
        ...
        public:
            ...
            /**
             * Redispatches the given set of unconsumed messages to the
             * consumers.
             * @param unconsumedMessages - unconsumed messages to be
             * redelivered.
             */
            ...
            /**
             * Fires the given exception to the exception listener of the
             * connection
             */
        ...
        public:  // Implements Methods
            ...
            virtual cms::QueueBrowser* createBrowser(
        ...
        public:  // ActiveMQSessionKernel specific Methods
            /**
             * Sends a message from the Producer specified using this session's
             * connection the message will be sent using the best available
             * means depending on the configuration of the connection. <p>
             ...
             * @param usage
             *      Pointer to a Usage tracker which if set will be increased by
             * the size of the given message.
             * @param sendTimeout
             *      The amount of time to block during send before failing, or 0
             * to wait forever.
             *
             ...
             * sessions connection and returns it.  Mainly intended for use by
             * the objects that this session creates so that they can notify the
             * client of exceptions that occur in the context of another thread.
             *
             ...
             */
            virtual void setMessageTransformer(
             ...
             *
             * @return long long containing the sequence id of the last
             * delivered Message.
             */
            ...
            /**
             * Sends a Command to the broker without requesting any Response be
             * returned.
             * .
             * @param command
             *      The message to send to the Broker.
             ...
             * broker. Converts any error responses into an exception.
             *
             * @param command
             ...
             * the Connection and store a reference to it so the session can
             * ensure that all resources are closed when the session is closed.
             *
             * @param consumer
             ...
             * the Connection and clean up any resources associated with it.
             *
             * @param consumer
             *      The ActiveMQConsumerKernel instance to remove from this
             * session.
             *
             * @throw ActiveMQException if an internal error occurs.
             */
             ...
             * Connection and store a reference to it so the session can ensure
             * that all resources are closed when the session is closed.
             *
             * @param producer
             ...
             * the session is not a Transacted Session then an exception is
             * thrown.  If a transaction is already in progress then this method
             * has no effect.
             *
            ...
            /**
             * Request that the Session inform all its consumers to Acknowledge
             * all Message's that have been received so far.
             */
            void acknowledge();

            /**
             * Request that this Session inform all of its consumers to deliver
             * their pending acks.
             */
            void deliverAcks();

            /**
             * Request that this Session inform all of its consumers to clear
             ...
             * meant for use by ActiveMQConnection, the connection object calls
             * this when it has been closed to skip some of the extraneous
             * processing done by the client level close method.
             */
            void doClose();

            /**
             * Cleans up the Session object's resources without attempting to
             * send the Remove command to the broker, this can be called from
             * ActiveMQConnection when it knows that the transport is down and
             * the doClose method would throw an exception when it attempt to
             * send the Remove Command.
             */
            void dispose();

            /**
             * Set the prefetch level for the given consumer if it exists in
             * this Session to the value specified.
             *
             * @param id
             ...
             * enqueued by calling each consumers iterate method.  Returns true
             * if this method needs to be called again because a consumer
             * requires further processing time to complete its dispatching.
             * Once all consumers are done this method returns false.
             *
            ...
            /**
             * Checks if any MessageConsumer owned by this Session has a set
             * MessageListener and throws an exception if so.  This enforces the
             * rule that the MessageConsumers belonging to a Session either
             * operate in sync or async receive as a group.
             */
            ...
            /**
             * Sends the given MessageAck command to the Broker either via
             * Synchronous call or an Asynchronous call depending on the value
             * of the async parameter.
             *
             * @param ack
            ...
            /**
             * Configures asynchronous message dispatch to this session's
             * consumers.
             *
            ...
            /**
             * Returns an ArrayList containing a copy of all consumers currently
             * in use on this Session.  Since this list is copied from the main
             * consumers list the usage is thread safe after return.
             *
