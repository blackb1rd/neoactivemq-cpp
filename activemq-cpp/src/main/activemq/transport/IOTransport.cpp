    ...
    void IOTransport::logMessageDispatchDetails(const Pointer<Command>& command)
    {
        try
        {
            ...
            AMQ_LOG_INFO("MessageReceived",
                         "Message ID: "
                             << (msgId != NULL ? msgId->toString() : "")
                             << " Correlation ID: " << msg->getCorrelationId()
                             << " Timestamp: " << msg->getTimestamp()
                             << " Type: " << msg->getType() << " Destination: "
...
void IOTransport::oneway(const Pointer<Command> command)
{
    try
    {
        ...
        synchronized(impl->outputStream)
        {
            ...
            this->impl->wireFormat->marshal(command,
                                            this,
...
void IOTransport::close()
{
    ...
    try
    {
        ...
        if (impl->closed.compareAndSet(false, true))
        {
            ...
            // (which is likely). Otherwise, the join that follows will block
            // forever.
            try
            {
    ...
    try
    {
        ...
        while (this->impl->started.get() && !this->impl->closed.get())
        {
            try
            {
                ...
                // Unmarshaling errors (corrupted message headers) close the
            ...
            catch (exceptions::ActiveMQException& ex)
            {
                ...
                AMQ_LOG_ERROR(
                    "IOTransport",
                    "run() caught ActiveMQException: " << ex.getMessage());
                ex.setMark(__FILE__, __LINE__);
                fire(ex);
