...
namespace util
{
    ...
    std::chrono::system_clock::time_point
        AMQLogger::flightRecorderWallClockStart;
    ...
    namespace
    {
        ...
        std::string formatLogMessage(AMQLogLevel        level,
    ...
    void AMQLogger::log(AMQLogLevel        level,
                        const char*        component,
                        const std::string& message)
    {
        ...
        // Check for context-specific handler first - handlers are ALWAYS called
        // if registered
        ...
        if (!currentLogContext.empty())
        {
            ...
            {
                // Handler registered - format and call it regardless of
                // recordOnlyMode
    ...
    AMQLogLevel AMQLogger::parseLevel(const std::string& levelStr)
    {
        ...
        if (lower == "debug")
        {
            return AMQLogLevel::DBG;
        }
        if (lower == "info")
        {
            return AMQLogLevel::INFO;
        }
        if (lower == "warn")
        {
            return AMQLogLevel::WARN;
        }
        if (lower == "error")
        {
            return AMQLogLevel::ERR;
        }
    ...
    void AMQLogger::logProtocolFeatures(
        const char* component,
        int         version,
        bool        cacheEnabled,
        int         cacheSize,
        ...
        long long   maxInactivityDurationInitialDelay)
    {
        ...
        oss << "OpenWire Protocol Features: "
            ...
            << maxInactivityDurationInitialDelay << "ms";
                                             ...
                                             std::size_t maxEntries)
    {
        ...
        std::size_t allocBytes =
            static_cast<std::size_t>(systemMemory * memoryPercent);
        ...
        auto elapsedUs = std::chrono::duration_cast<std::chrono::microseconds>(
                             endChrono - startChrono)
                             .count();
        if (elapsedUs > 0)
        {
            flightRecorderTicksPerMicrosecond =
                static_cast<double>(endTicks - startTicks) / elapsedUs;
        }
        else
        {
    ...
    void AMQLogger::recordToFlightRecorder(AMQLogLevel level,
                                           const char* component,
                                           const char* message)
    {
        ...
        uint64_t idx =
            flightRecorderWriteIndex.fetch_add(1, std::memory_order_relaxed);
        ...
        entry.timestampTicks =
            rdtsc();  // RDTSC is ~20 cycles vs ~100+ for chrono
    ...
    void AMQLogger::dumpFlightRecorder(std::ostream& out,
                                       std::size_t   maxEntries)
    {
        dumpFlightRecorder(
            [&out](const FlightRecorderEntry& entry)
            {
                // Convert RDTSC ticks to microseconds
                int64_t ticksElapsed = static_cast<int64_t>(
                    entry.timestampTicks - flightRecorderStartTicks);
                int64_t elapsedUs = static_cast<int64_t>(
                    ticksElapsed / flightRecorderTicksPerMicrosecond);

                // Calculate wall-clock time for this entry
                auto wallClockTime = flightRecorderWallClockStart +
                                     std::chrono::microseconds(elapsedUs);
                auto time_t_val =
                    std::chrono::system_clock::to_time_t(wallClockTime);
                auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                              wallClockTime.time_since_epoch()) %
                          1000;
    ...
    void AMQLogger::dumpFlightRecorder(
        ...
        std::size_t                                     maxEntries)
    {
        ...
        uint64_t total =
            flightRecorderTotalCount.load(std::memory_order_acquire);
        uint64_t currentIdx =
            flightRecorderWriteIndex.load(std::memory_order_acquire);

        // Calculate how many valid entries we have
        std::size_t validEntries = static_cast<std::size_t>(
            (std::min)(total, static_cast<uint64_t>(cap)));
    ...
    std::size_t AMQLogger::flightRecorderSize()
    {
        ...
        uint64_t total =
            flightRecorderTotalCount.load(std::memory_order_relaxed);
        std::size_t cap = flightRecorderBuffer.size();
    ...
    ////////////////////////////////////////////////////////////////////////////////
    void AMQLogger::setContextOutputHandler(
