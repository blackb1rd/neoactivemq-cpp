...
namespace util
{
     ...
     * Ordered by verbosity: NONE < ERR < WARN < INFO < DBG
     *
     * NOTE: ERR/DBG are used instead of ERROR/DEBUG to avoid
     * conflicts with Windows macros of the same name.
     */
     ...
     *
     * Design principles:
     * - Zero overhead when level is NONE (level check before string
     * construction)
     * - Thread-safe via atomic log level
     * - Uses std::ostringstream instead of fprintf
     * - Simple API - no complex configuration
     * - Flight Recorder: circular buffer stores recent log entries for
     * debugging
     *
     * Usage:
     *   AMQLogger::setLevel(AMQLogLevel::DBG);
    ...
    public:
        ...
        {
            uint64_t timestampTicks;  // RDTSC ticks (convert using
                                      // ticksPerMicrosecond)
    ...
    private:
        // Default/global log level - used when no context is set or context has
        // no specific level
        static std::atomic<AMQLogLevel> defaultLevel;

        // Record-only mode: skip expensive formatting in log(), only record to
        // flight recorder This is lock-free (relaxed atomic) for maximum
        // performance
        ...
        static std::unordered_map<
            std::string,
            std::function<void(AMQLogLevel, const std::string&)>>
                                                            contextHandlers;
        static std::unordered_map<std::string, AMQLogLevel> contextLevels;

        // Helper to get thread-local context (avoids DLL export issues with
        // thread_local static)
        ...
        static uint64_t flightRecorderStartTicks;         // RDTSC value at
                                                          // initialization
        static double flightRecorderTicksPerMicrosecond;  // For converting
                                                          // ticks to time
        static std::chrono::system_clock::time_point
            flightRecorderWallClockStart;

    public:
        /**
         * Set the default/global log level.
         * This level is used when no context is set or when the context has no
         * specific level.
         ...
         * @param context The context identifier (e.g., connection name, broker
         * URL)
         ...
         * When enabled, log() only records to flight recorder - no formatting
         * overhead. This is ideal for tests where you only need logs on
         * failure.
         ...
         *   AMQLogger::setContextOutputHandler("broker1", [](AMQLogLevel level,
         * const std::string& msg) { std::ofstream logFile("broker1.log",
         * std::ios::app); logFile << msg << std::endl;
         *   });
         *
         ...
         *
         * @param context The context identifier (e.g., broker URL, connection
         * ID)
         * @param handler Function that receives level and formatted message for
         * this context
         */
        static void setContextOutputHandler(
         ...
         * Logs from this thread will use the context-specific handler if
         * configured.
         * @param context The context identifier (e.g., broker URL, connection
         * ID)
         */
        ...
        /**
         * Internal: Write a log message. Use macros instead of calling
         * directly.
         */
        static void log(AMQLogLevel        level,
        ...
        static const char* levelToString(AMQLogLevel level)
        {
            switch (level)
            {
                case AMQLogLevel::ERR:
                    return "ERROR";
                case AMQLogLevel::WARN:
                    return "WARN";
                case AMQLogLevel::INFO:
                    return "INFO";
                case AMQLogLevel::DBG:
                    return "DEBUG";
                default:
                    return "NONE";
            }
         ...
         * @param levelStr Case-insensitive level name (none, error, info,
         * debug)
         ...
         * @param maxInactivityDurationInitialDelay Initial delay for inactivity
         * check in ms
         */
        static void logProtocolFeatures(
            const char* component,
            int         version,
            bool        cacheEnabled,
            int         cacheSize,
         ...
         *
         * @param memoryPercent Percentage of system memory to use (e.g., 0.005
         * for 0.5%)
         ...
         */
        static void dumpFlightRecorder(std::ostream& out,
         ...
         * @param maxEntries Maximum entries to dump (0 = all)
         */
        static void dumpFlightRecorder(
            std::function<void(const FlightRecorderEntry&)> callback,
            std::size_t                                     maxEntries = 0);
    ...
    private:
         ...
         */
        static void recordToFlightRecorder(AMQLogLevel level,
                                           const char* component,
                                           const char* message);
    };
...
#define AMQ_LOG_DEBUG(component, message)                                    \
    do                                                                       \
    {                                                                        \
        if (activemq::util::AMQLogger::isEnabled(                            \
                activemq::util::AMQLogLevel::DBG))                           \
        {                                                                    \
            std::ostringstream _amq_oss;                                     \
            _amq_oss << message;                                             \
            activemq::util::AMQLogger::log(activemq::util::AMQLogLevel::DBG, \
                                           component,                        \
                                           _amq_oss.str());                  \
        }                                                                    \
    } while (0)
...
#define AMQ_LOG_WARN(component, message)                                      \
    ...
    {                                                                         \
        if (activemq::util::AMQLogger::isEnabled(                             \
                activemq::util::AMQLogLevel::WARN))                           \
        {                                                                     \
            std::ostringstream _amq_oss;                                      \
            _amq_oss << message;                                              \
...
#define AMQ_LOG_INFO(component, message)                                      \
    ...
    {                                                                         \
        if (activemq::util::AMQLogger::isEnabled(                             \
                activemq::util::AMQLogLevel::INFO))                           \
        {                                                                     \
            std::ostringstream _amq_oss;                                      \
            _amq_oss << message;                                              \
...
#define AMQ_LOG_ERROR(component, message)                                    \
    ...
    {                                                                        \
        if (activemq::util::AMQLogger::isEnabled(                            \
                activemq::util::AMQLogLevel::ERR))                           \
        {                                                                    \
            std::ostringstream _amq_oss;                                     \
            _amq_oss << message;                                             \
            activemq::util::AMQLogger::log(activemq::util::AMQLogLevel::ERR, \
                                           component,                        \
                                           _amq_oss.str());                  \
        }                                                                    \
    } while (0)
...
#define AMQ_LOG_DEBUG_ENABLED() \
    activemq::util::AMQLogger::isEnabled(activemq::util::AMQLogLevel::DBG)
...
#define AMQ_LOG_ERROR_ENABLED() \
    activemq::util::AMQLogger::isEnabled(activemq::util::AMQLogLevel::ERR)
