...
////////////////////////////////////////////////////////////////////////////////
bool ActiveMQMessageTransformation::transformDestination(
    ...
    if (*amqDestination == NULL)
    {
        if (dynamic_cast<const cms::TemporaryQueue*>(destination) != NULL)
        {
            *amqDestination = new ActiveMQTempQueue(
                dynamic_cast<const cms::TemporaryQueue*>(destination)
                    ->getQueueName());
        }
        else if (dynamic_cast<const cms::TemporaryTopic*>(destination) != NULL)
        {
            *amqDestination = new ActiveMQTempTopic(
                dynamic_cast<const cms::TemporaryTopic*>(destination)
                    ->getTopicName());
        }
    ...
    else
    {
        if (dynamic_cast<cms::BytesMessage*>(message) != NULL)
        {
            cms::BytesMessage* bytesMsg =
                dynamic_cast<cms::BytesMessage*>(message);
            bytesMsg->reset();
            ...
            try
            {
                for (;;)
                {
                    // Reads a byte from the message stream until the stream is
                    // empty
        ...
        {
            cms::ObjectMessage* objMsg =
                dynamic_cast<cms::ObjectMessage*>(message);
        ...
        {
            cms::StreamMessage* streamMessage =
                dynamic_cast<cms::StreamMessage*>(message);
            streamMessage->reset();
            ...
            try
            {
                while (true)
                {
                    cms::Message::ValueType elementType =
                        streamMessage->getNextValueType();
                    int                        result = -1;
                    ...
                    switch (elementType)
                    {
                        ...
                        case cms::Message::BYTE_ARRAY_TYPE:
                            while ((result =
                                        streamMessage->readBytes(buffer)) != -1)
                            {
        ...
        else if (dynamic_cast<cms::TextMessage*>(message) != NULL)
        {
            cms::TextMessage* textMsg =
                dynamic_cast<cms::TextMessage*>(message);
        ...
        ActiveMQMessageTransformation::copyProperties(
            message,
            dynamic_cast<cms::Message*>(*amqMessage));
    }
    ...
    cms::Message*       toMessage)
{
    ...
    if (fromMessage->getCMSDestination() != NULL)
    {
        ...
        if (transformDestination(fromMessage->getCMSDestination(),
                                 &transformed))
        {
            toMessage->setCMSDestination(
                dynamic_cast<const cms::Destination*>(transformed));
            delete transformed;
        }
        else
        {
    ...
    for (; iter != propertyNames.end(); ++iter)
    {
        ...
        cms::Message::ValueType propertyType =
            fromMessage->getPropertyValueType(name);

        switch (propertyType)
        {
            case cms::Message::BOOLEAN_TYPE:
                toMessage->setBooleanProperty(
                    name,
                    fromMessage->getBooleanProperty(name));
                break;
            case cms::Message::BYTE_TYPE:
                toMessage->setByteProperty(name,
                                           fromMessage->getByteProperty(name));
                break;
            case cms::Message::SHORT_TYPE:
                toMessage->setShortProperty(
                    name,
                    fromMessage->getShortProperty(name));
                break;
            case cms::Message::INTEGER_TYPE:
                toMessage->setIntProperty(name,
                                          fromMessage->getIntProperty(name));
                break;
            case cms::Message::LONG_TYPE:
                toMessage->setLongProperty(name,
                                           fromMessage->getLongProperty(name));
                break;
            case cms::Message::FLOAT_TYPE:
                toMessage->setFloatProperty(
                    name,
                    fromMessage->getFloatProperty(name));
                break;
            case cms::Message::DOUBLE_TYPE:
                toMessage->setDoubleProperty(
                    name,
                    fromMessage->getDoubleProperty(name));
                break;
            case cms::Message::STRING_TYPE:
                toMessage->setStringProperty(
                    name,
                    fromMessage->getStringProperty(name));
                break;
            default:
                break;
        }
