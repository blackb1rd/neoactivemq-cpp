...
void StompFrame::toStream(decaf::io::DataOutputStream* stream) const
{
    ...
    stream->write((unsigned char*)cmdString.c_str(),
                  (int)cmdString.length(),
                  0,
                  (int)cmdString.length());
    stream->write('\n');
    ...
    for (std::size_t ix = 0; ix < headers.size(); ++ix)
    {
        ...
        stream->write((unsigned char*)name.c_str(),
                      (int)name.length(),
                      0,
                      (int)name.length());
        stream->write(':');
        stream->write((unsigned char*)value.c_str(),
                      (int)value.length(),
                      0,
                      (int)value.length());
        stream->write('\n');
    ...
    try
    {
        ...
        while (true)
        {
            ...
            if (offset >= 0)
            {
                ...
                this->setCommand(
                    reinterpret_cast<char*>(&buffer[(size_t)offset]));
                break;
            }
        }
    }
            ...
            else
            {
                ...
                for (size_t ix = 0; ix < buffer.size(); ++ix)
                {
                    ...
                    if (buffer[ix] == ':')
                    {
                        ...
                        const char* value =
                            reinterpret_cast<char*>(&buffer[ix + 1]);
