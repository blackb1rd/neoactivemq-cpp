...
/**
 * A BytesMessage object is used to send a message containing a stream of
 * unsigned bytes. It inherits from the Message interface and adds a bytes
 * message body. The receiver of the message supplies the interpretation of the
 * bytes using the methods added by the BytesMessage interface.
 *
 * The BytesMessage methods are based largely on those found in
 * decaf.io.DataInputStream and decaf.io.DataOutputStream.
 *
 * Although the CMS API allows the use of message properties with byte messages,
 ...
 * Because the C++ language is more limited when dealing with primitive types
 * the JMS equivalent generic read and write methods that take Java objects
 * cannot be provided in the CMS API.
 *
...
public:
    ...
    /**
     * Gets the bytes that are contained in this message and returns them in a
     * newly allocated array that becomes the property of the caller.  This is a
     * copy of the data contained in this message, changing the value contained
     * in this array has no effect on the data contained in this message.
     *
     * @return pointer to a byte buffer that the call owns upon completion of
     * this method.
     *
     * @throws CMSException - If an internal error occurs.
     ...
     *
     * @throws CMSException - If the provider fails to perform the reset
     * operation.
     ...
     *
     * @throws CMSException - if the CMS provider fails to read the message due
     ...
     *
     * @throws CMSException - if an internal error occurs while cloning the
     * Message.
     */
