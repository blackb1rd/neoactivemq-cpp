 ...
 * communication, a connection is a relatively heavy-weight object. Most clients
 * will do all their messaging with a single connection. Other more advanced
 * applications may use several connections. The CMS API does not architect a
 * reason for using multiple connections; however, there may be operational
 * reasons for doing so. <p> A CMS client typically creates a connection, one or
 * more sessions, and a number of message producers and consumers. When a
 * connection is created, it is in stopped mode. That means that no messages are
 * being delivered. <p> It is typical to leave the connection in stopped mode
 * until setup is complete (that is, until all message consumers have been
 * created). At that point, the client calls the connection's start method, and
 ...
 * delivery while the client is still in the process of setting itself up. <p>
 ...
 * message delivery while they are still in the process of setting up. <p> A
...
public:
     ...
     *
     * @throws CMSException
     *         if the provider fails to get the connection metadata for this
     * connection.
     *
    ...
    /**
     * Get the Client Id for this session, the client Id is provider specific
     * and is either assigned by the connection factory or set using the
     * setClientID method.
     *
     * @return Client Id String for this Connection.
     *
     * @throws CMSException if the provider fails to return the client id or an
     ...
     *
     * The preferred way to assign a CMS client's client identifier is for it to
     * be configured in a client-specific ConnectionFactory object and
     ...
     * identifier is a programming error that should throw an
     * IllegalStateException.
     *
     * @param clientID
     *      The unique client identifier to assign to the Connection.
     *
     * @throws CMSException if the provider fails to set the client id due to
     * some internal error.
     * @throws InvalidClientIDException if the id given is somehow invalid or is
     * a duplicate.
     * @throws IllegalStateException if the client tries to set the id after a
     * Connection method has been called.
     */
    ...
    /**
     * Set an MessageTransformer instance that is passed on to all Session
     ...
     *
     * @param transformer
     *      Pointer to the cms::MessageTransformer to set on all newly created
     * Session objects.
     */
