...
/**
 * Defines the interface for a factory that creates connection objects, the
 * Connection objects returned implement the CMS Connection interface and hide
 * the CMS Provider specific implementation details behind that interface.  A
 * Client creates a new ConnectionFactory either directly by instantiating the
 * provider specific implementation of the factory or by using the static method
 * <code>createCMSConnectionFactory</code> which all providers are required to
 * implement.
 *
 * @since 1.0
 */
...
public:
     ...
     *         is responsible for closing the connection and deleting the
     * instance.
     *
    ...
    /**
     * Set an ExceptionListener instance that is passed on to all Connection
     * objects created from this ConnectionFactory
     *
     * @param transformer
     *      Pointer to the cms::ExceptionListener to set on all newly created
     * Connection objects/
     */
    virtual void setExceptionListener(cms::ExceptionListener* listener) = 0;

    /**
     * Gets the currently configured ExceptionListener for this
     * ConnectionFactory.
     *
     * The CMS code never takes ownership of the ExceptionListener pointer which
     * implies that the client code must ensure that the object remains valid
     * for the lifetime of the CMS object to which the ExceptionListener has
     * been assigned.
     *
    ...
    /**
     * Set an MessageTransformer instance that is passed on to all Connection
     * objects created from this ConnectionFactory
     *
...
public:
     ...
     * ConnectionFactory interface, the caller is responsible for deleting this
     * resource.
     *
     * @throws CMSException if an internal error occurs while creating the
     * ConnectionFactory.
