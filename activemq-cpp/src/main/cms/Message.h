 ...
 * <B>Message Bodies</B>
 *
 * The CMS API defines four types of message bodies, each type is contained
 * within its own Message Interface definition.
 *
 *   - Stream - A StreamMessage object's message body contains a stream of
 * primitive values in the C++ language. It is filled and read sequentially.
 * Unlike the BytesMessage type the values written to a StreamMessage retain
 * information on their type and rules for type conversion are enforced when
 * reading back the values from the Message Body.
 *   - Map - A MapMessage object's message body contains a set of name-value
 * pairs, where names are std::string objects, and values are C++ primitives.
 * The entries can be accessed sequentially or randomly by name.  The MapMessage
 * makes no garuntee on the order of the elements within the Message body.
 *   - Text - A TextMessage object's message body contains a std::string object.
 * This message type can be used to transport plain-text messages, and XML
 * messages.
 *   - Bytes - A BytesMessage object's message body contains a stream of
 * uninterpreted bytes. This message type is for literally encoding a body to
 * match an existing message format. In many cases, it is possible to use one of
 * the other body types, which are easier to use.
 *
 ...
 * </PRE>
 *
 * When a Message is delivered its properties are considered to be in a
 * read-only mode and cannot be changed.  Attempting to change the value of a
 * delivered Message's properties will result in a CMSException being thrown.
 *
 * @see JMS API
 * @since 1.0
 */
...
public:
    ...
    /**
     * The Default Time to Live for a Message Producer is unlimited, the message
     * will never expire.
     */
    static const long long DEFAULT_TIME_TO_LIVE;

    /**
     * Defines the Type Identifiers used to identify the type contained within a
     * specific Message property or MapMessage keyed value.  CMS Providers can
     * store other types in their internal representations, which should map to
     * the UNKNOWN_TYPE from the CMS API.
     */
    enum ValueType
    {
        NULL_TYPE       = 0,
...
public:
     ...
     *
     * @throws CMSException if no property exists that matches the requested
     * key.
     */
