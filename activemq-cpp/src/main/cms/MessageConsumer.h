 ...
 * an asynchronous message delivery is in progress or until a
 * <code>receive</code> operation completes.  A blocked consumer in a
 * <code>receive</code> call will return a Null when the close method is called.
 *
 * While the MessageConsumer implements the Startable and Stoppable interfaces
...
public:
     ...
     * objects before they are dispatched to client code.
     *
     ...
     *
     * @param transformer
     *      Pointer to the cms::MessageTransformer to apply on each cms:;Message
     * dispatch.
     */
    ...
    /**
     * Sets the MessageAvailableListener that this class will send events to if
     * the consumer is in synchronous consumption mode and a new Message has
     * arrived.
     *
     * @param listener
    ...
    virtual void setMessageAvailableListener(
        cms::MessageAvailableListener* listener) = 0;
