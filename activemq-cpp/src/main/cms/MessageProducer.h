...
public:
     ...
     * ownership of the message, caller must still destroy it.  Uses default
     * values for deliveryMode, priority, and time to live.
     *
     * @param message
     *      The message to be sent.
     *
     * @throws CMSException - if an internal error occurs while sending the
     * message.
     ...
     * @throws UnsupportedOperationException - if a client uses this method with
     * a MessageProducer that did not specify a destination at creation time.
     */
     ...
     * Sends the message to the default producer destination, but does not take
     * ownership of the message, caller must still destroy it.  Uses default
     * values for deliveryMode, priority, and time to live.  If the
     * AsyncCallback parameter is set this method will return immediately and
     * the call-back will be notified once the CMS Provider as acknowledge
     * receipt of the Message or an Error occurs.
     *
     * @param message
     *      The message to be sent.
     * @param onComplete
     *      The AsyncCallback instance to notify on send complete or error,
     * caller retains ownership of this pointer and must destroy it only after
     * the send completes or the connection is closed.
     *
     * @throws CMSException - if an internal error occurs while sending the
     * message.
     * @throws MessageFormatException - if an Invalid Message is given.
    ...
    /**
     * Sends the message to the default producer destination, but does not take
     * ownership of the message, caller must still destroy it.  If the
     * AsyncCallback parameter is set this method will return immediately and
     * the call-back will be notified once the CMS Provider as acknowledge
     * receipt of the Message or an Error occurs.
     *
    ...
    /**
     * Sends the message to the designated destination, but does not take
     * ownership of the message, caller must still destroy it.  Uses default
     * values for deliveryMode, priority, and time to live.  If the
     * AsyncCallback parameter is set this method will return immediately and
     * the call-back will be notified once the CMS Provider as acknowledge
    ...
    /**
     * Set an MessageTransformer instance that is applied to all cms::Message
     * objects before they are sent on to the CMS bus.
     *
     ...
     *
     * @param transformer
     *      Pointer to the cms::MessageTransformer to apply on each
     * cms:;MessageSend.
     */
