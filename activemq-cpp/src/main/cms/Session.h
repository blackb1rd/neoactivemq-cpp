 ...
 *  - The close call will block until a receive call or message listener in
 * progress has completed. A blocked message consumer receive call returns null
 ...
 *    IllegalStateException. Closing a closed session must not throw  any
 * exceptions.
 *
 * <B>Transacted Sessions</B>
 *
 * When a Session is created it can be set to operate in a Transaction based
 * mode.  Each Session then operates in a single transaction for all Producers
 * and Consumers of that Session.  Messages sent and received within a
 * Transaction are grouped into an atomic unit that is committed or rolled back
 * together.
 *
 * For a MessageProducer this implies that all messages sent by the producer are
 * not sent to the Provider unit the commit call is made.  Rolling back the
 * Transaction results in all produced Messages being dropped.
 *
 * For a MessageConsumer this implies that all received messages are not
 * Acknowledged until the Commit call is made.  Rolling back the Transaction
 * results in all Consumed Message being redelivered to the client, the Provider
 * may allow configuration that limits the Maximum number of redeliveries for a
 * Message.
 *
 * While the Session interface implements the Startable and Stoppable interfaces
 * it is not required to implement these methods and can throw an
 * UnsupportedOperation exception if they are not available for the given CMS
 * provider.
 *
 * @since 1.0
 */
...
public:
    enum AcknowledgeMode
    {
         ...
         * acknowledge the given message and all messages received before it,
         * this mode only acknowledges one message.
         */
...
public:
     ...
     *
     * @throws CMSException - If an internal error occurs.
     * @throws IllegalStateException - if the method is not called by a
     * transacted session.
     */
     ...
     *
     * @throws CMSException - if the CMS provider fails to stop and restart
     ...
     * selector.  Sessions that create durable consumers must use the same
     * client Id as was used the last time the subscription was created in order
     * to receive all messages that were delivered while the client was offline.
     *
     * @param destination
    ...
    virtual MessageProducer* createProducer(
        const Destination* destination = NULL) = 0;
     ...
     *
     * @throws CMSException - If an internal error occurs.
     * @throws InvalidDestinationException - if the destination given is
     * invalid.
     */
     ...
     * @throws CMSException - If an internal error occurs.
     */
    virtual BytesMessage* createBytesMessage(const unsigned char* bytes,
                                             int bytesSize) = 0;
     ...
     * MessageProducer and MessageConsumer objects created from this Session.
     *
     * The CMS code never takes ownership of the MessageTransformer pointer
     * which implies that the client code must ensure that the object remains
     * valid for the lifetime of the CMS object to which the MessageTransformer
     * has been assigned.
     *
     * @param transformer
