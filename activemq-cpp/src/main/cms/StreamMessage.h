 ...
 * type whose body is a stream of self describing primitive types.  The
 * primitive values are read and written using accessors specific to the given
 * types.
 *
 * StreamMessage objects support the following conversion table. The marked
 * cases must be supported. The unmarked cases must throw a CMSException. The
...
public:
    ...
    /**
     * Returns the value type for the element in the StreamMessage.  The CMS
     * provider should translate all internal type identifiers into the CMS
     * Value types returning UNKNOWN_TYPE for any specialized types not directly
     * supported in the CMS API. The call can fail if the StreamMessage is
     ...
     * key.
     * @throw MessageEOFException - if unexpected end of message stream has been
     * reached.
     * @throw MessageFormatException - if the message contains invalid data.
     * @throw MessageNotReadableException - if the message is in write-only
     * mode.
     */
