 ...
 * exposes its XA support using an XAConnectionFactory object, which an
 * application uses to create XAConnection objects.
 *
 * The XAConnectionFactory interface is optional. CMS providers are not required
 * to support this interface. This interface is for use by CMS providers to
...
public:
    ...
    /**
     * Creates an XAConnection with the default user name and password. The
     * connection is created in stopped mode just as the standard Connection
     * object is created from the ConnectionFactory. No messages will be
     * delivered until the Connection.start method is explicitly called.
     *
     * @return a new XAConnectionFactory instance, the caller owns the returned
     * pointer.
     *
     * @throws CMSException if an internal error occurs while creating the
     * Connection.
     * @throws CMSSecurityException if the client authentication fails because
     * the user name or password are invalid.
     */
    virtual XAConnection* createXAConnection() = 0;

    /**
     * Creates an XA connection with the specified user name and password. The
     * connection is created in stopped mode just as the standard
     * ConnectionFactory creates a new Connection. No messages will be delivered
     * until the Connection.start method is explicitly called.
     *
     * @return a new XAConnectionFactory instance, the caller owns the returned
     * pointer.
     *
...
public:
     ...
     * being used.
     *
     * The XA interfaces are optional in CMS however if a provider chooses to
     * omit them it should still override this method and throw an
     * UnsupportedOperationException to indicate that it doesn't provide this
     * functionality.
     *
     * @param brokerURI
     ...
     * XAConnectionFactory interface, the caller is responsible for deleting
     * this resource.
     *
     * @throws CMSException if an internal error occurs while creating the
     * XAConnectionFactory.
     ...
     * the XA API.
     */
    static XAConnectionFactory* createCMSXAConnectionFactory(
