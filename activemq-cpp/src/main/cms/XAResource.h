...
/**
 * The XAResource interface is a CMS mapping of the industry standard XA
 ...
 * environment. A CMS provider implements this interface to support the
 * association between a global transaction and a message broker connection.
 *
 * The XAResource is exposed to CMS client so that they can proxy calls from the
 * Transaction Manager API of their choosing to the CMS provider.  The CMS
 * provider should behave and a standard XA Resource Manager its up to the
 * client however to transmit the Transaction Manager's calls to the CMS
