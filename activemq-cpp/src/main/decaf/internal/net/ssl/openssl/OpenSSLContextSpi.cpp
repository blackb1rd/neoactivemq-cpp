        ...
        namespace ssl
        {
            namespace openssl
            {
                ...
                public:
                    ...
                    static int passwordCallback(char* buffer,
                                                int   size,
                                                int,
                                                void* data)
                    {
                        ...
                        {
                            size = size < (int)ctxData->password.size()
                                       ? size
                                       : (int)ctxData->password.size();
                ...
                std::string ContextData::defaultCipherList =
...
void OpenSSLContextSpi::providerInit(SecureRandom* random)
{
    ...
    try
    {
        ...
        {
            X509_STORE* store =
                SSL_CTX_get_cert_store(this->data->openSSLContext);
            ...
            for (const char* storeName : winStores)
            {
                ...
                if (hStore)
                {
                           ...
                           NULL)
                    {
                        ...
                        X509*                x509 =
                            d2i_X509(NULL, &encoded, pContext->cbCertEncoded);
                        if (x509)
                        {
        ...
        //
        // TLS 1.3 introduces post-handshake messages (NewSessionTicket,
        // KeyUpdate) that cause SSL_read() to internally trigger SSL_write()
        // calls for responses. This codebase uses separate readMutex/writeMutex
        // to allow concurrent SSL_read + SSL_write (full-duplex), which is safe
        // in TLS 1.2.  However, with TLS 1.3 an SSL_read() call on the
        // IOTransport reader thread may internally write while the main thread
        // is simultaneously calling SSL_write(), corrupting the TLS write
        // sequence numbers and producing a bad_record_mac alert at the broker.
        // Disabling TLS 1.3 avoids this race until the transport layer is
        // refactored to use asio::ssl::stream (which handles TLS 1.3
        // post-handshake I/O correctly).
        SSL_CTX_set_options(this->data->openSSLContext,
                            SSL_OP_ALL | SSL_OP_NO_SSLv2 | SSL_OP_NO_TLSv1_3);
        ...
        SSL_CTX_set_default_passwd_cb(this->data->openSSLContext,
                                      &ContextData::passwordCallback);
        SSL_CTX_set_default_passwd_cb_userdata(this->data->openSSLContext,
                                               (void*)this->data);

        // Sets the Default set of allowed Ciphers, this could be overridden by
        // the system property: decaf.net.ssl.CipherSuites
        if (SSL_CTX_set_cipher_list(this->data->openSSLContext,
                                    ContextData::defaultCipherList.c_str()) !=
            1)
        {
            throw OpenSSLSocketException(__FILE__, __LINE__);
        }

        // Here we load the configured KeyStore, this is where the client and
        // server certificate are stored, a client doesn't necessary need this
        // if the server doesn't enforce client authentication.
        std::string keyStorePath =
            System::getProperty("decaf.net.ssl.keyStore");
        this->data->password =
            System::getProperty("decaf.net.ssl.keyStorePassword");

        // We assume the Public and Private keys are in the same file.
        if (!keyStorePath.empty())
        {
            if (SSL_CTX_use_certificate_chain_file(this->data->openSSLContext,
                                                   keyStorePath.c_str()) != 1)
            {
                throw OpenSSLSocketException(__FILE__, __LINE__);
            }
            if (SSL_CTX_use_PrivateKey_file(this->data->openSSLContext,
                                            keyStorePath.c_str(),
                                            SSL_FILETYPE_PEM) != 1)
            {
                throw OpenSSLSocketException(__FILE__, __LINE__);
...
SocketFactory* OpenSSLContextSpi::providerGetSocketFactory()
{
    try
    {
        synchronized(&(this->data->monitor))
        {
            ...
            {
                this->data->clientSocketFactory.reset(
                    new OpenSSLSocketFactory(this));
            }
        }
...
ServerSocketFactory* OpenSSLContextSpi::providerGetServerSocketFactory()
{
    try
    {
        synchronized(&(this->data->monitor))
        {
            ...
            {
                this->data->serverSocketFactory.reset(
                    new OpenSSLServerSocketFactory(this));
            }
        }
