...
namespace internal
{
    namespace nio
    {
         ...
         *
         *  1. Absolute and relative get and put methods that read and write
         * single bytes;
         *  2. Relative bulk get methods that transfer contiguous sequences of
         * bytes from this buffer into an array;
         *  3. Relative bulk put methods that transfer contiguous sequences of
         * bytes from a byte array or some other byte buffer into this buffer;
         *  4. Absolute and relative get and put methods that read and write
         * values of other primitive types, translating them to and from
         * sequences of bytes in a particular byte order;
         *  5. Methods for creating view buffers, which allow a byte buffer to
         ...
         * space for the buffer's content, or by wrapping an existing byte array
         * into a buffer.
         *
         * Access to binary data:
         *
         * This class defines methods for reading and writing values of all
         * other primitive types, except boolean. Primitive values are
         * translated to (or from) sequences of bytes according to the buffer's
         * current byte order.
         *
         * For access to heterogeneous binary data, that is, sequences of values
         * of different types, this class defines a family of absolute and
         * relative get and put methods for each type. For 32-bit floating-point
         ...
         *
         * Corresponding methods are defined for the types char, short, int,
         * long, and double. The index parameters of the absolute get and put
         * methods are in terms of bytes rather than of the type being read or
         * written.
         *
         * For access to homogeneous binary data, that is, sequences of values
         * of the same type, this class defines methods that can create views of
         * a given byte buffer. A view buffer is simply another buffer whose
         * content is backed by the byte buffer. Changes to the byte buffer's
         * content will be visible in the view buffer, and vice versa; the two
         * buffers' position, limit, and mark values are independent. The
         * asFloatBuffer method, for example, creates an instance of the
         * FloatBuffer class that is backed by the byte buffer upon which the
         * method is invoked. Corresponding view-creation methods are defined
         * for the types char, short, int, long, and double.
         *
         ...
         * transfer contiguous sequences of values between a buffer and an array
         * or some other buffer of the same type; and
         *
         * @since 1.0
         */
        ...
        public:
            /**
             * Creates a ByteArrayBuffer object that has its backing array
             ...
             * @param length
             *      The size of the sub-array, this is the limit we read and
             * write to.
             * @param readOnly
             *      Should this buffer be read-only, default as false.
             *
             ...
             * ByteArrayBuffer will be that of the remaining capacity of the
             * passed buffer.
             *
             * @param array
            ...
            /**
             * Create a ByteArrayBuffer that mirrors this one, meaning it shares
             * a reference to this buffers ByteArrayAdapter and when changes are
             * made to that data it is reflected in both.
             *
             * @param other
