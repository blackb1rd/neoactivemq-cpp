...
///////////////////////////////////////////////////////////////////////////////
LongArrayBuffer::LongArrayBuffer(long long* array,
                                 int        size,
                                 int        offset,
                                 int        length,
                                 bool       readOnly)
    ...
    try
    {
        if (!this->hasArray())
        {
            throw activemq::exceptions::UnsupportedOperationException(
                __FILE__,
                __LINE__,
                "LongArrayBuffer::array() - This Buffer has no backing "
                "array.");
        }
    ...
    try
    {
        if (!this->hasArray())
        {
            throw activemq::exceptions::UnsupportedOperationException(
                __FILE__,
                __LINE__,
                "LongArrayBuffer::arrayOffset() - This Buffer has no backing "
                "array.");
        }
        ...
        if (!this->hasRemaining())
        {
            throw activemq::exceptions::BufferUnderflowException(
                activemq::exceptions::buildSourceMessage(
                    __FILE__,
                    __LINE__,
                    "LongArrayBuffer::get - Not enough data to fill "
                    "request."));
        }
