...
namespace internal
{
    namespace util
    {
        ...
        /**
         * This class adapts primitive type arrays to a base byte array so that
         * the classes can inter-operate on the same base byte array without
         * copying data. All the array types are mapped down to a byte array and
         * methods are supplied for accessing the data in any of the primitive
         * type forms.
         *
        ...
        public:
            /**
             * Creates a byte array object that is allocated internally and is
             * then owned and deleted when this object is deleted.  The array is
            ...
            /**
             * Creates a byte array object that wraps the given array.  If the
             * own flag is set then it will delete this array when this object
             * is deleted.
             *
             * @param array
            ...
            /**
             * Gets the pointer to the array we are wrapping.  Changes to the
             * data in this array are reflected by all ByteArrayAdapter objects
            ...
            /**
             * Reads from the Byte array starting at the specified offset and
             * reading the specified length.  If the length is greater than the
             * size of this underlying byte array then an
            ...
            /**
             * Writes from the Byte array given, starting at the specified
             * offset and writing the specified amount of data into this objects
             * internal array..  If the length is greater than the size of this
             * underlying byte array then an BufferOverflowException is thrown.
             *
             * @param buffer
             *      The buffer to read data from this array into.
             * @param size
            ...
            /**
             * Resizes the underlying array to the new given size, preserving
             * all the Data that was previously in the array, unless the resize
             * is smaller than the current size in which case only the data that
            ...
            /**
             * Clear all data from that Array, setting the underlying bytes to
             * zero.
             */
            virtual void clear();
             ...
             * calling the non constant version allows the user to change the
             * value at index
             *
             * @param index
            ...
            /**
             * Reads eight bytes at the given index and returns it.  The index
             * is a relative to the size of the type to be read, in other words
             * when accessing the element in the array index * sizeof( type ) if
             * the actual start index of the type to be read.
             *
             * @param index
            ...
            /**
             * Reads four bytes at the given index and returns it. The index is
             * a relative to the size of the type to be read, in other words
             * when accessing the element in the array index * sizeof( type ) if
             * the actual start index of the type to be read.
             *
             * @param index
             ...
             * relative to the size of the type to be read, in other words when
             * accessing the element in the array index * sizeof( type ) if the
             * actual start index of the type to be read.
             *
             * @param index
             *      The index in the Buffer where the bytes are to be read.
             *
            ...
            /**
             * Writes the given byte into this buffer at the given index. The
             * index is a relative to the size of the type to be read, in other
             * words when accessing the element in the array index * sizeof(
             * type ) if the actual start index of the type to be read.
             *
             * @param index
            ...
            /**
             * Writes one byte containing the given value, into this buffer at
             * the given index. The index is a relative to the size of the type
             * to be read, in other words when accessing the element in the
             * array index * sizeof( type ) if the actual start index of the
             * type to be read.
             *
             * @param index
             *      The position in the Buffer to write the data.
            ...
            /**
             * Writes eight bytes containing the given value, into this buffer
             * at the given index. The index is a relative to the size of the
             * type to be read, in other words when accessing the element in the
             * array index * sizeof( type ) if the actual start index of the
             * type to be read.
             *
             * @param index
            ...
            /**
             * Writes four bytes containing the given value, into this buffer at
             * the given index. The index is a relative to the size of the type
             * to be read, in other words when accessing the element in the
             * array index * sizeof( type ) if the actual start index of the
             * type to be read.
            ...
            /**
             * Writes two bytes containing the given value, into this buffer at
             * the given index. The index is a relative to the size of the type
             * to be read, in other words when accessing the element in the
             * array index * sizeof( type ) if the actual start index of the
             * type to be read.
