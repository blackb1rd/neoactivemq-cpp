    ...
    namespace util
    {
        namespace concurrent
        {
            ...
            public:  // Mutex processing methods
                /**
                 * Creates a new Mutex instance at the location given by the
                 * mutex pointer argument.  The mutex must be destroyed by
                 * calling the destoryMutex method when it is no longer needed.
                 *
                 * @param mutex
                 *      Pointer to a memory location where the new Mutex is to
                 * be stored.
                 */
            ...
            public:  // Condition processing methods
                ...
                static void waitOnCondition(decaf_condition_t condition,
                                            decaf_mutex_t     mutex);

                /**
                 * @return true if the condition wait met the timeout
                 * parameters.
                 */
                static bool waitOnCondition(decaf_condition_t condition,
                                            decaf_mutex_t     mutex,
                                            long long         mills,
                                            int               nanos);

                static void interruptibleWaitOnCondition(
            ...
            public:  // Thread Methods.
                /**
                 * Given the Threading libraries max thread priority value,
                 * create a mapping to OS level thread priorities and place them
                 * in the provided vector.   This mapping will be used by the
                 * Threading library to map its values to the OS level values on
                 * calls to other methods like createNewThread and setPriority,
                 * etc.
                 *
                 * @param maxPriority
                 *      The maximum value that the Threading library uses for
                 * its priority range.
                 * @param mapping
                 *      A vector of int values that will be sized to maxPriority
                 * and maps the OS priority values to the Threading libs range
                 * of priority values.
                 */
                static void initPriorityMapping(int               maxPriority,
                                                std::vector<int>& mapping);

                static void createNewThread(decaf_thread_t*  handle,
                ...
                static void setStackSize(decaf_thread_t thread,
                                         long long      stackSize);

                /**
                 * Pause the current thread allowing another thread to be
                 * scheduled for execution, no guarantee that this will happen.
                 */
                static void yeild();
