...
public:
    SuspendedCompletionCondition(ThreadHandle* thread)
        : thread(thread)
    {
    }
...
private:
    ...
    MonitorWaitCompletionCondition& operator=(
        const MonitorWaitCompletionCondition&);

public:
    MonitorWaitCompletionCondition(ThreadHandle* handle)
        : handle(handle)
    {
    }
...
void           createThreadInstance(ThreadHandle* thread,
                                    long long     stackSize,
                                    int           priority,
                                    bool          suspended,
...
bool           doWaitOnMonitor(MonitorHandle* monitor,
                               ThreadHandle*  thread,
                               long long      mills,
                               int            nanos,
                               bool           interruptible);
...
void threadExit(ThreadHandle* self, bool destroy = false)
{
    ...
    // Don't detach - let join() call std::thread::join() for proper
    // synchronization PlatformThread::detachThread(handle);
    PlatformThread::exitThread();
}

PLATFORM_THREAD_CALLBACK_TYPE PLATFORM_CALLING_CONV
threadEntryMethod(PLATFORM_THREAD_ENTRY_ARG arg)
{
    ...
    if (thread->suspended == true)
    {
        ...
        PlatformThread::interruptibleWaitOnCondition(thread->condition,
                                                     thread->mutex,
                                                     completion);
    }
...
void runCallback(void* arg)
{
    ...
    catch (decaf::lang::Throwable& error)
    {
        ...
        {
            thread->parent->getUncaughtExceptionHandler()->uncaughtException(
                thread->parent,
                error);
        }
        else if (thread->parent->getDefaultUncaughtExceptionHandler() != NULL)
        {
            thread->parent->getDefaultUncaughtExceptionHandler()
                ->uncaughtException(thread->parent, error);
        }
    }
                          ...
                          void*         threadArg)
{
    ...
    PlatformThread::createNewThread(&thread->handle,
                                    threadEntryMethod,
                                    thread,
                                    library->priorityMapping[priority],
                                    stackSize,
                                    &thread->threadId);
}
...
bool interruptWaitingThread(ThreadHandle* self DECAF_UNUSED,
                            ThreadHandle*      target)
{
    bool result = false;
    ...
    createThreadInstance(target->interruptingThread,
                         0,
...
void doMonitorExit(MonitorHandle* monitor, ThreadHandle* thread DECAF_UNUSED)
{
    ...
    if (monitor->count == 0)
    {
        ...
        // since we are signaling waiting threads we unlock this under lock so
        // that they don't go back to sleep before we are done
        try
        {
...
void Threading::initialize()
{
    ...
    PlatformThread::initPriorityMapping(Thread::MAX_PRIORITY + 1,
                                        library->priorityMapping);
...
{
    // First shutdown the Executors static data to remove dependencies on
    // Threading.
    Executors::shutdown();
...
void Threading::dumpRunningThreads()
{
    ...
    std::list<ThreadHandle*>::const_iterator threads =
...
////////////////////////////////////////////////////////////////////////////////
ThreadHandle* Threading::createNewThread(Thread*     parent,
                                         const char* name,
                                         long long   stackSize)
{
    ...
    createThreadInstance(thread.get(),
                         stackSize,
                         Thread::NORM_PRIORITY,
                         true,
                         runCallback,
...
void Threading::destroyThread(ThreadHandle* thread)
{
    if (!thread->osThread)
    {
        ...
        // Check the state while holding the thread's mutex to ensure proper
        // synchronization
        PlatformThread::lockMutex(thread->mutex);
        ...
        // Threading::join returns immediately for threads in NEW state (never
        // started via start()) without calling the underlying
        // std::thread::join(). When destroyThread wakes a suspended NEW thread
        // (canceled=true above), the OS thread will still run threadExit() and
        // access library state (globalLock, etc.). We must block here until the
        // std::thread has fully returned before allowing cleanup to proceed --
        // otherwise the thread may access freed library state during shutdown
        // (e.g. Threading::shutdown() deletes library->globalLock while the
        // thread is still running threadExit(), causing a segfault). This call
        // is a no-op if the thread was already joined via the path above.
...
ThreadHandle* Threading::attachToCurrentThread()
{
    ...
    thread->name      = ::strdup(
        std::string(std::string("OS-Thread") +
                    Integer::toString(library->osThreadId.getAndIncrement()))
            .c_str());
...
void Threading::reinitialize(ThreadHandle* thread)
{
    try
    {
        ...
        thread->blocked     = false;
        thread->suspended =
            true;  // Start in suspended state as per createNewThread
        thread->interruptingThread = NULL;
        thread->joiners            = NULL;
        thread->joiningThread      = NULL;
...
void Threading::interrupt(ThreadHandle* thread)
{
    ...
    if (thread->interruptible == true)
    {
        if (thread->sleeping || thread->parked)
        {
            ...
            // If the thread is sleeping in a join, it's waiting on another
            // thread's condition Notify that thread's condition as well
            if (thread->joiningThread != NULL)
            {
                // printf("DEBUG interrupt: Thread is joining %p, notifying that
...
public:
    JoinCompletionCondition(ThreadHandle* self, ThreadHandle* target)
        : self(self),
          target(target)
    {
    }
    ...
    virtual bool operator()()
    {
        if (target != NULL)
        {
            // Read the target state atomically with sequentially consistent
            // ordering. Use seq_cst to ensure no reordering with mutex
            // operations. This ensures we see the state update that happened
...
bool Threading::join(ThreadHandle* thread, long long mills, int nanos)
{
    ...
    if (self->interrupted == true)
    {
        interrupted = true;
             ...
             self->state.load(std::memory_order_acquire) != Thread::TERMINATED)
    {
        // When blocking on ourself, we just enter a wait and hope there's
        // either a timeout, or we are interrupted.

        // If no timeout is specified, we can't wait forever (that's a
        // deadlock!) Just return immediately as if the join succeeded.
        if (mills == 0 && nanos == 0)
        {
            PlatformThread::unlockMutex(self->mutex);
            return false;
        ...
        // However, for consistency with the other wait path, we could unlock
        // first.
        timedOut = PlatformThread::interruptibleWaitOnCondition(self->condition,
                                                                self->mutex,
                                                                mills,
                                                                nanos,
                                                                completion);
    ...
    else
    {
        ...
        // Read the state atomically with acquire semantics to ensure proper
        // memory ordering
        int currentState = thread->state.load(std::memory_order_acquire);

        if (currentState >= Thread::RUNNABLE &&
            currentState != Thread::TERMINATED)
        {
            // Increment reference count to keep thread alive while we're
            // joining it This prevents the thread from being destroyed while
            // we're waiting
            ...
            // Enter wait - the completion condition will be checked in the wait
            // loop
            if (mills > 0 || nanos > 0)
            {
                self->timerSet = true;
                ...
                timedOut = PlatformThread::interruptibleWaitOnCondition(
                    thread->condition,
                    thread->mutex,
                    mills,
                    nanos,
            ...
            // IMPORTANT: Must read state BEFORE dereferencing, as dereference
            // may delete the thread
            PlatformThread::lockMutex(thread->mutex);
            int  finalState = thread->state.load(std::memory_order_acquire);
            bool threadTerminated = (finalState == Thread::TERMINATED);

            // Save handle before unlocking/dereferencing to ensure it remains
            // valid for join
            decaf_thread_t threadHandle = thread->handle;
            PlatformThread::unlockMutex(thread->mutex);

            // printf("DEBUG Threading::join: finalState=%d,
            // threadTerminated=%d\n", finalState, threadTerminated);

            // Release our reference on the thread now that we're done waiting
            dereferenceThread(thread);

            // Only call the underlying std::thread::join() if the thread
            // actually terminated If we timed out, the thread is still running,
            // so don't block on join
            if (threadTerminated)
            {
        ...
        else
        {
            ...
            // For NEW state threads, we should not call join on the underlying
            // thread because it was never properly started - just unlock and
            // return For TERMINATED threads, we still need to join for proper
            // synchronization
            ...
            else
            {
                ...
                // Thread already terminated, still call join for proper
                // synchronization
                PlatformThread::joinThread(threadHandle);
            }
        }

        // Additional memory fence to ensure volatile reads see the latest
        // values
...
public:
    SleepCompletionCondition(ThreadHandle* handle)
        : handle(handle)
    {
    }
...
bool Threading::isThreadAlive(ThreadHandle* handle DECAF_UNUSED)
{
    ...
    return currentState >= Thread::RUNNABLE &&
           currentState != Thread::TERMINATED;
}
...
{
    PlatformThread::setPriority(handle->handle,
                                library->priorityMapping[priority]);
...
public:
    ParkCompletionCondition(ThreadHandle* handle)
        : handle(handle)
    {
    }
...
bool Threading::park(Thread* thread, long long mills, int nanos)
{
    ...
    else
    {
        ...
        if (mills > 0 || nanos > 0)
        {
            ...
            timedOut =
                PlatformThread::interruptibleWaitOnCondition(handle->condition,
                                                             handle->mutex,
                                                             mills,
                                                             nanos,
                                                             completion);
...
void Threading::returnMonitor(MonitorHandle* monitor, bool alreadyLocked)
{
    ...
    // thread that's returning it, we will unlock it for the caller in this
    // case, otherwise we throw an error, and in every case we need to thrown if
    // someone's still waiting on the target monitor otherwise we could see a
    // segfault.
...
void Threading::exitMonitor(MonitorHandle* monitor)
{
    ...
    // Defensive: If we're not the owner, don't throw during potential exception
    // unwinding. Just return silently to avoid std::terminate(). This can
    // happen if:
...
////////////////////////////////////////////////////////////////////////////////
bool Threading::waitOnMonitor(MonitorHandle* monitor,
                              long long      mills,
                              int            nanos)
{
    ThreadHandle* self = getCurrentThreadHandle();
...
////////////////////////////////////////////////////////////////////////////////
void Threading::monitorExitUsingThreadId(MonitorHandle* monitor,
                                         ThreadHandle*  thread)
{
    if (monitor->owner != thread)
    {
...
////////////////////////////////////////////////////////////////////////////////
void Threading::monitorEnterUsingThreadId(MonitorHandle* monitor,
                                          ThreadHandle*  thread)
{
    if (monitor->owner == thread)
    {
...
////////////////////////////////////////////////////////////////////////////////
bool Threading::monitorTryEnterUsingThreadId(MonitorHandle* monitor,
                                             ThreadHandle*  thread)
{
    if (monitor->owner == thread)
    {
...
void Threading::destoryThreadLocalSlot(int slot)
{
    ...
    std::list<ThreadHandle*>::const_iterator iter =
        library->activeThreads.begin();
    ...
    std::vector<decaf::lang::Thread*>::const_iterator osIter =
