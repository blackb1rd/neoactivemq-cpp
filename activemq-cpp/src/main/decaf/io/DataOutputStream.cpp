...
////////////////////////////////////////////////////////////////////////////////
void DataOutputStream::doWriteArrayBounded(const unsigned char* buffer,
                                           int                  size,
                                           int                  offset,
                                           int                  length)
{
...
void DataOutputStream::writeBytes(const std::string& value)
{
    try
    {
        ...
        this->write((const unsigned char*)value.c_str(),
                    (int)value.length(),
                    0,
                    (int)value.length());
    }
...
void DataOutputStream::writeUTF(const std::string& value)
{
    try
    {
        ...
        for (std::size_t i = 0; i < length; i++)
        {
            ...
            {
                utfBytes[utfIndex++] =
                    (unsigned char)(0xc0 | (0x1f & (charValue >> 6)));
                utfBytes[utfIndex++] =
                    (unsigned char)(0x80 | (0x3f & charValue));
            }
            else
            {
                utfBytes[utfIndex++] =
                    (unsigned char)(0xe0 | (0x0f & (charValue >> 12)));
                utfBytes[utfIndex++] =
                    (unsigned char)(0x80 | (0x3f & (charValue >> 6)));
                utfBytes[utfIndex++] =
                    (unsigned char)(0x80 | (0x3f & charValue));
            }
        }
