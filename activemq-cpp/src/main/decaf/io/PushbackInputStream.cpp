...
////////////////////////////////////////////////////////////////////////////////
PushbackInputStream::PushbackInputStream(InputStream* stream,
                                         int          bufSize,
                                         bool         own)
    : FilterInputStream(stream, own),
      buffer(NULL),
      ...
      pos(bufSize)
{
    if (bufSize < 0)
    {
        throw IllegalArgumentException(
            __FILE__,
            __LINE__,
            "Size of Push Back buffer cannot be negative.");
    }
...
void PushbackInputStream::reset()
{
    throw IOException(__FILE__,
                      __LINE__,
                      "Reset is not Supported by the PushbackInputStream.");
}
...
void PushbackInputStream::unread(unsigned char value)
{
    if (pos == 0)
    {
        throw IOException(__FILE__,
                          __LINE__,
                          "No Space left in the unread buffer.");
    }
...
void PushbackInputStream::unread(const unsigned char* buffer, int size)
{
    if (size < 0)
    {
        throw IndexOutOfBoundsException(
            __FILE__,
            __LINE__,
            "Given size of the buffer was negatiev.");
    }
...
////////////////////////////////////////////////////////////////////////////////
void PushbackInputStream::unread(const unsigned char* buffer,
                                 int                  size,
                                 int                  offset,
                                 int                  length)
{
...
////////////////////////////////////////////////////////////////////////////////
int PushbackInputStream::doReadArrayBounded(unsigned char* buffer,
                                            int            size,
                                            int            offset,
                                            int            length)
{
    ...
    try
    {
        ...
        if (pos < bufferSize)
        {
            copyLength = (bufferSize - pos >= length) ? length
                                                      : bufferSize - pos;
        ...
        int inCopied =
            inputStream->read(buffer, size, newOffset, length - copiedBytes);
        if (inCopied > 0)
        {
