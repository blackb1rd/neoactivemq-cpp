...
namespace lang
{
    ...
    public:
        ...
        {
            // API calls for length() * 2 + 2 but we need to add one for Null
            // termination.
        ...
        void fixReversedMultibyte(char* string DECAF_UNUSED,
                                  int length   DECAF_UNUSED)
        {
            ...
            //            char temp;
            //
            //            // then scan all bytes and reverse each multibyte
            //            character for (scanl = scanr = str; temp = *scanr++;)
            //            {
            //                if ( (temp & 0x80) == 0) // ASCII char
            //                    scanl= scanr;
            //                else if ( (temp & 0xc0) == 0xc0 ) { // start of
            //                multibyte
            //                    scanr2= scanr;
            //                    switch (scanr - scanl) {
            //                        case 4: temp= *scanl, *scanl++= *--scanr,
            //                        *scanr= temp; // fallthrough case 3: //
            //                        fallthrough case 2: temp= *scanl,
            //                        *scanl++= *--scanr, *scanr= temp;
            //                    }
            //                    scanr = scanl = scanr2;
            //                }
            //            }
...
AbstractStringBuilder::AbstractStringBuilder(const std::string& source)
    : impl(new AbstractStringBuilderImpl((int)source.length() +
                                         INITIAL_CAPACITY))
{
...
////////////////////////////////////////////////////////////////////////////////
AbstractStringBuilder::AbstractStringBuilder(const CharSequence* source)
    : impl()
{
    if (source == NULL)
    {
...
void AbstractStringBuilder::doDeleteRange(int start, int end)
{
    ...
    // long as it's >= start. This means we have to clamp it to impl->length
    // here.
    if (end > impl->length)
    {
        end = impl->length;
    ...
    // impl->length is a NO-OP Since 'end' is already a clamped value, that case
    // is handled here.
    if (end == start)
    {
        return;
    ...
    if (length >= 0)
    {
        if (!impl->shared)
        {
            System::arraycopy(impl->value.get(),
                              end,
                              impl->value.get(),
                              start,
                              length);
...
////////////////////////////////////////////////////////////////////////////////
void AbstractStringBuilder::doInsert(int         index,
                                     const char* value,
                                     int         offset,
                                     int         length)
{
...
////////////////////////////////////////////////////////////////////////////////
void AbstractStringBuilder::getChars(int   start,
                                     int   end,
                                     char* dest,
                                     int   destSize,
                                     int   destStart) const
...
int AbstractStringBuilder::indexOf(const String& value, int start) const
{
    ...
    if (subCount > 0)
    {
        ...
        while (true)
        {
            ...
            {
                return -1;  // handles subCount > impl->length || start >=
                            // impl->length
            }
...
int AbstractStringBuilder::lastIndexOf(const String& value, int start) const
{
    ...
    if (subCount <= impl->length && start >= 0)
    {
        if (subCount > 0)
        {
            ...
            {
                start = impl->length -
                        subCount;  // impl->length and subCount are both >= 1
            }
...
String AbstractStringBuilder::toString() const
{
    ...
    if (wasted >= 256 ||
        (wasted >= INITIAL_CAPACITY && wasted >= (impl->length >> 1)))
    {
    ...
    // TODO optimize so that internal data can be shared with the returned
    // String
