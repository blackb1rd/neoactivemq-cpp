...
namespace lang
{
     ...
     * and is Thread Safe if the default Reference Counter is used.  This
     * Pointer type allows for the substitution of different Reference Counter
     * implementations which provide a means of using invasive reference
     * counting if desired using a custom implementation of
     * <code>ReferenceCounter</code>. <p> The Decaf smart pointer provide
     * comparison operators for comparing Pointer instances in the same manner
     * as normal pointer, except that it does not provide an overload of
     * operators ( <, <=, >, >= ).  To allow use of a Pointer in a STL container
     * that requires it, Pointer provides an implementation of std::less.
     *
     * @since 1.0
     */
    template <typename T>
        ...
        public:
            ...
            ArrayData()
                : value(NULL),
                  length(0),
                  refs(1),
                  arrayDeleter(&defaultArrayDeleter)
            {
            }

            ArrayData(T*  value,
                      int length,
                      void (*deleter)(T*) = &defaultArrayDeleter)
                : value(value),
                  length(length),
                  refs(1),
    ...
    public:
        ...
        typedef const T& ConstReferenceType;  // type returned by const
                                              // operator*
    ...
    public:
         ...
         * Default Constructor
         *
         * Initialized the contained array pointer to NULL, using the subscript
         * operator results in an exception unless reset to contain a real
         * value.
         */
        ArrayPointer()
            : array(new ArrayData()),
        ...
        /**
         * Create a new ArrayPointer instance and allocates an internal array
         * that is sized using the passed in size value.
         *
         * @param size
         *      The size of the array to allocate for this ArrayPointer
         * instance.
         */
        ...
        /**
         * Create a new ArrayPointer instance and allocates an internal array
         * that is sized using the passed in size value.  The array elements are
         ...
         * instance.
         * @param fillWith
         *      The value to initialize each element of the newly allocated
         * array with.
         */
        ...
        /**
         * Explicit Constructor, creates an ArrayPointer that contains value
         * with a single reference.  This object now has ownership until a call
         * to release.
         *
         * @param value
         *      The pointer to the instance of the array we are taking ownership
         * of.
         * @param size
         ...
         *
         * Stores a type-correct deleter so delete[] uses the original allocated
         * type U, avoiding undefined behavior when T is abstract (Apple clang
         * 17 traps the abstract D1 destructor called by delete[] through a base
         * pointer).
         *
         * @param value
         ...
         */
        template <typename U>
        explicit ArrayPointer(
            U*  value,
            int size,
            typename std::enable_if<std::is_convertible<U*, T*>::value &&
                                    !std::is_same<U, T>::value>::type* = NULL)
            : array(NULL),
              onDelete(onDeleteFunc)
        {
            try
            {
                this->array = new ArrayData(static_cast<T*>(value),
                                            size,
                                            &typedArrayDeleter<U>);
            }
            catch (std::exception& ex)
        ...
        /**
         * Copy constructor. Copies the value contained in the ArrayPointer to
         * the new instance and increments the reference counter.
         */
        ...
        /**
         * Resets the ArrayPointer to hold the new value.  Before the new value
         * is stored reset checks if the old value should be destroyed and if so
         * calls delete. Call reset with a value of NULL is supported and acts
         * to set this Pointer to a NULL pointer.
         *
         * @param value
        ...
        /**
         * Resets the ArrayPointer to hold a new derived-type array value,
         * storing a type-correct deleter so delete[] uses the original
         * allocated type U.
         */
        template <typename U>
        typename std::enable_if<std::is_convertible<U*, T*>::value &&
        ...
        /**
         * Releases the Pointer held and resets the internal pointer value to
         * Null.  This method is not guaranteed to be safe if the Pointer is
         * held by more than one object or this method is called from more than
         * one thread.
         *
         * @param value - The new value to contain.
         *
         * @return The pointer instance that was held by this Pointer object,
         * the pointer is no longer owned by this Pointer and won't be freed
         ...
         * This is not really safe since the caller could delete or alter the
         * pointer but it mimics the STL auto_ptr and gives access in cases
         * where the caller absolutely needs the real Pointer.  Use at your own
         * risk.
         *
        ...
        /**
         * Returns the current size of the contained array or zero if the array
         * is NULL.
         *
        ...
        /**
         * Creates a new ArrayPointer instance that is a clone of the value
         * contained in this ArrayPointer.
         *
        ...
        ArrayPointer clone() const
        {
            ...
            decaf::lang::System::arraycopy(this->array->value,
                                           0,
                                           copy.get(),
                                           0,
                                           this->array->length);
         ...
         * reference Count.
         * @param right - Pointer on the right hand side of an operator= call to
         * this.
         */
    ...
    /**
     * This implementation of Comparator is designed to allows objects in a
     * Collection to be sorted or tested for equality based on the value of the
     * value of the actual pointer to the array being contained in this
     * ArrayPointer.  This allows for a basic ordering to be acheived in Decaf
     * containers.
     *
     * Custom implementations are possible where an array of some type has a
     * logical natural ordering such as array of integers where the sum of all
     * ints in the array is used.
     */
    template <typename T>
    class ArrayPointerComparator
        : public decaf::util::Comparator<ArrayPointer<T>>
    {
    public:
        ...
        // Allows for operator less on types that implement Comparable or
        // provide a workable operator <
        virtual bool operator()(const ArrayPointer<T>& left,
        ...
        virtual int compare(const ArrayPointer<T>& left,
                            const ArrayPointer<T>& right) const
        {
            return left.get() < right.get()   ? -1
                   : right.get() < left.get() ? 1
                                              : 0;
        }
    };
