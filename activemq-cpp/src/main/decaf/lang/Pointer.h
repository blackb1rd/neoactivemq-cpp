...
namespace lang
{
    ...
    public:
         ...
         */
        explicit Pointer(const PointerType value)
            : REFCOUNTER(),
              value(value),
              onDelete(onDeleteFunc)
        {
         ...
         * new instance and increments the reference counter.
         *
         * @param value
         ...
         *
         * @param value
         *      A different but compatible Pointer instance that this Pointer
         * will copy.
         */
        ...
        /**
         * Static Cast constructor. Copies the value contained in the pointer to
         * the new instance and increments the reference counter performing a
         * static cast on the value contained in the source Pointer object.
         *
         * @param value
        ...
        Pointer(const Pointer<T1, R1>& value, const DYNAMIC_CAST_TOKEN&)
            : REFCOUNTER(value),
              value(dynamic_cast<T*>(value.get())),
              onDelete(onDeleteFunc)
        {
            if (this->value == NULL)
            {
                // Remove the reference we took in the Reference Counter's ctor
         ...
         * stored reset checks if the old value should be destroyed and if so
         * calls delete. Call reset with a value of NULL is supported and acts
         * to set this Pointer to a NULL pointer.
         *
         * @param value
         *      The new value to contain or NULL to empty the pointer (default
         * NULL if not set).
         */
         ...
         * but it mimics the STL auto_ptr and gives access in cases where the
         * caller absolutely needs the real Pointer.  Use at your own risk.
         *
    ...
    public:
        ...
        virtual bool operator()(const Pointer<T, R>& left,
                                const Pointer<T, R>& right) const
        {
            return *left < *right;
        }

        // Requires that the type in the pointer is an instance of a Comparable.
        virtual int compare(const Pointer<T, R>& left,
                            const Pointer<T, R>& right) const
        {
...
{  //: public binary_function<decaf::lang::Pointer<T>, decaf::lang::Pointer<T>,
   //: bool> {
