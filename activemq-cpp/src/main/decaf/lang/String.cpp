...
namespace lang
{
    ...
    public:
        ...
        /**
         * Contents created with the given length, the array is length + 1 to
         ...
         * window allowing for substring methods to not need to copy the
         * contents.
         */
...
String::String(const String& source)
    : contents(new Contents(*source.contents))
{
}

////////////////////////////////////////////////////////////////////////////////
String::String(const std::string& source)
    : contents(new Contents((int)source.length()))
{
    // load the passed string into the contents value.
    System::arraycopy(source.c_str(),
                      0,
                      contents->value.get(),
                      0,
                      source.length());
...
////////////////////////////////////////////////////////////////////////////////
String::String(const char* array, int size)
    : contents()
{
    if (size < 0)
    {
...
////////////////////////////////////////////////////////////////////////////////
String::String(const char* array, int size, int offset, int length)
    : contents()
{
    if (size < 0)
    {
...
const char* String::c_str() const
{
    ...
    System::arraycopy(contents->value.get(),
                      contents->offset,
                      newContents->value.get(),
                      0,
                      contents->length);
...
String String::compact() const
{
    ...
    {
        return String(contents->value.get(),
                      contents->offset,
                      contents->length);
    }
...
int String::compareTo(const String& string) const
{
    ...
    int end = contents->offset + (contents->length < string.contents->length
                                      ? contents->length
                                      : string.contents->length);

    while (o1 < end)
    {
        if ((result = contents->value[o1++] - string.contents->value[o2++]) !=
            0)
        {
            return result;
        }
...
int String::compareTo(const std::string& string) const
{
    ...
    int end = contents->offset + (contents->length < (int)string.length()
                                      ? contents->length
                                      : (int)string.length());
...
String String::concat(const String& string) const
{
    ...
    System::arraycopy(string.contents->value.get(),
...
{
    return regionMatches(contents->length - suffix.contents->length,
                         suffix,
                         0,
                         suffix.contents->length);
}
...
bool String::equals(const String& other) const
{
    ...
    {
        if (contents->value[contents->offset + i] !=
            other.contents->value[other.contents->offset + i])
        {
            return false;
        }
    }
...
bool String::equalsIgnoreCase(const String& string) const
{
    ...
    {
        if ((c1 = contents->value[offsetThis++]) !=
            (c2 = target[offsetOther++]))
        {
            // If we add support for multibyte strings we need to check both
            // cases toUpperCase and toLowerCase because of Unicode.
...
int String::findFirstOf(const String& chars, int start) const
{
    if (start < contents->length)
    {
        ...
        for (int i = contents->offset + start;
             i < contents->offset + contents->length;
             i++)
        {
...
////////////////////////////////////////////////////////////////////////////////
void String::getChars(int   srcBegin,
                      int   srcEnd,
                      char* dest,
                      int   destSize,
                      int   destBegin) const
...
int String::hashCode() const
{
    if (contents->hashCode == 0)
    {
        ...
        for (int i = contents->offset; i < contents->length + contents->offset;
             i++)
        {
...
////////////////////////////////////////////////////////////////////////////////
bool String::regionMatches(int           thisStart,
                           const String& string,
                           int           start,
                           int           length) const
{
...
////////////////////////////////////////////////////////////////////////////////
bool String::regionMatches(bool          ignoreCase,
                           int           thisStart,
                           const String& string,
                           int           start,
                           int           length) const
{
    ...
    while (thisStart < end)
    {
        if ((c1 = contents->value[thisStart++]) !=
            (c2 = string.contents->value[start++]))
        {
            // If we add support for multibyte strings we need to check both
            // cases toUpperCase and toLowerCase because of Unicode.
            if (Character::toUpperCase(c1) != Character::toUpperCase(c2))
...
String String::substring(int start) const
{
    ...
    {
        return String(contents->offset + start,
                      contents->length - start,
                      contents->value);
    }
...
////////////////////////////////////////////////////////////////////////////////
CharSequence* String::subSequence(int start DECAF_UNUSED,
                                  int end   DECAF_UNUSED) const
{
    try
    {
        if (start > end)
...
char* String::toCharArray() const
{
    ...
    System::arraycopy((const char*)contents->value.get(),
                      contents->offset,
                      buffer,
                      0,
                      contents->length);
...
String String::toLowerCase() const
{
    ...
    {
        newContents.value[i] =
            Character::toLowerCase(contents->value[offset + i]);
    }
...
String String::toUpperCase() const
{
    ...
    {
        newContents.value[i] =
            Character::toUpperCase(contents->value[offset + i]);
    }
...
std::string String::toString() const
{
    ...
    return std::string((const char*)contents->value.get() + contents->offset,
                       this->length());
}
