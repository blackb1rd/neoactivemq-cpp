...
namespace lang
{
     ...
     * Transaction ID).  This class imposes the restriction on the type that it
     * will contains that it must by both assignable and copyable.
     *
    ...
    template <typename E>
    class ThreadLocal
        : protected decaf::internal::util::concurrent::ThreadLocalImpl
    {
    private:
    ...
    public:
         ...
         * this thread-local variable is subsequently read by the current
         * thread, its value will be reinitialized by invoking its
         * initialValue() method, unless its value is set by the current thread
    ...
    protected:
         ...
         * previously invoked the set() method, in which case the initialValue
         ...
         * of subsequent invocations of remove() followed by get().
         *
         * This implementation simply returns E(); if the programmer desires
         * thread-local variables to have an initial value other than E(),
         * ThreadLocal must be subclassed, and this method overridden.
         * Typically, an inner class will be used.
         *
         * @param value
         *      Pointer to the thread local value created by this thread when
         * get() is first called.
         */
