...
namespace nio
{
     ...
     * operations:
     *   * Relative operations read or write one or more elements starting at
     * the current position and then increment the position by the number of
     *     elements transferred. If the requested transfer exceeds the limit
     * then a relative get operation throws a BufferUnderflowException and a
     * relative put operation throws a BufferOverflowException; in either case,
     ...
     * the reset method is invoked. The mark is not always defined, but when it
     * is defined it is never negative and is never greater than the position.
     * If the mark is defined then it is discarded when the position or the
     * limit is adjusted to a value smaller than the mark. If the mark is not
     * defined then invoking the reset method causes an InvalidMarkException to
     * be thrown.
     *
     * Invariants:
     *
     *   The following invariant holds for the mark, position, limit, and
     * capacity values: 0 <= mark <= position <= limit <= capacity
     *
     *   A newly-created buffer always has a position of zero and a mark that is
     *   undefined.  The initial limit may be zero, or it may be some other
     * value that depends upon the type of the buffer and the manner in which it
     * is constructed. The initial content of a buffer is, in general,
     * undefined.
     *
     ...
     * operations that will throw a ReadOnlyBufferException when invoked upon a
     * read-only buffer. A read-only buffer does not allow its content to be
     * changed, but its mark, position, and limit values are mutable. Whether or
     * not a buffer is read-only may be determined by invoking its isReadOnly
     * method.
     *
     * Thread safety:
     *
    ...
    public:
         ...
         *
         * @param newLimit
         *      The new limit value; must be no larger than this buffer's
         * capacity.
         *
         * @return A reference to This buffer
         *
         ...
         * named as if it did because it will most often be used in situations
         ...
         * the limit.
         *
         * @return true if, and only if, there is at least one element remaining
         * in this buffer.
         */
