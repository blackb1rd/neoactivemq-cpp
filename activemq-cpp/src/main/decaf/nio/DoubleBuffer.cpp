...
////////////////////////////////////////////////////////////////////////////////
DoubleBuffer* DoubleBuffer::wrap(double* buffer,
                                 int     size,
                                 int     offset,
                                 int     length)
{
...
DoubleBuffer* DoubleBuffer::wrap(std::vector<double>& buffer)
{
    try
    {
        ...
        return BufferFactory::createDoubleBuffer(&buffer[0],
                                                 (int)buffer.size(),
                                                 0,
                                                 (int)buffer.size());
    }
        ...
        if (length > remaining())
        {
            throw BufferUnderflowException(
                activemq::exceptions::buildSourceMessage(
                    __FILE__,
                    __LINE__,
                    std::string("DoubleBuffer::get - Not enough data to fill "
                                "length = ") +
...
////////////////////////////////////////////////////////////////////////////////
DoubleBuffer& DoubleBuffer::put(const double* buffer,
                                int           size,
                                int           offset,
                                int           length)
{
    try
    {
        ...
        if (length > this->remaining())
        {
            throw BufferOverflowException(__FILE__,
                                          __LINE__,
                                          "DoubleBuffer::put - Not Enough "
                                          "space to store requested Data.");
        }
