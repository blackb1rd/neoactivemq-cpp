...
namespace util
{
     ...
     * the set(int, E) method (which otherwise throws an
     * UnsupportedOperationException). If the list is variable-size the
     * programmer must additionally override the add(int, E) and remove(int)
     * methods.
     *
     ...
     * not have to provide an iterator implementation; the iterator and list
     * iterator are implemented by this class, on top of the "random access"
     * methods: get(int), set(int, E), add(int, E) and remove(int).
     *
    ...
    private:
        ...
        public:
            ...
            virtual void remove()
            {
                ...
                try
                {
                    if (this->lastPosition ==
                        this->parent->size() - this->numLeft)
                    {
                        this->numLeft--;  // we're removing after a call to
                                          // previous()
                    }
            ...
            virtual void add(const E& value)
            {
                ...
                try
                {
                    this->parent->add(this->parent->size() - this->numLeft,
                                      value);
                    this->expectedModCount = this->parent->modCount;
                    this->lastPosition     = -1;
                }
    ...
    public:
        ...
        // Use this method since our own addAll will hide the base class
        // version.
        ...
        {
            std::unique_ptr<decaf::util::ListIterator<E>> iter(
                this->listIterator());
    ...
    protected:
        ...
        {
            std::unique_ptr<decaf::util::Iterator<E>> iter(
                this->listIterator(start));
