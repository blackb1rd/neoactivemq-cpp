...
////////////////////////////////////////////////////////////////////////////////
BitSet::BitSet(unsigned long long* bits,
               int                 bitsSize,
               bool                needClear,
    ...
    : bits(bits),
      bitsSize(bitsSize),
      needClear(needClear),
      actualArrayLength(actualArrayLength),
      isLengthActual(isLengthActual)
{
}
...
void BitSet::AND(const BitSet& set)
{
    ...
    isLengthActual =
        !((actualArrayLength > 0) && (bits[actualArrayLength - 1] == 0));
}
...
void BitSet::andNot(const BitSet& set)
{
    ...
    int range = actualArrayLength < set.actualArrayLength
                    ? actualArrayLength
                    : set.actualArrayLength;
...
BitSet BitSet::get(int fromIndex, int toIndex) const
{
    ...
    if (idx1 == idx2)
    {
        unsigned long long result = (bits[idx1] & (factor1 & factor2)) >>
                                    (fromIndex % ELM_SIZE);
        if (result == 0)
        {
            return BitSet(0);
    ...
    // shift all the elements in the new bitset to the right by fromIndex %
    // ELM_SIZE
