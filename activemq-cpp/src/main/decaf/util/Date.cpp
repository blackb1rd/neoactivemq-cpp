...
std::string Date::toString() const
{
...
#ifdef _WIN32
        // Windows timezone handling - convert IANA timezone names to Windows
        // format
    ...
    char buffer[100];
#ifdef _WIN32
    // Windows has issues with %Z formatting for custom TZ values, so handle it
    // manually
    if (!originalTzValue.empty())
    {
        ...
        auto isUSDST = [timeInfo]() -> bool
        {
            ...
            // DST rules for US: second Sunday in March to first Sunday in
            // November
            if (month < 3 || month > 11)
            {
                return false;  // Dec, Jan, Feb - no DST
            }
            if (month > 3 && month < 11)
            {
                return true;  // Apr-Oct - always DST
            }
            ...
            if (month == 3)
            {
                if (day < 8)
                {
                    return false;  // Before second week
                }
                if (dayOfWeek == 0 && day >= 8 && day <= 14)
                {
                    return true;  // Second Sunday
                }
                if (day > 14)
                {
                    return true;  // After second Sunday
                }
            ...
            if (month == 11)
            {
                if (day > 7)
                {
                    return false;  // After first week
                }
                if (dayOfWeek == 0 && day <= 7)
                {
                    return false;  // First Sunday ends DST
                }
        ...
        {
            // BST (British Summer Time): last Sunday in March to last Sunday in
            // October
        ...
        else if (originalTzValue == "Europe/Paris" ||
                 originalTzValue == "Europe/Berlin")
        {
            // CEST (Central European Summer Time): last Sunday in March to last
            // Sunday in October
            int month     = timeInfo->tm_mon + 1;
            int day       = timeInfo->tm_mday;
            int dayOfWeek = timeInfo->tm_wday;
        ...
        {
            // AEDT (Australian Eastern Daylight Time): first Sunday in October
...
std::string Date::convertToWindowsTimezone(const std::string& ianaTimezone) const
{
    ...
    if (ianaTimezone == "America/New_York")
    {
        ...
        // When DST name is specified without offset, it defaults to standard
        // time + 1 hour So EST5EDT means: EST is UTC-5, EDT is UTC-5+1=UTC-4
    ...
    else if (ianaTimezone == "Europe/London")
    {
        ...
        // DST starts: Last Sunday in March at 1:00 AM, ends: Last Sunday in
        // October at 2:00 AM
