...
namespace util
{
     ...
     *
     * If many mappings are to be stored in a HashMap instance, creating it with
     * a sufficiently large capacity will allow the mappings to be stored more
     ...
     * contains is not a structural modification.) This is typically
    ...
    private:
        ...
        public:
            ConstEntryIterator(const HashMap* parent)
        ...
        public:
            ConstKeyIterator(const HashMap* parent)
                : ConstAbstractMapIterator(parent)
            {
            }
        ...
        public:
            ConstValueIterator(const HashMap* parent)
                : ConstAbstractMapIterator(parent)
            {
            }
    ...
    protected:
        ...
        private:
            ...
            ConstHashMapValueCollection& operator=(
    ...
    protected:
        ...
        /*
         * The internal data structure to hold Entries, Array of MapEntry
         * pointers.
         */
         ...
         * the HashMap and the iterator
         */
        int modCount;

        /*
         * maximum ratio of (stored elements)/(storage size) which does not lead
         * to rehash
         */
        float loadFactor;

        /*
         * maximum number of elements that can be put in this map before having
         * to rehash
         */
        int threshold;

        // Cached values that are only initialized once a request for them is
        // made.
        ...
        mutable decaf::lang::Pointer<ConstHashMapValueCollection>
    ...
    public:
              ...
              cachedConstValueCollection()
        {
            if (capacity >= 0)
            {
                ...
                elementCount = 0;
                elementData =
                    decaf::lang::ArrayPointer<HashMapEntry*>(capacity);
                computeThreshold();
            }
            else
            {
        ...
        /**
         * Creates a new HashMap with default configuration settings and fills
         * it with the contents of the given source Map instance.
         *
         * @param map
         *      The Map instance whose elements are copied into this HashMap
         * instance.
         */
    ...
    public:
        ...
        virtual Collection<V>& values()
        {
            if (this->cachedValueCollection == NULL)
            {
                this->cachedValueCollection.reset(
                    new HashMapValueCollection(this));
            }
        ...
        virtual const Collection<V>& values() const
        {
            if (this->cachedConstValueCollection == NULL)
            {
                this->cachedConstValueCollection.reset(
                    new ConstHashMapValueCollection(this));
            }
        ...
        virtual bool equals(const Map<K, V>& source) const
        {
            ...
            try
            {
                decaf::lang::Pointer<Iterator<MapEntry<K, V>>> iter(
                    entrySet().iterator());
                while (iter->hasNext())
                {
    ...
    protected:
        ...
        virtual HashMapEntry* createEntry(const K& key,
                                          int      index,
                                          const V& value)
        {
        ...
        virtual HashMapEntry* createHashedEntry(const K& key,
                                                int      index,
                                                int      hash)
        {
    ...
    protected:
        void putAllImpl(const Map<K, V>& map)
        {
            ...
            decaf::lang::Pointer<Iterator<MapEntry<K, V>>> iterator(
        ...
        HashMapEntry* findKeyEntry(const K& key, int index, int keyHash) const
        {
            ...
            while (entry != NULL &&
                   (entry->origKeyHash != keyHash || !(key == entry->getKey())))
            {
                entry = entry->next;
            }
            return entry;
        ...
        HashMapEntry* removeEntry(const K& key)
        {
            ...
            while (current != NULL &&
                   !(current->origKeyHash == hash && key == current->getKey()))
            {
                last    = current;
                current = current->next;
            }
