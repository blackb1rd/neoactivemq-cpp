...
namespace util
{
    ...
    /**
     * Hashed and linked list implementation of the Map interface, with
     ...
     * were inserted into the map (insertion-order). Note that insertion order
     * is not affected if a key is re-inserted into the map. (A key k is
     ...
     * chaotic ordering provided by HashMap, without incurring the increased
     * cost associated with TreeMap. It can be used to produce a copy of a map
     * that has the same order as the original, regardless of the original map's
     * implementation:
     *
     *    void foo(Map m) {
     ...
     * the copy. (Clients generally appreciate having things returned in the
     * same order they were presented.)
     *
     * A special constructor is provided to create a linked hash map whose order
     * of iteration is the order in which its entries were last accessed, from
     * least-recently accessed to most-recently (access-order). This kind of map
     * is well-suited to building LRU caches. Invoking the put or get method
     * results in an access to the corresponding entry (assuming it exists after
     * the invocation completes). The putAll method generates one entry access
     * for each mapping in the specified map, in the order that key-value
     ...
     *
     * The removeEldestEntry(MapEntry) method may be overridden to impose a
     * policy for removing stale mappings automatically when new mappings are
     * added to the map.  When the entries are about to be removed from the map
     * the onEviction method is called giving subclasses a chance to delete
     * pointer values or perform other cleanup prior to the mapping being
     * removed.
     *
     * This class provides all of the optional Map operations.  Like HashMap, it
     * provides constant-time performance for the basic operations (add,
     * contains and remove), assuming the hash function disperses elements
     * properly among the buckets. Performance is likely to be just slightly
     * below that of HashMap, due to the added expense of maintaining the linked
     * list, with one exception: Iteration over the collection-views of a
     * LinkedHashMap requires time proportional to the size of the map,
     * regardless of its capacity. Iteration over a HashMap is likely to be more
     ...
     * capacity and load factor. They are defined precisely as for HashMap.
     * Note, however, that the penalty for choosing an excessively high value
     * for initial capacity is less severe for this class than for HashMap, as
     ...
     * modifies the map structurally, it must be synchronized externally. This
     * is typically accomplished by synchronizing on some object that naturally
     * encapsulates the map. If no such object exists, the map should be
     * "wrapped" using the Collections.synchronizedMap method. This is best done
     ...
     * more mappings or, in the case of access-ordered linked hash maps, affects
     * iteration order. In insertion-ordered linked hash maps, merely changing
     * the value associated with a key that is already contained in the map is
     * not a structural modification. In access-ordered linked hash maps, merely
     * querying the map with get is a structural modification.)
     *
     * The iterators returned by the iterator method of the collections returned
    ...
    private:
        ...
        public:
            LinkedHashMapEntry(const K& key, const V& value, int hash)
                : HashMap<K, V, HASHCODE>::HashMapEntry(key, value, hash),
                  chainForward(),
                  chainBackward()
            {
            }
            ...
            LinkedHashMapEntry(const K& key, const V& value)
                : HashMap<K, V, HASHCODE>::HashMapEntry(key, value),
                  chainForward(),
                  chainBackward()
            {
            }
    ...
    private:
        ...
        class EntryIterator : public Iterator<MapEntry<K, V>>,
        ...
        public:
            EntryIterator(LinkedHashMap* parent)
                : AbstractMapIterator(parent)
            {
            }
        ...
        public:
            KeyIterator(LinkedHashMap* parent)
                : AbstractMapIterator(parent)
            {
            }
        ...
        public:
            ValueIterator(LinkedHashMap* parent)
                : AbstractMapIterator(parent)
            {
            }
    ...
    private:
        ...
        class ConstEntryIterator : public Iterator<MapEntry<K, V>>,
        ...
        public:
            ConstEntryIterator(const LinkedHashMap* parent)
                : ConstAbstractMapIterator(parent)
            {
            }
        ...
        class ConstKeyIterator : public Iterator<K>,
                                 public ConstAbstractMapIterator
        {
        private:
        ...
        public:
            ConstKeyIterator(const LinkedHashMap* parent)
                : ConstAbstractMapIterator(parent)
            {
            }
        ...
        class ConstValueIterator : public Iterator<V>,
                                   public ConstAbstractMapIterator
        {
        private:
        ...
        public:
            ConstValueIterator(const LinkedHashMap* parent)
                : ConstAbstractMapIterator(parent)
            {
            }
    ...
    private:
        ...
        class LinkedHashMapEntrySet
            : public HashMap<K, V, HASHCODE>::HashMapEntrySet
        {
        private:
            LinkedHashMap* associatedMap;
        ...
        public:
            LinkedHashMapEntrySet(LinkedHashMap* parent)
                : HashMap<K, V, HASHCODE>::HashMapEntrySet(parent),
                  associatedMap(parent)
            {
            }
        ...
        class ConstLinkedHashMapEntrySet
            : public HashMap<K, V, HASHCODE>::ConstHashMapEntrySet
        {
        private:
            const LinkedHashMap* associatedMap;
    ...
    private:
        class LinkedHashMapKeySet
            : public HashMap<K, V, HASHCODE>::HashMapKeySet
        {
        private:
            LinkedHashMap* associatedMap;
        ...
        public:
            LinkedHashMapKeySet(LinkedHashMap* parent)
                : HashMap<K, V, HASHCODE>::HashMapKeySet(parent),
                  associatedMap(parent)
            {
            }
        ...
        class ConstLinkedHashMapKeySet
            : public HashMap<K, V, HASHCODE>::ConstHashMapKeySet
        {
        private:
            const LinkedHashMap* associatedMap;
        ...
        public:
            ConstLinkedHashMapKeySet(const LinkedHashMap* parent)
                : HashMap<K, V, HASHCODE>::ConstHashMapKeySet(parent),
                  associatedMap(parent)
            {
            }
    ...
    private:
        class LinkedHashMapValueCollection
            : public HashMap<K, V, HASHCODE>::HashMapValueCollection
        {
        private:
            LinkedHashMap* associatedMap;
        ...
        private:
            ...
            LinkedHashMapValueCollection& operator=(
                const LinkedHashMapValueCollection&);

        public:
            LinkedHashMapValueCollection(LinkedHashMap* parent)
                : HashMap<K, V, HASHCODE>::HashMapValueCollection(parent),
                  associatedMap(parent)
            {
            }
        ...
        class ConstLinkedHashMapValueCollection
            : public HashMap<K, V, HASHCODE>::ConstHashMapValueCollection
        {
        private:
            const LinkedHashMap* associatedMap;

        private:
            ConstLinkedHashMapValueCollection(
                const ConstLinkedHashMapValueCollection&);
            ConstLinkedHashMapValueCollection& operator=(
                const ConstLinkedHashMapValueCollection&);

        public:
            ConstLinkedHashMapValueCollection(const LinkedHashMap* parent)
                : HashMap<K, V, HASHCODE>::ConstHashMapValueCollection(parent),
                  associatedMap(parent)
            {
            }
    ...
    public:
        ...
        LinkedHashMap(int capacity, float load)
            : HashMap<K, V, HASHCODE>(capacity, load),
              accessOrder(false),
              head(),
              tail()
        {
         ...
         *      The initial load factor for this map.
         * @param order
         *      True if the ordering should be done based on the last access
         * (from least-recently accessed to most-recently accessed), and  false
    ...
    protected:
         ...
         * the eldest member of the map should be deleted before adding the new
         * member. If this map was created with accessOrder = true, then the
         * result of removeEldestEntry is assumed to be false.
         *
         * @param eldest
        ...
        /**
         * This method is called when the removeEldestEntry has returned true
         * and a MapEntry is about to be removed from the Map.  This method
         * allows for Maps that contain pointers in their MapEntry object to
         * have a chance to properly delete the pointer when the entry is
         * removed.
         *
         * @param eldest
    ...
    public:
        ...
        {
            LinkedHashMapEntry* toRemove =
                (LinkedHashMapEntry*)this->removeEntry(key);
            if (toRemove != NULL)
            {
        ...
        virtual const Set<MapEntry<K, V>>& entrySet() const
        {
            ...
            {
                this->cachedConstEntrySet.reset(
                    new ConstLinkedHashMapEntrySet(this));
            }
        ...
        virtual const Set<K>& keySet() const
        {
            ...
            {
                this->cachedConstKeySet.reset(
                    new ConstLinkedHashMapKeySet(this));
            }
        ...
        virtual Collection<V>& values()
        {
            ...
            {
                this->cachedValueCollection.reset(
                    new LinkedHashMapValueCollection(this));
            }
        ...
        virtual const Collection<V>& values() const
        {
            ...
            {
                this->cachedConstValueCollection.reset(
                    new ConstLinkedHashMapValueCollection(this));
            }
    ...
    protected:
        ...
        virtual typename HashMap<K, V, HASHCODE>::HashMapEntry*
        ...
        void linkEntry(LinkedHashMapEntry* entry)
        {
            ...
            // of the chain depending on if the LinkedHashMap is accessOrder or
            // not
            LinkedHashMapEntry* prev = entry->chainBackward;
            LinkedHashMapEntry* next = entry->chainForward;
            if (prev == NULL)
        ...
        virtual bool putImpl(const K& key, const V& value, V& oldValue)
        {
            ...
            if (entry == NULL)
            {
                ...
                entry    = (LinkedHashMapEntry*)this->createHashedEntry(key,
                                                                     index,
                                                                     hash);
                replaced = false;
            }
