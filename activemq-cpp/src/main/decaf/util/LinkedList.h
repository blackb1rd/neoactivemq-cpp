...
namespace util
{
    ...
    /**
     * A complete implementation of the List interface using a doubly linked
     * list data structure.
     *
     * This class also implements the Deque interface providing a common
     * interface for additions into the list at the front and end as well as
     * allowing insertions anywhere in between. This class can be used then to
     * implement other data structures such as Stacks, Queue's or double ended
     * Queue's.
     *
     * The operations on this List object that index a particular element
     * involve iterating over the links of the list from beginning to end,
     * starting from whichever end is closer to the location the operation is to
     * be performed on.
     *
     * @since 1.0
     */
    ...
    public:
        ...
        virtual int indexOf(const E& value) const
        {
            ...
            for (int i = 0; location != &this->tail;
        ...
        virtual int lastIndexOf(const E& value) const
        {
            ...
            for (int i = this->listSize - 1; location != &this->head;
                 --i, location = location->prev)
            {
                if (location->value == value)
                {
                    return i;
    ...
    private:
        ...
        public:
                  ...
                  expectedModCount(0)
            {
                ...
                // that the list is empty.  We always want to start out one
                // before so that the call to next moves us onto the element in
                // question;

                if (index < this->list->listSize / 2)
                {
                    this->current = &this->list->head;
                    for (this->index = -1; this->index + 1 < index;
                         ++this->index)
                    {
                ...
                else
                {
                    ...
                    for (this->index = this->list->listSize;
                         this->index >= index;
                         --this->index)
                    {
