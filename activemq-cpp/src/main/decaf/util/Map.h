...
namespace util
{
     ...
     * equals and hashCode methods are no longer well defined on such a map.
     *
     ...
     * an empty map, and a constructor with a single argument of type Map, which
     * creates a new map with the same key-value mappings as its argument. In
     * effect, the latter constructor allows the user to copy any map, producing
     ...
     * in the insertion of an ineligible element into the map may throw an
     * exception or it may succeed, at the option of the implementation. Such
     * exceptions are marked as "optional" in the specification for this
     * interface.
     *
    ...
    public:
         ...
         * true if the two maps represent the same mappings. More formally, two
         * maps m1 and m2 represent the same mappings if
         * m1.entrySet().equals(m2.entrySet()). This ensures that the equals
        ...
        /**
         * Copies the content of the source map into this map.  Erases all
         * existing mappings in this map.  The copy is performed by using the
         * entrySet of the source Map and iterating over those entries,
         * inserting each into the target.
         *
         * @param source
         ...
         * mapping for a key k such that (key == k). (There can be at most one
         * such mapping.)
         *
         * @param key
         *      The key to look up.
         ...
         * at least one mapping to a value v such that (value==v). This
         ...
         *
         * @return true if this map contains at least one mapping for the value,
         * otherwise false.
         */
         ...
         *      The value to be set.
         *
         * @return true if the put operation replaced a value that was
         ...
         * m.containsKey(k) would return true.)
         *
         * This method accepts a reference to a value which will be assigned the
         * previous value for the given key (if any).  If there was no previous
         * mapping for the given key the out value is not written to.  A return
         * of true indicates that a value was replaced by this put operation.
         *
         * @param key
         *      The target key.
         * @param value
        ...
        /**
         * Removes the value (key/value pair) for the specified key from the
         * map, returns a copy of the value that was mapped to the key.  Care
         * must be taken when using this operation as it will throw an exception
         ...
         * operations.
         *
         * @return a reference  to a Set<MapEntry<K,V>> that is backed by this
         * Map.
         */
         ...
         * not support the add or addAll operations.  For the const version of
         * this method the Collection can only be used as a view into the Map.
         *
