...
namespace util
{
    ...
    /**
     * An unbounded priority queue based on a binary heap algorithm. The
     * elements of the priority queue are ordered according to their natural
     * ordering, or by a Comparator provided to one of the constructors that
     * accepts Comparators.  A priority queue relying on natural ordering also
     * does not permit insertion of non-comparable objects (doing so may result
     * in a compiler error).
     *
     * The head of this queue is the least element with respect to the specified
     * ordering. If multiple elements are tied for least value, the head is one
     * of those elements -- ties are broken arbitrarily. The queue retrieval
     ...
     * least as large as the queue size. As elements are added to a priority
     * queue, its capacity grows automatically. The details of the growth policy
     * are not specified.
     *
     ...
     * using <code>Arrays::sort( pq.toArray() )</code>.
     *
    ...
    private:
        ...
        public:
            PriorityQueueIterator(PriorityQueue* queue)
                : position(0),
                  allowRemove(false),
                  queue(queue)
            {
        ...
        private:
            ...
            ConstPriorityQueueIterator& operator=(
    ...
    public:
        ...
        /**
         * Creates a Priority Queue with the default initial capacity.  This new
         * PriorityQueue takes ownership of the passed Comparator instance and
         * uses that to determine the ordering of the elements in the Queue.
         *
         * @param initialCapacity
         *      The initial number of elements allocated to this PriorityQueue.
         * @param comparator
        ...
        /**
         * Creates a PriorityQueue containing the elements in the specified
         * priority queue. This priority queue will be ordered according to the
         * same ordering as the given priority queue.
         *
         * @param source
    ...
    public:
        ...
        virtual void clear()
        {
            // TODO - Provide a more efficient way to clear the array without
            // reallocating it
            //        we should keep the size it grew to since if reused it
            //        could get that big again and reallocating all that memory
            //        could be to slow.
        ...
        /**
         * obtains a Copy of the Pointer instance that this PriorityQueue is
         * using to compare the elements in the queue with.  The returned value
         * is a copy, the caller cannot change the value if the internal Pointer
         * value.
         *
    ...
    private:
        ...
        void upHeap()
        {
            ...
            while (current != 0 && _comparator->compare(elements[current],
                                                        elements[parent]) < 0)
            {
                // swap the two
        ...
        void downHeap(int pos)
        {
            ...
            while (child < _size && !this->isEmpty())
            {
                ...
                if (child + 1 < _size &&
                    _comparator->compare(elements[child + 1], elements[child]) <
                        0)
                {
                    child++;
                }
                ...
                if (_comparator->compare(elements[current], elements[child]) <
                    0)
                {
                    break;
                }
