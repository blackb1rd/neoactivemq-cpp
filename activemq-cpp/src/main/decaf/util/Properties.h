...
namespace util
{
     ...
     * <p>
     * The Properties list contains a key value pair of properties that can be
     * loaded and stored to a stream.  Each Properties instance can contain an
     * internal Properties list that contains default values for keys not found
     * in the Properties List. <p> The Properties list if a Thread Safe class,
     * it can be shared amongst objects in multiple threads without the need for
    ...
    protected:
        /**
         * Default list used to answer for any keys not found in the properties
         * list, can be filled in by another implementation of this class.
         */
    ...
    public:
         ...
         *
         * @return The value of the property specified by <code>name</code>, if
         * it exists, otherwise the <code>defaultValue</code>.
         */
         ...
         */
        std::string setProperty(const std::string& name,
        ...
        /**
         * Copies the contents of the given properties object to this one, if
         * the given Properties instance in empty then this List is not
         * modified.
         *
         * @param source
         ...
         * Objects are considered equivalent when they each contain the same
         * number of elements and each key / value pair contained within the two
         * are equal.
         *
         ...
         *
         * @return true if the contents of the two Properties objects are the
         * same.
         */
        bool equals(const Properties& source) const;

        /**
         * Formats the contents of the Properties Object into a string that can
         * be logged, etc.
         *
         ...
         * character stream in a simple line-oriented format. <p> Properties are
         * processed in terms of lines. There are two kinds of line, natural
         * lines and logical lines. A natural line is defined as a line of
         * characters that is terminated either by a set of line terminator
         * characters (\n or \r or \r\n) or by the end of the stream. A natural
         * line may be either a blank line, a comment line, or hold all or some
         * of a key-element pair. A logical line holds all the data of a
         * key-element pair, which may be spread out across several adjacent
         * natural lines by escaping the line terminator sequence with a
         * backslash character \. Note that a comment line cannot be extended in
         * this manner; every natural line that is a comment must have its own
         * comment indicator, as described below. Lines are read from input
         * until the end of the stream is reached. <p> A natural line that
         * contains only white space characters is considered blank and is
         * ignored. A comment line has an ASCII '#' or '!' as its first
         ...
         * format considers the characters space (' '), tab ('\t'), and form
         * feed ('\f') to be white space. <p> If a logical line is spread across
         * several natural lines, the backslash escaping the line terminator
         * sequence, the line terminator sequence, and any white space at the
         * start of the following line have no affect on the key or element
         * values. The remainder of the discussion of key and element parsing
         * (when loading) will assume all the characters constituting the key
         ...
         * examine the character preceding a line terminator sequence to decide
         * if the line terminator is escaped; there must be an odd number of
         * contiguous backslashes for the line terminator to be escaped. Since
         * the input is processed from left to right, a non-zero even number of
         * 2n contiguous backslashes before a line terminator (or elsewhere)
         * encodes n backslashes after escape processing. <p> The key contains
         * all of the characters in the line starting with the first non-white
         * space character and up to, but not including, the first unescaped
         * '=', ':', or white space character other than a line terminator. All
         * of these key termination characters may be included in the key by
         * escaping them with a preceding backslash character; for example,
         *
         *    \:\=
         *
         * would be the two-character key ":=". Line terminator characters can
         * be included using
         ...
         * performed as described above. <p> As an example, each of the
         * following three lines specifies the key "Truth" and the associated
         * element value "Beauty": <p> Truth = Beauty Truth:Beauty Truth :Beauty
         * <p>
         ...
         * leading white space on the continuation line are merely discarded and
         * are not replaced by one or more other characters. <p> As a third
         * example, the line:
         *
         *   cheeses
         *
         * specifies that the key is "cheeses" and the associated element is the
         * empty string "". <p> Characters in keys and elements can be
         * represented in escape sequences similar to those used for character
         * and string literals (see §3.3 and §3.10.6 of the Java Language
         * Specification). The differences from the character escape sequences
         * and Unicode escapes used for characters and strings are:
         *
         *   - Octal escapes are not recognized.
         *   - The character sequence \b does not represent a backspace
         * character.
         *   - The method does not treat a backslash character, \, before a
         * non-valid escape character as an error; the backslash is silently
         * dropped. For example, in a C++ string the sequence "\z" would cause a
         * compile time error. In contrast, this method silently drops the
         * backslash. Therefore, this method treats the two character sequence
         * "\b" as equivalent to the single character 'b'.
         *   - Escapes are not necessary for single and double quotes; however,
         ...
         * Properties table using the load(InputStream) method. <p> Properties
         * from the defaults table of this Properties table (if any) are not
         ...
         *   - Characters not in Latin-1 in the comments are written as \uxxxx
         * for their appropriate unicode hexadecimal value xxxx.
        ...
        /**
         * Writes this property list (key and element pairs) in this Properties
         * table to the output character stream in a format that can be read by
         * the load(Reader) method. <p> Properties from the defaults table of
         * this Properties table (if any) are not written out by this method.
         * <p>
         * If the comments argument is not empty, then an ASCII # character, the
         * comments string, and a line separator are first written to the output
         * stream. Thus, the comments can serve as an identifying comment. Any
         * one of a line feed ('\n'), a carriage return ('\r'), or a carriage
         * return followed immediately by a line feed in comments is replaced by
         * a line separator generated by the Writer and if the next character in
         * comments is not character # or character ! then an ASCII # is written
         * out after that line separator. <p> Next, a comment line is always
         * written, consisting of an ASCII # character, the current date and
         * time (as if produced by the toString method of Date for the current
         * time), and a line separator as generated by the Writer. <p> Then
         * every entry in this Properties table is written out, one per line.
         * For each entry the key string is written, then an ASCII =, then the
         * associated element string. For the key, all space characters are
         * written with a preceding \ character. For the element, leading space
         * characters, but not embedded or trailing space characters, are
         * written with a preceding \ character. The key and element characters
         * #, !, =, and : are written with a preceding backslash to ensure that
         * they are properly loaded. <p> After the entries have been written,
         * the output stream is flushed. The output stream remains open after
         * this method returns.
         *
         * @param writer
    ...
    private:
        void selectProperties(decaf::util::StlMap<std::string, std::string>&
                                  selectProperties) const;
    };
