...
namespace util
{
     ...
     * used to execute all of the timer's tasks, sequentially. Timer tasks
     * should complete quickly. If a timer task takes excessive time to
     * complete, it "hogs" the timer's task execution thread. This can, in turn,
     * delay the execution of subsequent tasks, which may "bunch up" and execute
     ...
     *
     * This class does not offer real-time guarantees: it schedules tasks using
    ...
    public:
        ...
        /**
         * Create a new Timer whose associated thread is assigned the name
         * given.
         *
         * @param name
         ...
         * timer has been terminated, its execution thread terminates
        ...
        /**
         * The caller will block until the Timer has completed termination
         * meaning all tasks that where scheduled before cancelation have now
         * completed and the executor is ready for deletion.  If the timeout
         * period elapses before the Timer reaches the terminated state then
         * this method return false to indicate it has not terminated.
         *
         * @param timeout
        ...
        /**
         * Removes all canceled tasks from this timer's task queue. Calling this
         * method has no effect on the behavior of the timer, but eliminates the
         * canceled tasks from the queue causing the Timer to destroy the
         * TimerTask pointer it was originally given, the caller should ensure
         * that they no longer have any references to TimerTasks that were
         * previously scheduled.
         *
         * Most programs will have no need to call this method. It is designed
         * for use by the rare application that cancels a large number of tasks.
         * Calling this method trades time for space: the runtime of the method
         * may be proportional to n + c log n, where n is the number of tasks in
         * the queue and c is the number of canceled tasks.
         *
         * This method can be called on a Timer object that has no scheduled
         * tasks without error.
         *
         ...
         *
         * The TimerTask pointer is considered to be owned by the Timer class
         * once it has been scheduled, the Timer will destroy its TimerTask's
         * once they have been canceled or the Timer itself is canceled. A
         * TimerTask is considered scheduled only when this method return
         * without throwing an exception, until that time ownership is not
         * considered to have been transferred to the Timer and the caller
         * should ensure that the TimerTask gets deleted if an exception is
         * thrown and no further attempts to schedule that TimerTask instance
         * are planned.
         *
         ...
         * tasks, such as blinking a cursor at regular intervals. It also
         * includes tasks wherein regular activity is performed in response to
         * human input, such as automatically repeating a character as long as a
         * key is held down.
         *
         * @param task - task to be scheduled.
         * @param delay - delay in milliseconds before task is to be executed.
    ...
    private:
        void scheduleTask(const decaf::lang::Pointer<TimerTask>& task,
                          long long                              delay,
                          long long                              period,
                          bool                                   fixed);
    };
