...
UUID::UUID(long long mostSigBits, long long leastSigBits)
    : uuid(new UUIDImpl(mostSigBits, leastSigBits))
{
}

////////////////////////////////////////////////////////////////////////////////
UUID::UUID(const UUID& source)
    : uuid(new UUIDImpl(source.getMostSignificantBits(),
...
{
    delete this->uuid;
    this->uuid = new UUIDImpl(source.getMostSignificantBits(),
                              source.getLeastSignificantBits());
    return *this;
}
...
{
    return StringUtils::compare(this->toString().c_str(),
...
UUID UUID::fromString(const std::string& uuid)
{
    ...
    long long m2 = Long::parseLong(
        uuid.substr(position[0] + 1, (position[1] - position[0]) - 1),
        16);
    long long m3 = Long::parseLong(
        uuid.substr(position[1] + 1, (position[2] - position[1]) - 1),
        16);

    long long lsb1 = Long::parseLong(
        uuid.substr(position[2] + 1, (position[3] - position[2]) - 1),
        16);
