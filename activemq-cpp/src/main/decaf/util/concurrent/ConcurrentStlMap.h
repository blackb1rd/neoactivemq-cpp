...
namespace util
{
    namespace concurrent
    {
        ...
        /**
         * Map template that wraps around a std::map to provide a more
         * user-friendly interface and to provide common functions that do not
         * exist in std::map.
         *
         * This version of Map extends the ConcurrentMap inteface and implements
         * all the methods defined in that interface.  Unlike a Java
         * ConcurrentHashMap this implementations synchronizes all methods such
         * that any call to this class will block if another thread is already
         * holding a lock, much like the Java HashTable.
         *
         * @since 1.0
         */
        ...
        private:
            ...
            public:
                EntryIterator(ConcurrentStlMap* parent)
                ...
                virtual MapEntry<K, V> next()
                {
                    ...
                    {
                        this->makeNext();
                        return MapEntry<K, V>(this->currentEntry->first,
                                              this->currentEntry->second);
                    }
            ...
            public:
                KeyIterator(ConcurrentStlMap* parent)
                    : AbstractMapIterator(parent)
                {
                }
            ...
            public:
                ValueIterator(ConcurrentStlMap* parent)
                    : AbstractMapIterator(parent)
                {
                }
        ...
        private:
            ...
            private:
                ...
                ConstAbstractMapIterator& operator=(
            ...
            public:
                ConstEntryIterator(const ConcurrentStlMap* parent)
            ...
            public:
                ConstKeyIterator(const ConcurrentStlMap* parent)
                    : ConstAbstractMapIterator(parent)
                {
                }
            ...
            public:
                ConstValueIterator(const ConcurrentStlMap* parent)
                    : ConstAbstractMapIterator(parent)
                {
                }
        ...
        private:
            ...
            public:
                StlMapEntrySet(ConcurrentStlMap* parent)
                    : AbstractSet<MapEntry<K, V>>(),
                      associatedMap(parent)
                {
                }
                ...
                virtual bool remove(const MapEntry<K, V>& entry)
                {
                    synchronized(&this->associatedMap->mutex)
                    {
                        if (this->associatedMap->containsKey(entry.getKey()) &&
                            this->associatedMap->get(entry.getKey()) ==
                                entry.getValue())
                        {
            ...
            public:
                ConstStlMapEntrySet(const ConcurrentStlMap* parent)
                    : AbstractSet<MapEntry<K, V>>(),
                      associatedMap(parent)
                {
                }
                ...
                virtual Iterator<MapEntry<K, V>>* iterator()
                {
                    throw activemq::exceptions::UnsupportedOperationException(
                        __FILE__,
                        __LINE__,
                        "Can't return a non-const iterator for a const "
                        "collection");
                }
        ...
        private:
            ...
            public:
                StlMapKeySet(ConcurrentStlMap* parent)
                    : AbstractSet<K>(),
                      associatedMap(parent)
                {
                }
            ...
            public:
                ConstStlMapKeySet(const ConcurrentStlMap* parent)
                    : AbstractSet<K>(),
                      associatedMap(parent)
                {
                }
        ...
        private:
            ...
            public:
                StlMapValueCollection(ConcurrentStlMap* parent)
            ...
            private:
                ...
                ConstStlMapValueCollection& operator=(
                    const ConstStlMapValueCollection&);

            public:
                ConstStlMapValueCollection(const ConcurrentStlMap* parent)
                    : AbstractCollection<V>(),
                      associatedMap(parent)
                {
                }
        ...
        private:
            // Cached values that are only initialized once a request for them
            // is made.
            ...
            // Cached values that are only initialized once a request for them
            // is made.
            mutable decaf::lang::Pointer<ConstStlMapEntrySet>
                                                            cachedConstEntrySet;
            mutable decaf::lang::Pointer<ConstStlMapKeySet> cachedConstKeySet;
            mutable decaf::lang::Pointer<ConstStlMapValueCollection>
        ...
        public:
            ...
            virtual bool equals(const Map<K, V>& source) const
            {
                synchronized(&mutex)
                {
                    typename std::unique_ptr<Iterator<K>> iterator(
            ...
            virtual void copy(const ConcurrentStlMap& source)
            {
                ...
                {
                    this->valueMap.clear();
                    this->valueMap.insert(source.valueMap.begin(),
                                          source.valueMap.end());
                }
            }
            ...
            virtual void putAll(const ConcurrentStlMap<K, V, COMPARATOR>& other)
            {
                synchronized(&mutex)
                {
                    this->valueMap.insert(other.valueMap.begin(),
                                          other.valueMap.end());
                    this->modCount++;
                }
            }
             ...
             * or throws an activemq::exceptions::InterruptedException
             * if there was no mapping for key.
             *
             * @throws activemq::exceptions::InterruptedException if
                ...
                synchronized(&mutex)
                {
                    if (this->cachedValueCollection == NULL)
                    {
                        this->cachedValueCollection.reset(
                            new StlMapValueCollection(this));
                    }
                }
                ...
                synchronized(&mutex)
                {
                    if (this->cachedConstValueCollection == NULL)
                    {
                        this->cachedConstValueCollection.reset(
                            new ConstStlMapValueCollection(this));
                    }
                }
