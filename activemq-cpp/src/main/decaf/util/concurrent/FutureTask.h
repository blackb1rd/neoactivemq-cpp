...
namespace util
{
    namespace concurrent
    {
         ...
         * has completed, the computation cannot be restarted or canceled.
         *
         ...
         *
         * In addition to serving as a stand-alone class, this class provides
        ...
        private:
            /**
             * A Callable subclass that runs given task and returns given
             * result, used to wrap either a Runnable or Callable pointer and
             */
            class FutureTaskAdapter
                : public decaf::util::concurrent::Callable<T>
            {
            private:
            ...
            public:
                FutureTaskAdapter(decaf::lang::Runnable* task,
                                  const T&               result,
                                  bool                   owns = true)
                ...
                FutureTaskAdapter(decaf::util::concurrent::Callable<T>* task,
                                  bool owns = true)
                    : decaf::util::concurrent::Callable<T>(),
                      task(NULL),
                      callable(task),
            ...
            private:
                 ...
                 * indicates that the results are accessible.
                 */
            ...
            public:
                    ...
                    {  // recheck after setting thread
                        ...
                        catch (...)
                        {
                            this->parent->setException(decaf::lang::Exception(
                                __FILE__,
                                __LINE__,
                                "FutureTask Caught Unknown exception during "
                                "task execution."));
                            return;
                        }
                ...
                bool innerRunAndReset()
                {
                    ...
                    {
                        this->parent->setException(
                            decaf::lang::Exception(new std::exception(stdex)));
                        return false;
                    }
                    catch (...)
                    {
                        this->parent->setException(decaf::lang::Exception(
                            __FILE__,
                            __LINE__,
                            "FutureTask Caught Unknown exception during task "
                            "execution."));
                        return false;
                    }
                }
        ...
        public:
            /**
             * Creates a FutureTask instance that will, upon running, execute
             * the given Callable.
             *
             * @param callable
                ...
                : sync(NULL)
            {
                ...
                this->sync.reset(new FutureTaskSync(
                    this,
             ...
             * Runnable, and arrange that the get method will return the given
        ...
        public:
             ...
             * to invoke completion callbacks or perform bookkeeping. Note that
             * you can query status inside the implementation of this method to
             * determine whether this task has been canceled.
             */
            virtual void done()
            {
            }
            ...
            /**
             * Sets the result of this Future to the given value unless this
             * future has already been set or has been cancelled.  This method
             * is invoked internally by the <tt>run</tt> method upon successful
             ...
             * Exception as its cause, unless this Future has already been set
             * or has been canceled.  This method is invoked internally by the
             * run method upon failure of the computation.
             *
             * @param error
             ...
             * designed for use with tasks that intrinsically execute more than
             * once.
             *
