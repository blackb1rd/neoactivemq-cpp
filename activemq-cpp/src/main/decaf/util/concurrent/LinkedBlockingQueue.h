...
namespace util
{
    namespace concurrent
    {
        ...
        private:
            ...
            public:
                TotalLock(const LinkedBlockingQueue<E>* parent)
                    : parent(parent)
                {
                    parent->putLock.lock();
        ...
        public:
            ...
            /**
             * Create a new instance with a Capacity of Integer::MAX_VALUE and
             * adds all the values contained in the specified collection to this
             * Queue.
             *
             * @param collection
            ...
            /**
             * Create a new instance with a Capacity of Integer::MAX_VALUE and
             * adds all the values contained in the specified
             * LinkedBlockingQueue to this Queue.
             *
             * @param queue
        ...
        public:
            LinkedBlockingQueue<E>& operator=(
                const LinkedBlockingQueue<E>& queue)
            {
                this->clear();
                this->addAll(queue);
                return *this;
        ...
        public:
            ...
            virtual void put(const E& value)
            {
                ...
                try
                {
                    ...
                    // not protected by lock. This works because count can only
                    // decrease at this point (all other puts are shut  out by
                    // lock), and we (or some other waiting put) are signaled if
                    // it ever changes from capacity. Similarly for all other
                    // uses of count in other wait guards.
                    while (this->count.get() == this->capacity)
                    {
                        this->notFull->await();
                    }

                    // This method now owns the putLock so we know we have at
                    // least enough capacity for one put, if we enqueue an item
                    // and there's still more room we should signal a waiting
                    // put to ensure that threads don't wait forever.
                    enqueue(value);
                ...
                // When c is zero it means we at least incremented once so there
                // was something in the Queue, another take could have already
                // happened but we don't know so wake up a waiting taker.
                if (c == 0)
                {
                    this->signalNotEmpty();
                }
            ...
            virtual E take()
            {
                ...
                try
                {
                    ...
                    // Since this methods owns the takeLock and count != 0 we
                    // know that its safe to take one element.  if c is greater
                    // than one then there is at least one more so we try to
                    // wake up another taker if any.
                    value = dequeue();
            ...
            virtual bool remove(const E& value)
            {
                ...
                for (Pointer<QueueNode<E>> predicessor = this->head,
                                           p           = predicessor->next;
                     p != NULL;
            ...
            virtual std::vector<E> toArray() const
            {
                ...
                for (Pointer<QueueNode<E>> p = this->head->next; p != NULL;
                     p                       = p->next)
                {
            ...
            virtual int drainTo(Collection<E>& sink, int maxElements)
            {
                ...
                try
                {
                    // We get the count of Nodes that exist now, any puts that
                    // are done after this are not drained and since we hold the
                    // lock nothing can get taken so state should remain
                    // consistent.
                    result =
                        decaf::lang::Math::min(maxElements, this->count.get());
                    ...
                    if (i > 0)
                    {
                        this->head = node;
                        signalNotFull =
                            (this->count.getAndAdd(-i) == this->capacity);
                    }
                }
                catch (decaf::lang::Exception& ex)
                {
        ...
        private:
            ...
            public:
                LinkedIterator(LinkedBlockingQueue<E>* parent)
                    : current(),
                      last(),
                      currentElement(),
                      parent(parent)
                ...
                virtual E next()
                {
                    ...
                    this->currentElement =
                        (this->current == NULL) ? E() : this->current->get();
                ...
                virtual void remove()
                {
                    ...
                    for (Pointer<QueueNode<E>> trail = this->parent->head,
                                               p     = trail->next;
                         p != NULL;
                         trail = p, p = p->next)
                    {
            ...
            private:
                Pointer<QueueNode<E>> nextNode(Pointer<QueueNode<E>>& p)
                {
                    ...
                    // Handle Nodes that have been removed from the interior of
                    // the Queue, these are tagged but still retain their next()
                    // value in order to account for multiple removes.  If all
                    // nodes were removed from the last call then eventually we
                    // reach next() == NULL which is the old tail.
            ...
            public:
                ConstLinkedIterator(const LinkedBlockingQueue<E>* parent)
                    : current(),
                      last(),
                      currentElement(),
                      parent(parent)
        ...
        private:
            void unlink(Pointer<QueueNode<E>>& p,
                        Pointer<QueueNode<E>>& predicessor)
            {
                // In order to prevent Iterators from losing their ability to
                // provide weakly consistent iteration the next value of p is
                // left intact but the node is marked as unlinked and it value
                // is reset to default.
                p->unlink();
