...
namespace util
{
    namespace concurrent
    {
         ...
         * acquire that item. Note that no synchronization lock is held when
         * acquire() is called as that would prevent an item from being returned
         * to the pool. The semaphore encapsulates the synchronization needed to
         * restrict access to the pool, separately from any synchronization
         * needed to maintain the consistency of the pool itself.
         *
         * A semaphore initialized to one, and which is used such that it only
         * has at most one permit available, can serve as a mutual exclusion
         * lock. This is more commonly known as a binary semaphore, because it
         * only has two states: one permit available, or zero permits available.
         ...
         * deadlock recovery.
         *
         * The constructor for this class optionally accepts a fairness
         ...
         * permitted, that is, a thread invoking acquire() can be allocated a
         * permit ahead of a thread that has been waiting - logically the new
         * thread places itself at the head of the queue of waiting threads.
         * When fairness is set true, the semaphore guarantees that threads
         * invoking any of the acquire methods are selected to obtain permits in
         * the order in which their invocation of those methods was processed
         * (first-in-first-out; FIFO). Note that FIFO ordering necessarily
         * applies to specific internal points of execution within these
         * methods. So, it is possible for one thread to invoke acquire before
         * another, but reach the ordering point after the other, and similarly
         * upon return from the method. Also note that the untimed tryAcquire
         * methods do not honor the fairness setting, but will take any permits
         * that are available.
         *
         ...
         * multiple permits at a time. Beware of the increased risk of
         * indefinitely postponing when these methods are used without fairness
         * set true.
         *
         * @since 1.0
         */
        ...
        public:
            ...
            /**
             * Creates a Semaphore with the given number of permits and the
             ...
             * If no permit is available then the current thread becomes
             * disabled for thread scheduling purposes and lies dormant until
             * some other thread invokes the release() method for this semaphore
             * and the current thread is next to be assigned a permit.
             *
             * If the current thread is interrupted while waiting for a permit
             * then it will continue to wait, but the time at which the thread
             * is assigned a permit may change compared to the time it would
             * have received the permit had no interruption occurred. When the
             ...
             *
             * Acquires a permit, if one is available and returns immediately,
             * with the value true, reducing the number of available permits by
             * one.
             *
             ...
             * policy, a call to tryAcquire() will immediately acquire a permit
             * if one is available, whether or not other threads are currently
             * waiting. This "barging" behavior can be useful in certain
             * circumstances, even though it breaks fairness. If you want to
             * honor the fairness setting, then use tryAcquire(0,
             ...
             * returned. If the time is less than or equal to zero, the method
             ...
             * have acquired that permit by calling acquire(). Correct usage of
             ...
             * returns immediately, reducing the number of available permits by
             * the given amount.
             *
             ...
             * semaphore, the current thread is next to be assigned permits and
             * the number of available permits satisfies this request; or
             *   * Some other thread interrupts the current thread.
             *
             * If the current thread:
             *
             ...
             *
             * If the current thread is interrupted while waiting for permits
             * then it will continue to wait and its position in the queue is
             * not affected. When the thread does return from this method its
             ...
             *
             * Acquires the given number of permits, if they are available, and
             * returns immediately, with the value true, reducing the number of
             * available permits by the given amount.
             *
             * If insufficient permits are available then this method will
             * return immediately with the value false and the number of
             * available permits is unchanged.
             *
             * Even when this semaphore has been set to use a fair ordering
             * policy, a call to tryAcquire will immediately acquire a permit if
             * one is available, whether or not other threads are currently
             * waiting. This "barging" behavior can be useful in certain
             * circumstances, even though it breaks fairness. If you want to
             * honor the fairness setting, then use tryAcquire(permits, 0,
             * TimeUnit.SECONDS) which is almost equivalent (it also detects
             * interruption).
             *
             * @param permits the number of permits to acquire
             ...
             * If the specified waiting time elapses then the value false is
             * returned. If the time is less than or equal to zero, the method
             * will not wait at all. Any permits that were to be assigned to
             * this thread, are instead assigned to other threads trying to
             * acquire permits, as if the permits had been made available by a
             * call to release().
             *
             * @param permits the number of permits to acquire
             * @param timeout the maximum amount of time to wait to acquire the
             * permits.
             ...
             * available permits by that amount. If any threads are trying to
             * acquire permits, then one is selected and given the permits that
             * were just released. If the number of available permits satisfies
             * that thread's request then that thread is (re)enabled for thread
             * scheduling purposes; otherwise the thread will wait until
             * sufficient permits are available. If there are still permits
             * available after this thread's request has been satisfied, then
             * those permits are assigned in turn to other threads trying to
             * acquire permits.
             *
            ...
            /**
             * Returns a string identifying this semaphore, as well as its
             * state. The state, in brackets, includes the String "Permits ="
             ...
             * currently waiting to acquire, this value changes dynamically so
             * the result of this method can be invalid immediately after it is
             * called.
             *
            ...
            /**
             * @return true if there are threads that are currently waiting to
        ...
        protected:
            /**
             * Reduces the number of available permits which can be useful for
             * subclasses.  If the subclass is tracking a resource that is
             * transiently available this method can be used to modify the
             * Semaphore to reflect that resources current state.  This method
             * does not block waiting for the number of permits to be available,
             * unlike the acquire method.
             *
             * @param reduceBy
            ...
            /**
             * Creates and returns a new Collection object that contains a best
             * effort snapshot of the threads that are currently waiting to
             * acquire.
             *
             * @return a Collection pointer that contains waiting threads for
             * lock acquisition. The caller owns the returned pointer.
             */
