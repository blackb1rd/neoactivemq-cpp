...
namespace util
{
    namespace concurrent
    {
         ...
         * not even a capacity of one.  You cannot <tt>peek</tt> at a
         * synchronous queue because an element is only present when you try to
         * remove it; you cannot insert an element (using any method) unless
         * another thread is trying to remove it; you cannot iterate as there is
         * nothing to iterate.  The <em>head</em> of the queue is the element
         * that the first queued inserting thread is trying to add to the queue;
         * if there is no such queued thread then no element is available for
         * removal and <tt>poll()</tt> will return <tt>null</tt>.  For purposes
         * of other <tt>Collection</tt> methods (for example <tt>contains</tt>),
         * a <tt>SynchronousQueue</tt> acts as an empty collection.  This queue
        ...
        public:
             ...
             * for another thread to receive it.
             *
             ...
             * necessary up to the specified wait time for another thread to
             * receive it.
             *
             ...
             * to insert it.
             *
             * @param result a reference to the value where the head of the
             * Queue should be copied to.
             * @param timeout the time that the method should block if there is
             * no element available to return.
             * @param unit the Time Units that the timeout value represents.
             *
             * @return true if the head of the Queue was copied to the result
             * param or false if no value could be returned.
             */
            virtual bool poll(E& result            DECAF_UNUSED,
                              long long timeout    DECAF_UNUSED,
                              const TimeUnit& unit DECAF_UNUSED)
            {
                // Object e = transferer.transfer(null, true,
