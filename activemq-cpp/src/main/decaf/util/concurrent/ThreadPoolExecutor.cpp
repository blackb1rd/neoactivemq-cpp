...
namespace util
{
    namespace concurrent
    {
        ...
        /**
         * Any task that we don't own we wrap in this Runnable object so that
         * the task deletion logic can remain unchanged and thread safe.
         */
        ...
        public:
            UnownedTaskWrapper(Runnable* task)
                : Runnable(),
                  task(task)
            {
            }
        ...
        private:
            /**
             * The worker class does a small amount of Bookkeeping and provides
             * a locking point for the kernel to access the running task.
             */
            ...
            public:
                ...
                void run()
                {
                    // Delegate the running of this task to the Kernel so that
                    // all the logic for task execution and cleanup is contained
                    // in one place.
            ...
            /**
             * TimerTask implementation used to clean up Worker objects that
             * have terminated for some reason.  Since they can't delete
             * themselves the cleanup is delegated to the Timer's thread.
             */
            ...
            public:
                WorkerKiller(ExecutorKernel* kernel)
                    : kernel(kernel)
                {
                }
                ...
                virtual void run()
                {
                    ...
                    try
                    {
                        Pointer<Iterator<Worker*>> iter(
                            toDeleteList.iterator());
                        while (iter->hasNext())
                        {
                            delete iter->next();
                            iter->remove();
        ...
        public:
            ...
            /**
             * List containing all worker threads in pool. Accessed only when
             * holding mainLock.
             */
            LinkedList<Worker*> workers;

            /**
             * List to hold Worker object that have terminated for some reason.
             * Usually this is because of a call to setMaximumPoolSize or
             * setCorePoolSize but can also occur because of an exception from a
             * task that the worker was running.
             */
            LinkedList<Worker*> deadWorkers;

            /**
             * Timer used to periodically clean up the dead worker objects. They
             * must be cleaned up on a separate thread because the Worker
             * generally adds itself to the deadWorkers list from the context of
             * its run method and cannot delete itself.
             */
            Timer cleanupTimer;
             ...
             * threads. We do not require that workQueue.poll() returning NULL
             * necessarily means that workQueue.isEmpty(), so rely solely on
             * isEmpty to see if the queue is empty (which we must do for
             * example when deciding whether to transition from SHUTDOWN to
             * TIDYING).  This accommodates special- purpose queues such as
             * DelayQueues for which poll() is allowed to return NULL even if it
             * may later return non-NULL when delays expire.
             */
            Pointer<BlockingQueue<decaf::lang::Runnable*>> workQueue;

            /**
             * Lock held on access to workers set and related bookkeeping. While
             * we could use a concurrent set of some sort, it turns out to be
             * generally preferable to use a lock. Among the reasons is that
             * this serializes interruptIdleWorkers, which avoids unnecessary
             * interrupt storms, especially during shutdown. Otherwise exiting
             * threads would concurrently interrupt those that have not yet
             * interrupted. It also simplifies some of the associated statistics
             * bookkeeping of largestPoolSize etc. We also hold mainLock on
             * shutdown and shutdownNow, for the sake of ensuring workers set is
             * stable while separately interrupting.
             */
        ...
        public:
            ...
            ~ExecutorKernel()
            {
                try
                {
                    // Turn off the cleanup timer first so that it doesn't fire
                    // while we transition all the remaining workers into the
                    // dead workers queue while can lead to lock contention. Its
                    // run method holds the mainLock so we need to wait for its
                    // release before moving on.
                    try
                    {
                        this->mainLock.lock();
                    ...
                    // not have run recently.
                    Pointer<Iterator<Worker*>> workers(
                        this->deadWorkers.iterator());
            ...
            /**
             * Force an interrupt of all threads even if they are currently
             * active.
             */
            ...
            /**
             * Interrupts threads that might be waiting for tasks (as indicated
             * by not being locked) so they can check for termination or
             * configuration changes.
             *
             * @param onlyOne
             *      If true, interrupt at most one worker. This is called only
             * from tryTerminate when termination is otherwise enabled but there
             * are still other workers.  In this case, at most one waiting
             * worker is interrupted to propagate shutdown signals in case all
             * threads are currently waiting.  Interrupting any arbitrary thread
             * ensures that newly arriving workers since shutdown began will
             * also eventually exit. To guarantee eventual termination, it
             * suffices to always interrupt only one idle worker, but shutdown()
             * interrupts all idle workers so that redundant workers exit
             * promptly, not waiting for a straggler task to finish.
             */
            ...
            /**
             * Ensures that unless the pool is stopping, the current thread does
             * not have its interrupt set. This requires a double-check of state
             * in case the interrupt was cleared concurrently with a shutdownNow
             * -- if so, the interrupt is re-enabled.
             */
            void clearInterruptsForTaskRun()
            {
                if (this->runStateLessThan(ctl.get(), STOP) &&
                    Thread::interrupted() &&
            ...
            /**
             * State check needed by ScheduledThreadPoolExecutor to enable
             * running tasks during shutdown.
             *
             * @param shutdownOK
                ...
                try
                {
                    while (task != NULL || (task = getTask()) != NULL)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                Exception ex(__FILE__,
                                             __LINE__,
                                             new std::exception(stdex),
                                             "Caught unknown exception while "
                                             "executing task.");
        ...
        private:
             ...
             * which poll or drainTo may fail to remove some elements, it
            ...
            {
                // Some Queue implementations can fail in poll and drainTo so we
                // check after attempting to drain the Queue and if its not
                // empty we remove the tasks one by one.
            ...
            /**
             * Checks if a new worker can be added with respect to current pool
             * state and the given bound (either core or maximum). If so, the
             * worker count is adjusted accordingly, and, if possible, a new
             * worker is created and started running firstTask as its first
             * task. This method returns false if the pool is stopped or
             ...
             * corePoolSize threads (in which case we always start one), or when
             * the queue is full (in which case we must bypass queue). Initially
             * idle threads are usually created via prestartCoreThread or to
            ...
            retry:
                for (;;)
                {
                    ...
                    for (;;)
                    {
                        ...
                        if (wc >= CAPACITY || wc >= (core ? this->corePoolSize
                                                          : this->maxPoolSize))
                        {
                            return false;
                        }
            ...
            success:
                ...
                try
                {
                    // Recheck while holding lock. Back out on ThreadFactory
                    // failure or if shut down before lock acquired.
                    int c  = ctl.get();
                    int rs = runStateOf(c);

                    if (t == NULL || (rs >= SHUTDOWN &&
                                      !(rs == SHUTDOWN && firstTask == NULL)))
                    {
                ...
                // which could result in a rare missed interrupt, because
                // Thread::interrupt is not guaranteed to have any effect on a
                // non-yet-started Thread (see Thread#interrupt).
             ...
             * from worker threads. Unless completedAbruptly is set, assumes
             * that workerCount has not already been adjusted to account for
             * exit.  This method removes thread from worker set, and possibly
             * terminates the pool or replaces the worker if either it exited
             * due to user task exception or if fewer than corePoolSize workers
             ...
             */
            void processWorkerExit(Worker*                w,
                                   bool completedAbruptly DECAF_UNUSED)
            {
                mainLock.lock();
                try
                {
                ...
                if (runStateLessThan(c, STOP))
                {
                    ...
                    {
                        int min = this->coreThreadsCanTimeout ? 0
                                                              : corePoolSize;
             ...
             * configuration settings, or returns NULL if this worker must exit
             * because of any of:
             *
            ...
            retry:
                for (;;)
                {
                    ...
                    for (;;)
                    {
                        ...
                        timed  = this->coreThreadsCanTimeout ||
      ...
      kernel(NULL)
{
    try
    {
        ...
        Pointer<RejectedExecutionHandler> handler(
            new ThreadPoolExecutor::AbortPolicy());
        Pointer<ThreadFactory> threadFactory(
            Executors::getDefaultThreadFactory());

        this->kernel = new ExecutorKernel(this,
                                          corePoolSize,
                                          maxPoolSize,
...
////////////////////////////////////////////////////////////////////////////////
bool ThreadPoolExecutor::awaitTermination(long long       timeout,
                                          const TimeUnit& unit)
{
    try
    {
...
////////////////////////////////////////////////////////////////////////////////
void ThreadPoolExecutor::setKeepAliveTime(long long       timeout,
                                          const TimeUnit& unit)
{
...
////////////////////////////////////////////////////////////////////////////////
void ThreadPoolExecutor::beforeExecute(Thread* thread DECAF_UNUSED,
                                       Runnable* task DECAF_UNUSED)
{
}
