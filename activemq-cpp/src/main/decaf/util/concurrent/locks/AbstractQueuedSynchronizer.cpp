...
public:
     ...
     *   SIGNAL:     The successor of this node is (or will soon be) blocked
     * (via park), so the current node must unpark its successor when  it
     * releases or cancels. To avoid races, acquire methods must first indicate
     * they need a signal, then retry the atomic acquire, and then, on failure,
     *               block.
     *   CANCELLED:  This node is canceled due to timeout or interrupt.  Nodes
     * never leave this state. In particular, a thread with canceled node never
     * again blocks. CONDITION:  This node is currently on a condition queue. It
     * will not be used as a sync queue node until transferred, at which time
     * the status will be set to 0. (Use of this value here has nothing to do
     * with the other uses of the field, but simplifies mechanics.) PROPAGATE:
     * A releaseShared should be propagated to other nodes. This is set (for
     * head node only) in doReleaseShared to ensure propagation continues, even
...
/**
 * When a thread no longer needs its Node in the AQS it is moved to the
 * NodePool.
 *
 * The Pool will create Node's on demand or access them from its internal pool
 * of old Nodes.  The Pool can at times shrink the list of old nodes if it needs
 * to.
 */
class NodePool
{
...
namespace util
{
    namespace concurrent
    {
        namespace locks
        {
            ...
            /**
             * The real implementation of AQS hidden here to allow code fixes
             * without altering ABI.
             */
            class SynchronizerState
            {
            public:
                /**
                 * The object that owns this one, allows this object to call
                 * back into its parent as well as checking for exclusive
                 * ownership of Conditions.
                 */
                AbstractQueuedSynchronizer* parent;

                /**
                 * The Sync state, subclasses can get / set this value to
                 * indicate when a Thread can acquire the lock or must wait.
                 */
                volatile int state;
                 ...
                 * via method enqueue() to add new wait node.
                 */
                AtomicReference<Node> tail;

                /**
                 * Pool used to store discarded Nodes for reuse by another
                 * thread.
                 */
                NodePool nodePool;
            ...
            public:
                ...
                /**
                 * Enqueue of a Node is Atomic with respect to the end of the
                 * list, if the list is empty so no lock needs to occur.  If the
                 * head and tail were previously set then we have to account for
                 * contention on the list by two or more enqueues.
                 *
                 * @param node
                 *      The new node to add.
                 *
                ...
                /**
                 * Since we can append itself in one atomic step we don't lock
                 * here.  If we can't get the fast append done we will enter
                 * into the longer looping enqueue method.
                 *
                 * @param mode
                ...
                {
                    Node* node =
                        nodePool.takeNode(Thread::currentThread(), mode);

                    // Try the fast add method first, then fall-back to the
                    // slower one.
                    Node* pred = tail.get();
                    if (pred != NULL)
                    {
                ...
                /**
                 * The only place head is altered, we NULL out prev since that
                 * Node will be Destroyed or pooled after this, but leave next
                 * alone since it should still be valid.  The method calling
                 * this needs to be the lock holder which ensures that no other
                 * thread can alter head.
                 *
                 * @param node
                ...
                void unparkSuccessor(Node* node)
                {
                    ...
                    // Thread to un-park is held in successor, which is normally
                    ...
                    {
                        successor = NULL;
                        for (Node* t = tail.get(); t != NULL && t != node;
                             t       = t->prev)
                        {
                            if (t->waitStatus <= 0)
                            {
                ...
                void doReleaseShared()
                {
                    ...
                    // in-progress acquires/releases.  This proceeds in the
                    // usual way of trying to unparkSuccessor of head if it
                    // needs signal. But if it does not, status is set to
                    // PROPAGATE to ensure that upon release, propagation
                    // continues. Additionally, we must loop in case a new node
                    // is added while we are doing this. Also, unlike other uses
                    // of unparkSuccessor, we need to know if CAS to reset
                ...
                /**
                 * Sets head of queue, and checks if successor may be waiting in
                 * shared mode, if so propagating if either propagate > 0 or
                ...
                {
                    Node* head =
                        setHead(node);  // Record old head for check below

                    // Try to signal next queued node if:
                    //   Propagation was indicated by caller, or was recorded
                    //   (as head->waitStatus) by a previous operation (note:
                    //   this uses sign-check of waitStatus because PROPAGATE
                    //   status may transition to SIGNAL.) and the next node is
                    //   waiting in shared mode, or we don't know, because it
                    //   appears NULL.
                    //
                    // The conservatism in both of these checks may cause
                    // unnecessary wake-ups, but only when there are multiple
                    // racing acquires/releases, so most need signals now or
                    // soon anyway.
                ...
                /**
                 * Cancels an ongoing attempt to acquire.  A canceled node can
                 * be returned to the pool once its status has been updated and
                 * its links are updated.
                 *
                 * @param node
                 *      The node that was attempting to acquire, will be
                ...
                void cancelAcquire(Node* node)
                {
                    ...
                    // predNext is the apparent node to unsplice. CASes below
                    // will fail if not, in which case, we lost race vs another
                    // cancel or signal, so no further action is necessary.
                    ...
                    {
                        // If successor needs signal, try to set pred's
                        // next-link so it will get one. Otherwise wake it up to
                        // propagate.
                        int ws;
                        if (pred != head.get() &&
                    ...
                    node->next = NULL;  // Help any lingering threads that land
                                        // on this node
                 ...
                 * control in all acquire loops. Requires that pred ==
                 * node->prev
                 *
                 * @param pred
                ...
                bool shouldParkAfterFailedAcquire(Node* pred, Node* node)
                {
                    ...
                    if (ws == Node::SIGNAL)
                    {
                        // This node has already set status asking a release to
                        // signal it, so it can safely park.
                        return true;
                    }
                    ...
                    else
                    {
                        ...
                        // need a signal, but don't park yet.  Caller will need
                        // to retry to make sure it cannot acquire before
                        // parking.
                ...
                /**
                 * Acquires in exclusive uninterruptible mode for thread already
                 * in queue Used by condition wait methods as well as acquire.
                 * We can access the Node's data here as it can't be altered on
                 * us because it is guaranteed to exist until this method
                 * returns.
                 *
                 * @param node
                 *      The node.
                ...
                bool doAcquireNanos(int arg, long long nanosTimeout)
                {
                    ...
                    try
                    {
                        for (;;)
                        {
                            ...
                            if (shouldParkAfterFailedAcquire(p, node) &&
                                nanosTimeout > spinForTimeoutLimit)
                            {
                ...
                Thread* fullGetFirstQueuedThread()
                {
                    ...
                    // have been unset after setHead. So we must check to see if
                    // tail is actually first node. If not, we continue on,
                    // safely traversing from tail back to head to find first,
                ...
                /**
                 * Returns true if a node, always one that was initially placed
                 * on a condition queue, is now waiting to reacquire on sync
                 * queue.
                 *
                 * @param node the node
                 *
                ...
                /**
                 * Returns true if node is on sync queue by searching backwards
                 * from tail. Called only when needed by isOnSyncQueue.
                 *
                ...
                {
                    // If we cannot change waitStatus, the node has been
                    // canceled.
                    ...
                    // Get the Node onto the list, once there we need to update
                    // its predecessor to indicate it should signal this Node
                    // once it is woken up either by cancel or by acquiring the
                    // lock.
                    enqueue(node);

                    // Splice onto queue and try to set waitStatus of
                    // predecessor to indicate that thread is (probably)
                    // waiting. If canceled or attempt to set waitStatus fails,
                    // wake up to resync (in which case the waitStatus can be
                    // transiently and harmlessly wrong).
                    Node* p  = node->prev;
                    int   ws = p->waitStatus;
                ...
                /**
                 * Transfers node, if necessary, to sync queue after a canceled
                 * wait. Returns true if thread was canceled before being
                 * signaled.  If the Node is a Condition and has been signaled
                ...
                /**
                 * Invokes release with current state value; returns saved
                 * state.  Cancels node and throws exception on failure.  When a
                 * monitor state exception is thrown the Node is added to the
                 * sync queue so that it can be deleted safely later.
                 *
                 * @param node
                ...
                static bool compareAndSetWaitStatus(Node* node,
                                                    int   expect,
                                                    int   update)
                {
                    return Atomics::compareAndSet32(&node->waitStatus,
                                                    expect,
                                                    update);
                }

                static bool compareAndSetNext(Node* node,
                                              Node* expect,
                                              Node* update)
                {
                    return Atomics::compareAndSet(
                        (volatile void**)(&node->next),
                        (void*)expect,
                        (void*)update);
                }
            };
            ...
            /**
             * Provides a default implementation that most
             * AbstractQueuedSynchronizer derived classes can use without
             * needing to write one from scratch.  Since the methods of this
             * Object must always be called from a locked synchronizer we can
             * assume that only one thread at a time will read or modify the
             * list of waiters so we don't need to lock around modifications to
             * the list.
             *
             * This object creates Node instance but leaves the deletion of
             * those objects up to the sync queue.  In all cases the Node needs
             * to be transfered to the sync queue with its state value set to
             * zero or CANCELLED.
             */
            class DefaultConditionObject
            ...
            public:
                ...
                virtual void await()
                {
                    ...
                    if (impl->acquireQueued(node, savedState) &&
                ...
                /**
                 * The Node that's been waiting the longest is moved from this
                 * Conditions wait queue to that of the parent Synchronizer.
                 */
                virtual void signal()
                {
                ...
                virtual bool isOwnedBy(
                    const AbstractQueuedSynchronizer* sync) const
                {
                ...
                virtual Collection<decaf::lang::Thread*>* getWaitingThreads()
                    const
                {
                    if (!impl->isHeldExclusively())
                    {
            ...
            private:
                /**
                 * Adds a new Node to this Condition object to be used by one of
                 * the wait methods. During this addition we scan for canceled
                 * Nodes and remove them as they are now contained on the sync
                 * queue and will be removed from there.  Its safe to do this
                 * here as this method is called while the sync queue is locked
                 * so no other changes can occur to the list of Nodes.
                 *
                ...
                Node* addConditionWaiter()
                {
                    ...
                    Node* node =
                        impl->nodePool.takeNode(Thread::currentThread(),
                                                Node::CONDITION);
                    if (t == NULL)
                    {
                        head = node;
                ...
                /**
                 * Removes and transfers nodes until hit non-canceled one or a
                 * NULL one. or stated another way traverses the list of waiters
                 * removing canceled nodes until it finds a non-canceled one to
                 * signal.
                 *
                ...
                void doSignal(Node* first)
                {
                    ...
                    } while (!impl->transferForSignal(first) &&
                             (first = head) != NULL);
                }

                /**
                 * Removes and transfers all nodes.  Removes all Nodes from this
                 * Condition object starting from the given node pointer,
                 * canceled waiter Nodes are moved to so we don't need to track
                 * them anymore.  This is safe to do here since this method must
                 * be called while the sync queue lock is held.
                 *
                 * @param first (non-NULL)
                 *      The first node on condition queue to start transferring
                 * from.
                 */
                ...
                /**
                 * Unlinks canceled waiter nodes from condition queue.  Called
                 * only while holding lock. This is called when cancellation
                 * occurred during condition wait, and upon insertion of a new
                 * waiter when tail is seen to have been canceled. This method
                 * is needed to avoid garbage retention in the absence of
                 * signals. So even though it may require a full traversal, it
                 * comes into play only when timeouts or cancellations occur in
                 * the absence of signals. It traverses all nodes rather than
                 * stopping at a particular target to unlink all pointers to
                 ...
                 * before signaled, REINTERRUPT if after signaled, or 0 if not
                 * interrupted.  The canceled node is transfered to the sync
                 * queue so that its waiter can safely re-acquire the lock.
                 *
                 * @param node
                 *      The node to transfer to the sync queue if interrupted.
                 *
                 * @returns value indicating if an action is needed in response
                 * to an interrupt.
                 */
...
std::string AbstractQueuedSynchronizer::toString() const
{
    ...
    return prefix + "[State = " + Integer::toString(s) + ", " + q +
           "empty queue]";
}
...
{
    if (!tryAcquire(arg) &&
        this->impl->acquireQueued(this->impl->addWaiter(Node::EXCLUSIVE), arg))
    {
...
////////////////////////////////////////////////////////////////////////////////
bool AbstractQueuedSynchronizer::tryAcquireNanos(int       arg,
...
////////////////////////////////////////////////////////////////////////////////
bool AbstractQueuedSynchronizer::tryAcquireSharedNanos(int       arg,
                                                       long long nanosTimeout)
{
    if (Thread::interrupted())
    {
    ...
    return tryAcquireShared(arg) >= 0 ||
           this->impl->doAcquireSharedNanos(arg, nanosTimeout);
}
...
Thread* AbstractQueuedSynchronizer::getFirstQueuedThread() const
{
    ...
    return (this->impl->head.get() == this->impl->tail.get())
               ? NULL
               : this->impl->fullGetFirstQueuedThread();
}
...
////////////////////////////////////////////////////////////////////////////////
bool AbstractQueuedSynchronizer::hasWaiters(
    const ConditionObject* condition) const
{
    if (!owns(condition))
    {
...
////////////////////////////////////////////////////////////////////////////////
int AbstractQueuedSynchronizer::getWaitQueueLength(
    const ConditionObject* condition) const
{
    if (!owns(condition))
    {
...
////////////////////////////////////////////////////////////////////////////////
Collection<Thread*>* AbstractQueuedSynchronizer::getWaitingThreads(
    const ConditionObject* condition) const
{
    if (!owns(condition))
    {
...
AbstractQueuedSynchronizer::createDefaultConditionObject()
{
...
bool AbstractQueuedSynchronizer::hasQueuedPredecessors() const
{
    ...
    Node* t =
        this->impl->tail.get();  // Read fields in reverse initialization order
    Node* h = this->impl->head.get();
    Node* s = NULL;
