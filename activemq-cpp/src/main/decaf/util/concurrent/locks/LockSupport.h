...
namespace util
{
    namespace concurrent
    {
        namespace locks
        {
             ...
             * immediately if the permit is available, consuming it in the
             * process; otherwise it may block. A call to unpark makes the
             * permit available, if it was not already available. (Unlike with
             * Semaphores though, permits do not accumulate. There is at most
             * one.)
             *
             * Methods park and unpark provide efficient means of blocking and
             * unblocking threads Races between one thread invoking park and
             * another thread trying to unpark it will preserve liveness, due to
             * the permit. Additionally, park will return if the caller's thread
             * was interrupted, and timeout versions are supported. The park
             * method may also return at any other time, for "no reason", so in
             * general must be invoked within a loop that rechecks conditions
             * upon return. In this sense park serves as an optimization of a
             * "busy wait" that does not waste as much time spinning, but must
             * be paired with an unpark to be effective.
             *
             * These methods are designed to be used as tools for creating
             * higher-level synchronization utilities, and are not in themselves
             * useful for most concurrency control applications. The park method
             * is designed for use only in constructions of the form:
             *
             *   while (!canProceed()) { ... LockSupport.park(this); }
             *
             * where neither canProceed nor any other actions prior to the call
             * to park entail locking or blocking. Because only one permit is
             * associated with each thread, any intermediary uses of park could
             ...
             *            if( Thread::interrupted() ) // ignore interrupts while
             * waiting wasInterrupted = true;
             *            }
             *
             *            waiters.remove();
             *            if( wasInterrupted )          // reassert interrupt
             * status on exit current.interrupt();
             *      }
             *
             *      void unlock() {
             *         locked.set( false );
            ...
            public:
                 ...
                 * not already available. If the thread was blocked on park then
                 * it will unblock. Otherwise, its next call to park is
                 * guaranteed not to block. This operation is not guaranteed to
                 * have any effect at all if the given thread has not been
                 * started.
                 *
                 * @param thread the thread to unport, or NULL in which case the
                ...
                /**
                 * Disables the current thread for thread scheduling purposes,
                 * until the specified deadline, unless the permit is available.
                 *
                 * If the permit is available then it is consumed and the call
                 * returns immediately; otherwise the current thread becomes
                 * disabled for thread scheduling purposes and lies dormant
