...
namespace util
{
    namespace logging
    {
         ...
         * application component. Loggers are normally named, using a
         * hierarchical dot-separated namespace. Logger names can be arbitrary
         * strings, but they should normally be based on the namespace or class
         * name of the logged component, such as decaf.net or org.apache.decaf.
         * In addition it is possible to create "anonymous" Loggers that are not
         * stored in the Logger namespace.
         *
         * Logger objects may be obtained by calls on one of the getLogger
         * factory methods. These will either create a new Logger or return a
         * suitable existing Logger.
         *
         * Logging messages will be forwarded to registered Handler objects,
         * which can forward the messages to a variety of destinations,
         * including consoles, files, OS logs, etc.
         *
         * Each Logger keeps track of a "parent" Logger, which is its nearest
         * existing ancestor in the Logger namespace.
         *
         * Each Logger has a "Level" associated with it. This reflects a minimum
         * Level that this logger cares about. If a Logger's level is set to
         * Level::INHERIT, then its effective level is inherited from its
         * parent, which may in turn obtain it recursively from its parent, and
         * so on up the tree.
         *
         * The log level can be configured based on the properties from the
         * logging configuration file, as described in the description of the
         * LogManager class. However it may also be dynamically changed by calls
         * on the Logger.setLevel method. If a logger's level is changed the
         * change may also affect child loggers, since any child logger that has
         * 'inherit' as its level will inherit its effective level from its
         * parent.
         *
         * On each logging call the Logger initially performs a cheap check of
         * the request level (e.g. SEVERE or FINE) against the effective log
         * level of the logger. If the request level is lower than the log
         * level, the logging call returns immediately.
         *
         * After passing this initial (cheap) test, the Logger will allocate a
         * LogRecord to describe the logging message. It will then call a Filter
         * (if present) to do a more detailed check on whether the record should
         * be published. If that passes it will then publish the LogRecord to
         * its output Handlers. By default, loggers also publish to their
         * parent's Handlers, recursively up the tree.
         *
        ...
        protected:
             ...
             * the subsystem, such as decaf.net or org.apache.decaf.  It may be
             * empty for anonymous Loggers.
             */
        ...
        public:
             ...
             * Logger in this Loggers namespace.
             *
             ...
             *
             * By default, Loggers also send their output to their parent
             * logger. Typically the root Logger is configured with a set of
             * Handlers that essentially act as default handlers for all
             * loggers.
             *
             * The ownership of the given Handler is passed to the Logger and
             * the Handler will be deleted when this Logger is destroyed unless
             * the caller first calls removeHandler with the same pointer value
             * as was originally given.
             *
        ...
        public:  // Logging Methods.
             ...
             * log record with the given information is logged at the
             * Level::FINER log level.
             *
             * @param blockName
             ...
             *
             * This is a convenience method to log that a method is terminating
             * by throwing an exception. The logging is done using the FINER
             * level.
             *
             ...
             * forwarded to all registered output handlers. The LogRecord's
             * message is set to "THROW".
             *
             * @param file
             *      The file name where the log was generated.
             * @param line
             ...
             * @return true if the given message level is currently being
             * logged.
             */
