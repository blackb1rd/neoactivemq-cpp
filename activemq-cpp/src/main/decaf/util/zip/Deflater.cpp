...
namespace util
{
    namespace zip
    {
        ...
        public:
            static void initZLibDeflate(DeflaterData* handle,
                                        int           level,
                                        bool          nowrap = false)
            {
                if (handle == NULL)
...
////////////////////////////////////////////////////////////////////////////////
void Deflater::setInput(const unsigned char* buffer,
                        int                  size,
                        int                  offset,
                        int                  length)
{
    try
    {
        ...
        // We can only change the level and strategy once an entire block of
        // data is compressed.
        if (this->data->stream->next_in == NULL)
        {
            deflateParams(this->data->stream,
                          this->data->level,
                          this->data->strategy);
        }
...
////////////////////////////////////////////////////////////////////////////////
void Deflater::setDictionary(const unsigned char* buffer,
                             int                  size,
                             int                  offset,
                             int                  length)
{
    try
    {
        ...
        // stream or if the compression method is bsort). deflateSetDictionary
        // does not perform any compression: this will be done by deflate().
        if (deflateSetDictionary(this->data->stream,
                                 buffer + offset,
                                 (uInt)length) != Z_OK)
        {
...
int Deflater::deflate(unsigned char* buffer, int size, int offset, int length)
{
    try
    {
        ...
        // Call ZLib and then process the resulting data to figure out what
        // happened.
