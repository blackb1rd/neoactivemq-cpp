...
namespace test
{
    namespace openwire
    {
        ...
        /**
         * High-volume system tests for message listeners with multiple
         * connections.
         *
        ...
        public:
            ...
            /**
             * Test high-volume message delivery (10k messages) on failover
             * connection
             */
            void testHighVolumeFailoverListener();

            /**
             * Test high-volume message delivery (10k messages) on direct
             * connection to broker3
             */
            void testHighVolumeDirectListener();

            /**
             * Test high-volume message delivery on both connections
             * simultaneously:
            ...
            /**
             * Test message delivery stability when broker is briefly
             * interrupted during high-volume message flow
             */
            void testHighVolumeWithBrokerInterruption();

            /**
             * Test durable topic consumer with SESSION_TRANSACTED (commit per
             * message), persistent delivery mode, concurrent consumption from 2
             * servers with different topics (~20k messages)
             */
        ...
        protected:
            ...
            // Timeout for durable topic transacted test (longer due to
            // per-message commit)
            static constexpr int DURABLE_TOPIC_TIMEOUT_MS =
                300000;  // 5 minutes
            ...
            // Timeout for multi-topic test (longer due to large messages and
            // per-message commit)
