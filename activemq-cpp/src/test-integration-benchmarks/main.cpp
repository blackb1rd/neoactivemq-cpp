...
int main(int argc, char** argv)
{
    ...
    long long testTimeoutSeconds = 600;  // Per-test timeout: 10 minutes default
                                         // for integration benchmark tests
    ...
    {
        watchdog.reset(
            new test::util::TestWatchdog(testTimeoutSeconds,
                                         true,
    ...
    catch (...)
    {
        ...
        std::cout << "- Do you have all Brokers Running?     -" << std::endl;
        std::cout << "- REQUIRES: docker compose --profile failover up"
                  << std::endl;
        std::cout << "----------------------------------------" << std::endl;
