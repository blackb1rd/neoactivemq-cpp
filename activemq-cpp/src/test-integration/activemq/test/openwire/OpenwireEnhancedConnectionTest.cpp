...
public:
    ...
    virtual void onDestinationEvent(cms::DestinationEvent* event)
    {
        cms::Destination::DestinationType type =
            event->getDestination()->getDestinationType();
        switch (type)
        {
...
TEST_F(OpenwireEnhancedConnectionTest, testDestinationSource)
{
    ...
    ActiveMQConnection* amq =
        dynamic_cast<ActiveMQConnection*>(connection.get());
    ASSERT_TRUE(amq != NULL);

    cms::EnhancedConnection* enhanced =
        dynamic_cast<cms::EnhancedConnection*>(connection.get());
    ASSERT_TRUE(enhanced != NULL);

    std::unique_ptr<cms::DestinationSource> source(
        enhanced->getDestinationSource());
...
TEST_F(OpenwireEnhancedConnectionTest, testDestinationSourceGetters)
{
    ...
    std::unique_ptr<Destination> destination1(
        session->createTopic(UUID::randomUUID().toString()));
    std::unique_ptr<MessageConsumer> consumer1(
        session->createConsumer(destination1.get()));
    std::unique_ptr<Destination> destination2(
        session->createQueue(UUID::randomUUID().toString()));
    std::unique_ptr<MessageConsumer> consumer2(
        session->createConsumer(destination2.get()));
    ...
    ASSERT_EQ(currentQueueCount + 1, listener.queueCount)
        << ("Should be one Queue");
    ASSERT_TRUE(listener.topicCount > currentTopicCount)
        << ("Should be at least Topic");
    ASSERT_EQ(currentTempQueueCount + 1, listener.tempQueueCount)
        << ("Should be one temp Queue");
    ASSERT_EQ(currentTempTopicCount + 1, listener.tempTopicCount)
        << ("Should be one temp Topic");
    ...
    ASSERT_EQ(currentQueueCount, listener.queueCount)
        << ("Should be no Queues created by this test");
    ...
    ASSERT_EQ(currentTempQueueCount + 1, listener.tempQueueCount)
        << ("Temp Queue Counts shouldn't change");
    ASSERT_EQ(currentTempTopicCount + 1, listener.tempTopicCount)
        << ("Temp Topic Counts shouldn't change");
    ...
    ASSERT_EQ(currentTempQueueCount + 3, listener.tempQueueCount)
        << ("Should be three total temp Queues from this test");
    ASSERT_EQ(currentTempTopicCount + 3, listener.tempTopicCount)
        << ("Should be three total temp Topics from this test");
