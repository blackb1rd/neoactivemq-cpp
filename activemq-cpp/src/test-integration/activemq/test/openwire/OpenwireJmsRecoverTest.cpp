...
void OpenwireJmsRecoverTest::doTestSynchRecover()
{
    ...
    producer->send(
        std::unique_ptr<cms::Message>(session->createTextMessage("First"))
            .get());
    producer->send(
        std::unique_ptr<cms::Message>(session->createTextMessage("Second"))
            .get());

    std::unique_ptr<TextMessage> message(
        dynamic_cast<TextMessage*>(consumer->receive(2000)));
...
public:
    ClientAckMessageListener(cms::Session*        session,
                             std::vector<string>* errorMessages,
                             CountDownLatch*      doneCountDownLatch)
        : session(session),
          errorMessages(errorMessages),
          doneCountDownLatch(doneCountDownLatch),
          counter(0)
    {
    }
    ...
    virtual void onMessage(const cms::Message* msg)
    {
        ...
        try
        {
            ...
            switch (counter)
            {
                ...
                case 3:
                    ...
                    break;
                default:
                    errorMessages->push_back(string("Got too many messages: ") +
        ...
        {
            errorMessages->push_back(string("Got exception: ") +
                                     e.getMessage());
            doneCountDownLatch->countDown();
        }
    }
...
void OpenwireJmsRecoverTest::doTestAsynchRecover()
{
    ...
    ClientAckMessageListener listener(session.get(),
                                      &errorMessages,
                                      &doneCountDownLatch);
...
public:
    AutoAckMessageListener(cms::Session*        session,
                           std::vector<string>* errorMessages,
                           CountDownLatch*      doneCountDownLatch)
        : session(session),
          errorMessages(errorMessages),
...
void OpenwireJmsRecoverTest::doTestAsynchRecoverWithAutoAck()
{
    ...
    AutoAckMessageListener listener(session.get(),
                                    &errorMessages,
                                    &doneCountDownLatch);
    consumer->setMessageListener(&listener);
...
TEST_F(OpenwireJmsRecoverTest, testQueueSynchRecover)
{
    destination = new ActiveMQQueue(
        string("Queue-") + Long::toString(System::currentTimeMillis()));
    doTestSynchRecover();
}
...
{
    destination = new ActiveMQTopic(
        string("Topic-") + Long::toString(System::currentTimeMillis()));
    doTestSynchRecover();
}
