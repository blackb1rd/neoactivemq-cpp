...
namespace test
{
    namespace openwire_ssl
    {
        ...
        class OpenwireSslXATransactionsTest : public ::testing::Test
        {
        protected:
...
////////////////////////////////////////////////////////////////////////////////
const int OpenwireSslXATransactionsTest::batchCount = 10;
const int OpenwireSslXATransactionsTest::batchSize  = 20;

////////////////////////////////////////////////////////////////////////////////
cms::Xid* OpenwireSslXATransactionsTest::createXid() const
{
    std::string branchQualStr = UUID::randomUUID().toString();
    std::string globalTxIdStr = this->txIdGen.generateId();

    std::vector<unsigned char> branchQual(branchQualStr.begin(),
                                          branchQualStr.end());
    std::vector<unsigned char> globalTxId(globalTxIdStr.begin(),
                                          globalTxIdStr.end());
...
////////////////////////////////////////////////////////////////////////////////
TEST_F(OpenwireSslXATransactionsTest, testCreateXAConnectionFactory)
{
    ...
    ConnectionFactory* cmsFactory =
        dynamic_cast<ConnectionFactory*>(factory.get());
    ASSERT_TRUE(cmsFactory != NULL);

    ActiveMQConnectionFactory* amqFactory =
        dynamic_cast<ActiveMQConnectionFactory*>(factory.get());
    ASSERT_TRUE(amqFactory != NULL);

    ActiveMQXAConnectionFactory* amqXAFactory =
        dynamic_cast<ActiveMQXAConnectionFactory*>(factory.get());
    ASSERT_TRUE(amqXAFactory != NULL);
}

////////////////////////////////////////////////////////////////////////////////
TEST_F(OpenwireSslXATransactionsTest, testCreateXAConnection)
{
    std::unique_ptr<XAConnectionFactory> factory(
        XAConnectionFactory::createCMSXAConnectionFactory(getBrokerURL()));
    ASSERT_TRUE(factory.get() != NULL);
    ...
    ActiveMQXAConnection* amqXAConnection =
        dynamic_cast<ActiveMQXAConnection*>(connection.get());
    ASSERT_TRUE(amqXAConnection != NULL);
}

////////////////////////////////////////////////////////////////////////////////
TEST_F(OpenwireSslXATransactionsTest, testCreateXASession)
{
    std::unique_ptr<XAConnectionFactory> factory(
        XAConnectionFactory::createCMSXAConnectionFactory(getBrokerURL()));
    ASSERT_TRUE(factory.get() != NULL);
    ...
    ActiveMQXASession* amqXASession =
        dynamic_cast<ActiveMQXASession*>(session.get());
    ASSERT_TRUE(amqXASession != NULL);
}

////////////////////////////////////////////////////////////////////////////////
TEST_F(OpenwireSslXATransactionsTest, testGetXAResource)
{
    std::unique_ptr<XAConnectionFactory> factory(
        XAConnectionFactory::createCMSXAConnectionFactory(getBrokerURL()));
    ASSERT_TRUE(factory.get() != NULL);
...
////////////////////////////////////////////////////////////////////////////////
TEST_F(OpenwireSslXATransactionsTest, testSendReceiveOutsideTX)
{
    std::unique_ptr<XAConnectionFactory> factory(
        XAConnectionFactory::createCMSXAConnectionFactory(getBrokerURL()));
    ASSERT_TRUE(factory.get() != NULL);
...
////////////////////////////////////////////////////////////////////////////////
TEST_F(OpenwireSslXATransactionsTest, testSendReceiveTransactedBatches)
{
    std::unique_ptr<XAConnectionFactory> factory(
        XAConnectionFactory::createCMSXAConnectionFactory(getBrokerURL()));
    ASSERT_TRUE(factory.get() != NULL);
    ...
    for (int j = 0; j < batchCount; j++)
    {
        ...
        ASSERT_NO_THROW(xaResource->end(txIdSend.get(), XAResource::TMSUCCESS))
            << ("Should not have thrown an Exception for xaResource->end");
        ASSERT_NO_THROW(xaResource->prepare(txIdSend.get()))
            << ("Should not have thrown an Exception for xaResource->prepare");
        ASSERT_NO_THROW(xaResource->commit(txIdSend.get(), false))
        ...
        ASSERT_NO_THROW(xaResource->end(txIdRecv.get(), XAResource::TMSUCCESS))
            << ("Should not have thrown an Exception for xaResource->end");
        ASSERT_NO_THROW(xaResource->prepare(txIdRecv.get()))
            << ("Should not have thrown an Exception for xaResource->prepare");
        ASSERT_NO_THROW(xaResource->commit(txIdRecv.get(), false))
            << ("Should not have thrown an Exception for xaResource->commit");
    }
    ...
    ASSERT_TRUE(message.get() == NULL)
        << ("Unexpected Message Received after XA Batches all processed");
}

////////////////////////////////////////////////////////////////////////////////
TEST_F(OpenwireSslXATransactionsTest, testSendRollback)
{
    std::unique_ptr<XAConnectionFactory> factory(
        XAConnectionFactory::createCMSXAConnectionFactory(getBrokerURL()));
    ASSERT_TRUE(factory.get() != NULL);
...
////////////////////////////////////////////////////////////////////////////////
TEST_F(OpenwireSslXATransactionsTest, testSendRollbackCommitRollback)
{
    std::unique_ptr<XAConnectionFactory> factory(
        XAConnectionFactory::createCMSXAConnectionFactory(getBrokerURL()));
    ASSERT_TRUE(factory.get() != NULL);
...
////////////////////////////////////////////////////////////////////////////////
TEST_F(OpenwireSslXATransactionsTest, testWithTTLSet)
{
    std::unique_ptr<XAConnectionFactory> factory(
        XAConnectionFactory::createCMSXAConnectionFactory(getBrokerURL()));
    ASSERT_TRUE(factory.get() != NULL);
...
////////////////////////////////////////////////////////////////////////////////
TEST_F(OpenwireSslXATransactionsTest, testXAResource_Exception1)
{
    std::unique_ptr<XAConnectionFactory> factory(
        XAConnectionFactory::createCMSXAConnectionFactory(getBrokerURL()));
    ASSERT_TRUE(factory.get() != NULL);
...
////////////////////////////////////////////////////////////////////////////////
TEST_F(OpenwireSslXATransactionsTest, testXAResource_Exception2)
{
    std::unique_ptr<XAConnectionFactory> factory(
        XAConnectionFactory::createCMSXAConnectionFactory(getBrokerURL()));
    ASSERT_TRUE(factory.get() != NULL);
...
////////////////////////////////////////////////////////////////////////////////
TEST_F(OpenwireSslXATransactionsTest, testXAResource_Exception3)
{
    std::unique_ptr<XAConnectionFactory> factory(
        XAConnectionFactory::createCMSXAConnectionFactory(getBrokerURL()));
    ASSERT_TRUE(factory.get() != NULL);
    ...
    ASSERT_THROW(xaResource->end(ixIdOther.get(), XAResource::TMSUSPEND),
                 XAException)
