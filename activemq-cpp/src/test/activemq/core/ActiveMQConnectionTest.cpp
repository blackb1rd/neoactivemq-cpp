...
private:
    ...
    TestCloseCancelsHungStartRunnable& operator=(
        const TestCloseCancelsHungStartRunnable);

    std::unique_ptr<cms::Connection> connection;

public:
    TestCloseCancelsHungStartRunnable()
        : connection(nullptr)
    {
    }
        ...
        try
        {
            std::unique_ptr<ActiveMQConnectionFactory> factory(
                new ActiveMQConnectionFactory(
                    "failover://(tcp://123.132.0.1:61616)"));
...
TEST_F(ActiveMQConnectionTest, testExceptionInOnException)
{
    try
    {
        ...
        transport::mock::MockTransport* transport =
            transport::mock::MockTransport::getInstance();
