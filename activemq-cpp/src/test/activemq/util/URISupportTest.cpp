...
TEST_F(URISupportTest, test)
{
    ...
    string test5 = "failover:(tcp://"
                   "127.0.0.1:61616)?startupMaxReconnectAttempts=10&"
...
{
    CompositeData data =
        URISupport::parseComposite(URI("broker:()/localhost?persistent=false"));
    ...
    data =
        URISupport::parseComposite(URI("test:(part1://host,part2://host,part3:/"
                                       "/host,part4://host?option=value)"));
    ASSERT_TRUE(4 == data.getComponents().size());
    ASSERT_TRUE(data.getComponents().get(0).toString() == "part1://host");
    ASSERT_TRUE(data.getComponents().get(1).toString() == "part2://host");
    ASSERT_TRUE(data.getComponents().get(2).toString() == "part3://host");
    ASSERT_TRUE(data.getComponents().get(3).toString() ==
...
TEST_F(URISupportTest, testCreateWithQuery)
{
    ...
    ASSERT_EQ(2, URISupport::parseParameters(dest).size())
        << ("correct param count");
...
TEST_F(URISupportTest, testApplyParameters)
{
    ...
    ASSERT_EQ(2, appliedParameters.size())
        << ("all params applied with no prefix");
    ...
    ASSERT_TRUE(appliedParameters.isEmpty())
        << ("no params applied as none match joe");
