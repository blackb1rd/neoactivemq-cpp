...
protected:
    ...
    public:
        ...
        {
            const SimpleDataStructure* srcObj =
                dynamic_cast<const SimpleDataStructure*>(src);
            if (srcObj == NULL)
            {
                return;
            }
    ...
    public:
        ...
        virtual void copyDataStructure(const DataStructure* src)
        {
            const ComplexDataStructure* srcObj =
                dynamic_cast<const ComplexDataStructure*>(src);
            if (srcObj == NULL)
            {
                return;
            }
            ...
            {
                cachedChild = dynamic_cast<SimpleDataStructure*>(
                    srcObj->cachedChild->cloneDataStructure());
            }
        }
    };
...
TEST_F(BaseDataStreamMarshallerTest, testLooseMarshal)
{
    SimpleDataStructureMarshaller* simpleMarshaller =
        new SimpleDataStructureMarshaller();
    ComplexDataStructureMarshaller* complexMarshaller =
        new ComplexDataStructureMarshaller();
    Properties     props;
    ...
    ASSERT_EQ(dataStructure->cachedChild->shortValue,
              ds.cachedChild->shortValue);
    ASSERT_EQ(dataStructure->cachedChild->intValue, ds.cachedChild->intValue);
    ASSERT_EQ(dataStructure->cachedChild->longValue1,
              ds.cachedChild->longValue1);
    ASSERT_EQ(dataStructure->cachedChild->longValue2,
              ds.cachedChild->longValue2);
    ASSERT_EQ(dataStructure->cachedChild->longValue3,
              ds.cachedChild->longValue3);
    ASSERT_EQ(dataStructure->cachedChild->longValue4,
              ds.cachedChild->longValue4);
    ASSERT_EQ(dataStructure->cachedChild->longValue5,
              ds.cachedChild->longValue5);
    ASSERT_EQ(dataStructure->cachedChild->floatValue,
              ds.cachedChild->floatValue);
    ASSERT_EQ(dataStructure->cachedChild->doubleValue,
              ds.cachedChild->doubleValue);
    ASSERT_EQ(dataStructure->cachedChild->stringValue,
              ds.cachedChild->stringValue);
...
TEST_F(BaseDataStreamMarshallerTest, testTightMarshal)
{
    ...
    BooleanStream bs;
    size +=
        complexMarshaller->tightMarshal1(&openWireFormat, dataStructure, &bs);
    size += bs.marshalledSize();

    // Phase 2 - marshal
    dataOut.writeByte(dataStructure->getDataStructureType());
    bs.marshal(&dataOut);
    complexMarshaller->tightMarshal2(&openWireFormat,
                                     dataStructure,
                                     &dataOut,
                                     &bs);
