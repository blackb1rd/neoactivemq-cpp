...
TEST_F(URIEncoderDecoderTest, testValidate)
{
    ...
    ASSERT_NO_THROW(URIEncoderDecoder::validate(str1, legalSet1))
        << ("1. String should Validate:");

    ASSERT_NO_THROW(URIEncoderDecoder::validate(str2, ""))
        << ("2. String should Validate:");

    ASSERT_THROW(URIEncoderDecoder::validate(str1, ""), URISyntaxException)
...
TEST_F(URIEncoderDecoderTest, testQuoteIllegal)
{
    ...
    ASSERT_TRUE(URIEncoderDecoder::quoteIllegal(pound1, "") == "%23")
        << ("1. Result not equal to: %23");

    ASSERT_TRUE(URIEncoderDecoder::quoteIllegal(pound2, "") == "%23%23")
        << ("2. Result not equal to: %23%23");

    ASSERT_TRUE(URIEncoderDecoder::quoteIllegal(pound3, "*") == "A%23*")
        << ("3. Result not equal to: A%23*");

    ASSERT_TRUE(URIEncoderDecoder::quoteIllegal(legal, "") == "A")
...
TEST_F(URIEncoderDecoderTest, testEncodeOthers)
{
    ...
    ASSERT_TRUE(URIEncoderDecoder::encodeOthers(test1) == "%9C")
        << ("1. Result not equal to: %9C");

    ASSERT_TRUE(URIEncoderDecoder::encodeOthers(test2) == "A")
...
TEST_F(URIEncoderDecoderTest, testDecode)
{
    ...
    ASSERT_TRUE(URIEncoderDecoder::decode(test) == "A B C $%")
