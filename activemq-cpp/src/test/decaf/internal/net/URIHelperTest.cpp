...
TEST_F(URIHelperTest, testParseURI)
{
    ...
    {
        ASSERT_NO_THROW(URIHelper().parseURI(constructorTests.at(i), false))
    ...
    {
        ASSERT_THROW(URIHelper().parseURI(constructorTestsInvalid.at(i), false),
                     URISyntaxException)
...
TEST_F(URIHelperTest, isValidIPv4Address)
{
    ...
    {
        ASSERT_EQ(true, uriHelper.isValidIPv4Address(*address))
            << ("Valid address tested as invalid");
    }

    address = invalidIPs.begin();
    for (; address != invalidIPs.end(); ++address)
    {
        ASSERT_EQ(false, uriHelper.isValidIPv4Address(*address))
            << ("Invalid address tested as valid");
    }
}
