...
public:
    ...
    virtual int doReadArrayOffsetAndLength(unsigned char* buffer,
                                           int            size,
                                           int            offset,
                                           int            length)
    {
...
TEST_F(BufferedInputStreamTest, testConstructor)
{
...

    {
        ...
        ASSERT_THROW(nullStream.read(), IOException)
...
TEST_F(BufferedInputStreamTest, testAvailable)
{
    ...
    try
    {
        ASSERT_TRUE(is.available() == (int)testStr.length())
...
TEST_F(BufferedInputStreamTest, testMarkSupported)
{
    ...
    ASSERT_TRUE(is.markSupported())
        << ("markSupported returned incorrect value");
}
...
TEST_F(BufferedInputStreamTest, testRead)
{
    ...
    BufferedInputStream is(new ByteArrayInputStream(&bytes[0], 256), 12, true);
...
TEST_F(BufferedInputStreamTest, testRead2)
{
    ...
    try
    {
        ...
        ASSERT_TRUE(string((const char*)&buf1[0], 100) ==
                    testStr.substr(3000, 100))
...
TEST_F(BufferedInputStreamTest, testReadException)
{
    ...
    ASSERT_THROW(bis.read(NULL, 1, 0, 1), IOException)
...
TEST_F(BufferedInputStreamTest, testMarkI)
{
    ...
    stream.setByteArray((const unsigned char*)testString.c_str(),
...
TEST_F(BufferedInputStreamTest, testReset)
{
    ...
    is.read(buf2, 10);
    is.reset();
    ASSERT_EQ(std::string(buf1, buf1 + 10), std::string(buf2, buf2 + 10))
        << ("Reset failed");
...
TEST_F(BufferedInputStreamTest, testSkipJ)
{
    ...
    is.read(buf1, 10);
    is.reset();
    ASSERT_EQ(std::string(buf1, buf1 + 10), testString.substr(1000, 10))
