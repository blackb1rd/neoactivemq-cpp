...
protected:
    void testHelper(unsigned char* input,
                    int            inputLength,
    ...
    void openDataInputStream()
    {
        ...
        this->bais.reset(
            new ByteArrayInputStream(array.first, array.second, true));
...
TEST_F(DataInputStreamTest, testRead1)
{
    try
    {
        ...
        ASSERT_TRUE(string((const char*)&result[0], result.size()) == testData)
            << ("Incorrect data read");
    }
    catch (IOException& e)
    {
...
TEST_F(DataInputStreamTest, testRead2)
{
    try
    {
        ...
        ASSERT_TRUE(string((const char*)result, (int)testData.size()) ==
                    testData)
            << ("Incorrect data read");
        delete[] result;
    }
...
TEST_F(DataInputStreamTest, test_readByte)
{
    try
    {
        ...
        os->close();
        openDataInputStream();
        ASSERT_TRUE(is->readByte() == (unsigned char)127)
            << ("Incorrect byte read");
    }
    catch (IOException& e)
    {
...
TEST_F(DataInputStreamTest, test_readDouble)
{
    try
    {
        ...
        os->close();
        openDataInputStream();
        ASSERT_TRUE(2345.76834720202 == is->readDouble())
            << ("Incorrect double read");
    }
    catch (IOException& e)
    {
...
TEST_F(DataInputStreamTest, test_readFully1)
{
    ...
    ASSERT_THROW(is->readFully(&result[0], -1), IndexOutOfBoundsException)
...
TEST_F(DataInputStreamTest, test_readFully2)
{
    ...
    ASSERT_THROW(is->readFully(&temp[0], (int)temp.size(), -1, -1),
                 IndexOutOfBoundsException)
        << ("should throw IndexOutOfBoundsException");

    ASSERT_THROW(is->readFully(&temp[0], (int)temp.size(), 0, -1),
                 IndexOutOfBoundsException)
        << ("should throw IndexOutOfBoundsException");
...
TEST_F(DataInputStreamTest, test_readFullyNullArray)
{
    ...
    ASSERT_THROW(is.readFully(nullByteArray, 0, 0, 1), NullPointerException)
        << ("should throw NullPointerException");

    ASSERT_THROW(is.readFully(nullByteArray, 0, 1, 1), NullPointerException)
        << ("should throw NullPointerException");

    ASSERT_THROW(is.readFully(nullByteArray, 0, 1, Integer::MAX_VALUE),
                 NullPointerException)
        << ("should throw NullPointerException");
}
...
TEST_F(DataInputStreamTest, test_readFullyNullStream)
{
    ...
    ASSERT_THROW(is.readFully(byteArray, (int)testData.length(), 1, 1),
                 NullPointerException)
        << ("should throw NullPointerException");

    ASSERT_THROW(is.readFully(byteArray, (int)testData.length(), 0, 1),
                 NullPointerException)
        << ("should throw NullPointerException");
...
TEST_F(DataInputStreamTest, test_readLong)
{
    try
    {
        ...
        os->close();
        openDataInputStream();
        ASSERT_TRUE(9875645283333LL == is->readLong())
            << ("Incorrect long read");
    }
    catch (IOException& e)
    {
...
TEST_F(DataInputStreamTest, test_readUnsignedByte)
{
    ...
    {
        FAIL() << ("IOException during readUnsignedByte test : " +
                   e.getMessage());
    }
}
...
TEST_F(DataInputStreamTest, test_skipBytes)
{
    try
    {
        ...
        is->read(&result[0], 50);
        is->close();
        ASSERT_TRUE(string((const char*)&result[0], 50) ==
                    testData.substr(100, 50))
            << ("Incorrect data read");
    }
    catch (IOException& e)
    {
    ...
    try
    {
        ...
        ASSERT_NO_THROW(skipped = (std::size_t)is->skip(500000))
            << ("Should throw an EOFException");

        ASSERT_TRUE(skipped == testData.length())
...
TEST_F(DataInputStreamTest, test)
{
    ...
    unsigned char      arrayVal[3] = {'a', 'b', 'c'};
...
////////////////////////////////////////////////////////////////////////////////
void DataInputStreamTest::testHelper(unsigned char* input,
                                     int            inputLength,
                                     unsigned char* expect,
                                     int            expectLength)
{
...
TEST_F(DataInputStreamTest, testUTFDecoding)
{
    ...
    // Test with bad UTF-8 encoding, missing 2nd byte of two byte value
    {
        ...
        ASSERT_THROW(reader.readUTF(), UTFDataFormatException)
