...
TEST_F(DataOutputStreamTest, testWrite1)
{
    try
    {
        ...
        is->read(rbuf, 150, 0, 150);
        is->close();
        ASSERT_TRUE(string((const char*)rbuf, 150) == testData.substr(0, 150))
...
TEST_F(DataOutputStreamTest, testWriteByte)
{
    try
    {
        ...
        is->close();
        ASSERT_TRUE(c == (unsigned char)127)
            << ("Incorrect unsigned char written");
    }
    catch (IOException& e)
    {
...
TEST_F(DataOutputStreamTest, testWriteUTF)
{
    ...
    os->close();
    openDataInputStream();
    ASSERT_TRUE(is->available() == (int)testString.length() + 2)
        << ("Failed to write string in UTF format");
...
TEST_F(DataOutputStreamTest, testWriteUTFStringLength)
{
    ...
    ASSERT_THROW(os->writeUTF(testStr), UTFDataFormatException)
        << ("Should throw a UTFDataFormatException");

    baos->reset();
    // String of length 65535 of non Null Characters since Null encodes as
    // UTF-8. Expect: Success.
    testStr.resize(65535);
    ASSERT_NO_THROW(os->writeUTF(testStr))
        << ("String of 65535 Non-Null chars should not throw.");

    baos->reset();
    // Set one of the 65535 bytes to a value that will result in a 2 byte UTF8
    // encoded sequence. This will cause the string of length 65535 to have a
    // utf length of 65536. Expect: UTFDataFormatException.
    testStr[0] = char(255);
    ASSERT_THROW(os->writeUTF(testStr), UTFDataFormatException)
...
////////////////////////////////////////////////////////////////////////////////
void DataOutputStreamTest::testHelper(unsigned char* input,
                                      int            inputLength,
                                      unsigned char* expect,
                                      int            expectLength)
{
