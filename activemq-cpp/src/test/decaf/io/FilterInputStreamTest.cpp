...
TEST_F(FilterInputStreamTest, testAvailable)
{
    ...
    ASSERT_EQ((int)testStr.length(), available)
...
TEST_F(FilterInputStreamTest, testRead2)
{
    ...
    is.read(buf, 30, 0, 30);
    ASSERT_TRUE(string((const char*)buf, 30) == testStr.substr(0, 30))
...
TEST_F(FilterInputStreamTest, testRead3)
{
    ...
    ASSERT_TRUE(bufferString == testSubString)
        << ("Failed to read correct data");
}
...
TEST_F(FilterInputStreamTest, testSkip)
{
    ...
    is.skip(1000);
    is.read(buf, 100, 0, 100);
    ASSERT_TRUE(string((const char*)buf, 100) == testStr.substr(1000, 100))
...
TEST_F(FilterInputStreamTest, testReadBIIIExceptions)
{
...

    {
        ...
        is.close();
        ASSERT_THROW(is.read(buf, 1000, 0, 100), IOException)
