...
////////////////////////////////////////////////////////////////////////////////
const std::string InputStreamReaderTest::TEST_STRING =
    "This is a test message with some simple text in it.";
...
{
    this->buffer1 =
        new ByteArrayInputStream((unsigned char*)TEST_STRING.c_str(),
...
TEST_F(InputStreamReaderTest, testClose)
{
    ...
    ASSERT_THROW(this->reader1->read(), IOException)
        << ("Should throw an IOException");

    ASSERT_THROW(this->reader1->ready(), IOException)
        << ("Should throw an IOException");
...
TEST_F(InputStreamReaderTest, testRead)
{
    ...
    this->reader1->read(&buffer[0],
                        (int)buffer.size(),
                        0,
                        (int)TEST_STRING.length() - 5);
