...
public:
    ...
    MockReader(std::vector<char>& data)
        : Reader(),
          contents(data),
          current_offset(0),
          length((int)contents.size())
    {
    }
...
TEST_F(ReaderTest, testReaderCharBufferZeroChar)
{
    ...
    // to the CharBuffer is 0. Furthermore, the MockReader is intact in its
    // content.
    std::string       s = "MY TEST STRING";
    std::vector<char> srcBuffer(s.begin(), s.end());
...
TEST_F(ReaderTest, testReaderCharBufferChar)
{
    ...
    CharSequence* subseq =
        charBuffer->subSequence(CHARBUFFER_SIZE - CHARBUFFER_REMAINING,
                                CHARBUFFER_SIZE);
    ...
    mockReader.read(destBuffer);
    ASSERT_EQ(s.substr(CHARBUFFER_REMAINING),
...
TEST_F(ReaderTest, testMark)
{
    ...
    ASSERT_THROW(mockReader.mark(0), IOException)
...
TEST_F(ReaderTest, testRead)
{
    ...
    {
        char c = srcBuffer[ix];
        ASSERT_EQ((int)c, mockReader.read())
...
{
    MockReader mockReader;
    ASSERT_THROW(mockReader.reset(), IOException)
