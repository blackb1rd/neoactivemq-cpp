...
{
    ASSERT_THROW(MessageDigest::getInstance("MD3"), NoSuchAlgorithmException)
...
TEST_F(MessageDigestTest, testResults1)
{
    ...
    bytes = "";
    result =
        digest->digest((const unsigned char*)bytes.data(), (int)bytes.size());
    ASSERT_EQ(std::string("d41d8cd98f00b204e9800998ecf8427e"),
              toString(result));

    bytes = "a";
    result =
        digest->digest((const unsigned char*)bytes.data(), (int)bytes.size());
    ASSERT_EQ(std::string("0cc175b9c0f1b6a831c399e269772661"),
              toString(result));
    ...
    result =
        digest->digest((const unsigned char*)bytes.data(), (int)bytes.size());
    ASSERT_EQ(std::string("c3fcd3d76192e4007dfb496cca67e13b"),
              toString(result));

    bytes = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
    result =
        digest->digest((const unsigned char*)bytes.data(), (int)bytes.size());
    ASSERT_EQ(std::string("d174ab98d277d9f5a5611c2c9f419d9f"),
              toString(result));

    bytes = "123456789012345678901234567890123456789012345678901234567890123456"
            "78901234567890";
    result =
        digest->digest((const unsigned char*)bytes.data(), (int)bytes.size());
    ASSERT_EQ(std::string("57edf4a22be3c955ac49da2e2107b67a"),
              toString(result));
}
...
TEST_F(MessageDigestTest, testResults2)
{
    ...
    result =
        digest->digest((const unsigned char*)bytes.data(), (int)bytes.size());
    ASSERT_EQ(std::string("31d6cfe0d16ae931b73c59d7e0c089c0"),
              toString(result));

    bytes = "a";
    result =
        digest->digest((const unsigned char*)bytes.data(), (int)bytes.size());
    ASSERT_EQ(std::string("bde52cb31de33e46245e05fbdbd6fb24"),
              toString(result));

    bytes = "abc";
    result =
        digest->digest((const unsigned char*)bytes.data(), (int)bytes.size());
    ASSERT_EQ(std::string("a448017aaf21d8525fc10ae87aa6729d"),
              toString(result));

    bytes = "message digest";
    result =
        digest->digest((const unsigned char*)bytes.data(), (int)bytes.size());
    ASSERT_EQ(std::string("d9130a8164549fe818874806e1c7014b"),
              toString(result));

    bytes = "abcdefghijklmnopqrstuvwxyz";
    result =
        digest->digest((const unsigned char*)bytes.data(), (int)bytes.size());
    ASSERT_EQ(std::string("d79e1c308aa5bbcdeea8ed63df412da9"),
              toString(result));

    bytes = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
    result =
        digest->digest((const unsigned char*)bytes.data(), (int)bytes.size());
    ASSERT_EQ(std::string("043f8582f241db351ce627e153e7f0e4"),
              toString(result));
    ...
    result =
        digest->digest((const unsigned char*)bytes.data(), (int)bytes.size());
    ASSERT_EQ(std::string("e33b4ddc9c38f2199c3e7b164fcc0536"),
              toString(result));
}
...
TEST_F(MessageDigestTest, testResults3)
{
    ...
    //    bytes = "";
    //    result = digest->digest((const unsigned char*)bytes.data(),
    //    (int)bytes.size());
    //    ASSERT_EQ(std::string("da39a3ee5e6b4b0d3255bfef95601890afd80709"),
    //    toString(result));
    //
    //    bytes = "The quick brown fox jumps over the lazy dog";
    //    result = digest->digest((const unsigned char*)bytes.data(),
    //    (int)bytes.size());
    //    ASSERT_EQ(std::string("2fd4e1c67a2d28fced849ee1bb76e7391b93eb12"),
    //    toString(result));
}
