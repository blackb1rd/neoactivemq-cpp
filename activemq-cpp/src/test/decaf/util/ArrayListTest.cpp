...
TEST_F(ArrayListTest, testAdd2)
{
    ...
    ASSERT_TRUE(array.get(51) == mirror[50] && (array.get(52) == mirror[51]))
        << ("Failed to fix up list after insert");
    int oldItem = array.get(25);
    array.add(25, 0);
    ASSERT_TRUE(array.get(25) == 0) << ("Should have returned zero");
    ASSERT_TRUE(array.get(26) == oldItem)
        << ("Should have returned the old item from slot 25");
...
TEST_F(ArrayListTest, testAddAll1)
{
    ...
    ASSERT_EQ(200, array.size())
        << ("Returned incorrect size after adding to existing list");

    for (int i = 0; i < 50; i++)
    {
        ASSERT_TRUE(array.get(i) == mirror[i])
            << ("Manipulated elements < index");
    }

    for (int i = 0; i >= 50 && (i < 150); i++)
    {
        ASSERT_TRUE(array.get(i) == mirror[i - 50])
            << ("Failed to ad elements properly");
    }

    for (int i = 0; i >= 150 && (i < 200); i++)
    {
        ASSERT_TRUE(array.get(i) == mirror[i - 100])
            << ("Failed to ad elements properly");
    }
}
...
TEST_F(ArrayListTest, testAddAll2)
{
...

    {
        ...
        list1.addAll(list2);
        ASSERT_TRUE(list1.containsAll(list2) && list2.containsAll(list1))
        ...
        ASSERT_TRUE(list3.containsAll(list1) && list1.containsAll(list3))
            << ("The object list is not the same as original list");
    }
    {
        ArrayList<std::string> list1;
...
TEST_F(ArrayListTest, testAddAll5)
{
    ...
    {
        ASSERT_EQ(l.get(i), array.get(i))
            << ("Failed to add elements properly");
    }
    array.addAll(array);
    ASSERT_EQ(200, array.size())
        << ("Returned incorrect size after adding to existing list");
    ...
    for (int i = 0; i < 100; i++)
    {
        ASSERT_EQ(array.get(i), l.get(i))
            << ("Added to list in incorrect order");
        ASSERT_EQ(array.get(i + 100), l.get(i))
            << ("Failed to add to existing list");
    }
...
TEST_F(ArrayListTest, testContains)
{
    ...
    ASSERT_TRUE(array.contains(mirror[99]))
        << ("Returned false for valid element");
...
TEST_F(ArrayListTest, testEnsureCapacity)
{
    ...
    ASSERT_TRUE(location == array.indexOf(testValue))
        << ("EnsureCapacity moved objects around in array1.");
    array.removeAt(0);
    array.ensureCapacity(capacity);
    ASSERT_TRUE(--location == array.indexOf(testValue))
        << ("EnsureCapacity moved objects around in array2.");
    array.ensureCapacity(capacity + 2);
    ASSERT_TRUE(location == array.indexOf(testValue))
        << ("EnsureCapacity did not change location.");
...
TEST_F(ArrayListTest, testIndexOf)
{
    ...
    ASSERT_EQ(-1, array.indexOf(SIZE + 10))
        << ("Returned index for invalid Object");
}
...
TEST_F(ArrayListTest, testIsEmpty)
{
    ...
    ASSERT_TRUE(ArrayList<int>(10).isEmpty())
        << ("isEmpty returned false for new list");
    ASSERT_TRUE(!array.isEmpty())
        << ("Returned true for existing list with elements");
}
...
TEST_F(ArrayListTest, testLastIndexOf)
{
    ...
    ASSERT_EQ(-1, array.lastIndexOf(2048))
        << ("Returned index for invalid Object");
...
TEST_F(ArrayListTest, testSet)
{
    ...
    ASSERT_TRUE(array.size() == SIZE)
        << (std::string("") + "Setting increased the list's size to: " +
...
TEST_F(ArrayListTest, testSize)
{
    ...
    ASSERT_EQ(SIZE, array.size())
        << ("Returned incorrect size for exiting list");
    ASSERT_EQ(0, ArrayList<int>().size())
        << ("Returned incorrect size for new list");
}
...
TEST_F(ArrayListTest, testToArray)
{
    ...
    for (int i = 0; i < (int)stlvec.size(); i++)
    {
        ...
        {
            ASSERT_EQ(array.get(i), stlvec[i])
...
TEST_F(ArrayListTest, testTrimToSize)
{
    ...
    {
        ASSERT_TRUE(array.get(i) == mirror[i])
            << ("Trimmed list contained incorrect elements");
    }
