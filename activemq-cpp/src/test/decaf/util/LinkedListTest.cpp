...
TEST_F(LinkedListTest, testRemoveByValue)
{
    ...
    list.add(SIZE + 1);
    list.remove(SIZE + 1);
    ASSERT_TRUE(!list.contains(SIZE + 1))
...
TEST_F(LinkedListTest, testListIterator1)
{
    ...
    while (iter->hasNext())
    {
        if (index == 0)
        {
            ASSERT_TRUE(!iter->hasPrevious())
                << ("The first element cannot have a previous");
        }
        if (index == SIZE)
        {
            FAIL() << (std::string("List index should be capped at ") +
        ...
        {
            ASSERT_TRUE(!iter->hasNext())
                << ("The last element cannot have a next");
        }
        ...
        {
            ASSERT_TRUE(iter->nextIndex() == index + 1)
                << ("Next index returned incorrect value");
            ASSERT_EQ(iter->previousIndex(), index)
                << ("previousIndex returned incorrect value :");
        }
