...
{
    {
        Properties           properties;
        string               value(" a= b");
        ByteArrayInputStream stream((const unsigned char*)value.c_str(),
                                    (int)value.size());
...
TEST_F(PropertiesTest, testPropertyNames)
{
    ...
    {
        ASSERT_TRUE(*name == "test.prop" || *name == "bogus.prop")
            << ("Incorrect names returned");
    }
}
...
TEST_F(PropertiesTest, testStoreOutputStream)
{
    ...
    {
        FAIL() << (string("IOException occurred reading/writing file : ") +
                   ioe.getMessage());
    }
    ...
    for (; iterator != nameSet.end(); ++iterator)
    {
        ASSERT_TRUE(string(myProps2.getProperty(*iterator)) ==
                    string(myProps.getProperty(*iterator)));
    }
}
