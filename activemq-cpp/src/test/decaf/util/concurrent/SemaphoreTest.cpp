...
private:
    TestAcquireReleaseInDifferentThreadsRunnable(
        const TestAcquireReleaseInDifferentThreadsRunnable&);
    TestAcquireReleaseInDifferentThreadsRunnable operator=(
        const TestAcquireReleaseInDifferentThreadsRunnable&);

public:
    TestAcquireReleaseInDifferentThreadsRunnable(Semaphore*     sem,
                                                 SemaphoreTest* parent)
        : Runnable(),
          sem(sem),
          parent(parent)
...
class TestUninterruptibleAcquireReleaseInDifferentThreadsRunnable
    : public Runnable
{
private:
    Semaphore*     sem;
...
private:
    TestUninterruptibleAcquireReleaseInDifferentThreadsRunnable(
        const TestUninterruptibleAcquireReleaseInDifferentThreadsRunnable&);
    TestUninterruptibleAcquireReleaseInDifferentThreadsRunnable operator=(
        const TestUninterruptibleAcquireReleaseInDifferentThreadsRunnable&);
...
{
    Semaphore                                                   s(0, false);
    TestUninterruptibleAcquireReleaseInDifferentThreadsRunnable runnable(&s,
                                                                         this);
    Thread                                                      t(&runnable);
...
private:
    TestTimedAcquireReleaseInDifferentThreadsRunnable(
        const TestTimedAcquireReleaseInDifferentThreadsRunnable&);
    TestTimedAcquireReleaseInDifferentThreadsRunnable operator=(
        const TestTimedAcquireReleaseInDifferentThreadsRunnable&);

public:
    TestTimedAcquireReleaseInDifferentThreadsRunnable(Semaphore*     sem,
                                                      SemaphoreTest* parent)
        : Runnable(),
          sem(sem),
          parent(parent)
...
public:
    ...
    virtual void run()
    {
        try
        {
            sem->tryAcquire(SemaphoreTest::MEDIUM_DELAY_MS,
                            TimeUnit::MILLISECONDS);
            parent->threadShouldThrow();
        }
        catch (InterruptedException& success)
...
private:
    TestAcquireReleaseInDifferentThreadsFairRunnable(
        const TestAcquireReleaseInDifferentThreadsFairRunnable&);
    TestAcquireReleaseInDifferentThreadsFairRunnable operator=(
        const TestAcquireReleaseInDifferentThreadsFairRunnable&);

public:
    TestAcquireReleaseInDifferentThreadsFairRunnable(Semaphore*     sem,
                                                     SemaphoreTest* parent)
        : Runnable(),
          sem(sem),
          parent(parent)
...
private:
    TestAcquireReleaseNInDifferentThreadsFairRunnable(
        const TestAcquireReleaseNInDifferentThreadsFairRunnable&);
    TestAcquireReleaseNInDifferentThreadsFairRunnable operator=(
        const TestAcquireReleaseNInDifferentThreadsFairRunnable&);

public:
    TestAcquireReleaseNInDifferentThreadsFairRunnable(Semaphore*     sem,
                                                      SemaphoreTest* parent)
        : Runnable(),
          sem(sem),
          parent(parent)
...
private:
    TestAcquireReleaseNInDifferentThreadsFair2Runnable(
        const TestAcquireReleaseNInDifferentThreadsFair2Runnable&);
    TestAcquireReleaseNInDifferentThreadsFair2Runnable operator=(
        const TestAcquireReleaseNInDifferentThreadsFair2Runnable&);

public:
    TestAcquireReleaseNInDifferentThreadsFair2Runnable(Semaphore*     sem,
                                                       SemaphoreTest* parent)
        : Runnable(),
          sem(sem),
          parent(parent)
...
private:
    TestTimedAcquireReleaseInDifferentThreadsFairRunnable(
        const TestTimedAcquireReleaseInDifferentThreadsFairRunnable&);
    TestTimedAcquireReleaseInDifferentThreadsFairRunnable operator=(
        const TestTimedAcquireReleaseInDifferentThreadsFairRunnable&);

public:
    TestTimedAcquireReleaseInDifferentThreadsFairRunnable(Semaphore*     sem,
                                                          SemaphoreTest* parent)
        : Runnable(),
          sem(sem),
          parent(parent)
...
private:
    TestTimedAcquireReleaseNInDifferentThreadsFairRunnable(
        const TestTimedAcquireReleaseNInDifferentThreadsFairRunnable&);
    TestTimedAcquireReleaseNInDifferentThreadsFairRunnable operator=(
        const TestTimedAcquireReleaseNInDifferentThreadsFairRunnable&);

public:
    TestTimedAcquireReleaseNInDifferentThreadsFairRunnable(Semaphore* sem,
                                                           SemaphoreTest* parent)
        : Runnable(),
          sem(sem),
          parent(parent)
